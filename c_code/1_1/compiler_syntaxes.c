/* Generated by Nim Compiler v0.11.2 */
/*   (c) 2015 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 32
#include "nimbase.h"

#include <string.h>

#include <stdio.h>
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct Tnode193813 Tnode193813;
typedef struct Tparsers221035 Tparsers221035;
typedef struct Tparser209205 Tparser209205;
typedef struct Tlexer183185 Tlexer183185;
typedef struct Tbaselexer181024 Tbaselexer181024;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct Tllstream180204 Tllstream180204;
typedef struct Tlineinfo165338 Tlineinfo165338;
typedef struct Ttoken183181 Ttoken183181;
typedef struct Tident169021 Tident169021;
typedef struct Tidobj169015 Tidobj169015;
typedef struct Ttype193849 Ttype193849;
typedef struct Tsym193843 Tsym193843;
typedef struct Tnodeseq193807 Tnodeseq193807;
typedef struct Ttypeseq193845 Ttypeseq193845;
typedef struct Tloc193827 Tloc193827;
typedef struct Ropeobj163009 Ropeobj163009;
typedef struct Tscope193837 Tscope193837;
typedef struct TY193944 TY193944;
typedef struct Tstrtable193817 Tstrtable193817;
typedef struct Tsymseq193815 Tsymseq193815;
typedef struct Tlib193831 Tlib193831;
typedef struct Tinstantiation193833 Tinstantiation193833;
typedef struct Tlistentry133022 Tlistentry133022;
typedef struct TY193933 TY193933;
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
typedef NimStringDesc* TY221020[4];
typedef NimStringDesc* TY221026[4];
typedef N_NIMCALL_PTR(void, TY3289) (void* p, NI op);
typedef N_NIMCALL_PTR(void*, TY3294) (void* p);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY3289 marker;
TY3294 deepcopy;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  Tbaselexer181024  {
  TNimObject Sup;
NI bufpos;
NCSTRING buf;
NI buflen;
Tllstream180204* stream;
NI linenumber;
NI sentinel;
NI linestart;
};
struct  Tlineinfo165338  {
NI16 line;
NI16 col;
NI32 fileindex;
};
typedef struct {
N_NIMCALL_PTR(void, ClPrc) (Tlineinfo165338 info, NU16 msg, NimStringDesc* arg, void* ClEnv);
void* ClEnv;
} Terrorhandler183183;
struct  Tlexer183185  {
  Tbaselexer181024 Sup;
NI32 fileidx;
NI indentahead;
NI currlineindent;
NIM_BOOL strongspaces;
Terrorhandler183183 errorhandler;
};
struct  Ttoken183181  {
NU8 toktype;
NI indent;
Tident169021* ident;
NI64 inumber;
NF fnumber;
NU8 base;
NI8 strongspacea;
NI8 strongspaceb;
NimStringDesc* literal;
NI line;
NI col;
};
struct  Tparser209205  {
NI currind;
NIM_BOOL firsttok;
NIM_BOOL strongspaces;
Tlexer183185 lex;
Ttoken183181 tok;
NI inpragma;
NI insemistmtlist;
};
struct  Tparsers221035  {
NU8 skin;
Tparser209205 parser;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct  Tidobj169015  {
  TNimObject Sup;
NI id;
};
struct  Tident169021  {
  Tidobj169015 Sup;
NimStringDesc* s;
Tident169021* next;
NI h;
};
struct  Tnode193813  {
Ttype193849* typ;
Tlineinfo165338 info;
NU16 flags;
NU8 kind;
union{
struct {NI64 intval;
} S1;
struct {NF floatval;
} S2;
struct {NimStringDesc* strval;
} S3;
struct {Tsym193843* sym;
} S4;
struct {Tident169021* ident;
} S5;
struct {Tnodeseq193807* sons;
} S6;
} kindU;
NimStringDesc* comment;
};
typedef NU8 TY194031[20];
typedef NimStringDesc* TY146637[1];
struct  Tllstream180204  {
  TNimObject Sup;
NU8 kind;
FILE* f;
NimStringDesc* s;
NI rd;
NI wr;
NI lineoffset;
};
struct  Tloc193827  {
NU8 k;
NU8 s;
NU16 flags;
Ttype193849* t;
Ropeobj163009* r;
Ropeobj163009* heaproot;
};
struct  Ttype193849  {
  Tidobj169015 Sup;
NU8 kind;
NU8 callconv;
NU32 flags;
Ttypeseq193845* sons;
Tnode193813* n;
Tsym193843* owner;
Tsym193843* sym;
Tsym193843* destructor;
Tsym193843* deepcopy;
Tsym193843* assignment;
NI64 size;
NI16 align;
NI16 locklevel;
Tloc193827 loc;
};
struct  Tstrtable193817  {
NI counter;
Tsymseq193815* data;
};
struct  Tsym193843  {
  Tidobj169015 Sup;
NU8 kind;
union{
struct {Ttypeseq193845* typeinstcache;
Tscope193837* typscope;
} S1;
struct {TY193944* procinstcache;
Tsym193843* gcunsafetyreason;
} S2;
struct {TY193944* usedgenerics;
Tstrtable193817 tab;
} S3;
struct {Tsym193843* guard;
} S4;
} kindU;
NU16 magic;
Ttype193849* typ;
Tident169021* name;
Tlineinfo165338 info;
Tsym193843* owner;
NU32 flags;
Tnode193813* ast;
NU32 options;
NI position;
NI offset;
Tloc193827 loc;
Tlib193831* annex;
Tnode193813* constraint;
};
struct  Ropeobj163009  {
  TNimObject Sup;
Ropeobj163009* left;
Ropeobj163009* right;
NI length;
NimStringDesc* data;
};
struct  Tscope193837  {
NI depthlevel;
Tstrtable193817 symbols;
Tnodeseq193807* usingsyms;
Tscope193837* parent;
};
struct  Tlistentry133022  {
  TNimObject Sup;
Tlistentry133022* prev;
Tlistentry133022* next;
};
struct  Tlib193831  {
  Tlistentry133022 Sup;
NU8 kind;
NIM_BOOL generated;
NIM_BOOL isoverriden;
Ropeobj163009* name;
Tnode193813* path;
};
struct  Tinstantiation193833  {
Tsym193843* sym;
Ttypeseq193845* concretetypes;
TY193933* usedby;
};
struct Tnodeseq193807 {
  TGenericSeq Sup;
  Tnode193813* data[SEQ_DECL_SIZE];
};
struct Ttypeseq193845 {
  TGenericSeq Sup;
  Ttype193849* data[SEQ_DECL_SIZE];
};
struct TY193944 {
  TGenericSeq Sup;
  Tinstantiation193833* data[SEQ_DECL_SIZE];
};
struct Tsymseq193815 {
  TGenericSeq Sup;
  Tsym193843* data[SEQ_DECL_SIZE];
};
struct TY193933 {
  TGenericSeq Sup;
  NI32 data[SEQ_DECL_SIZE];
};
N_NIMCALL(Tnode193813*, parsefile_221041)(NI32 fileidx);
N_NIMCALL(void, objectInit)(void* dest, TNimType* typ);
N_NIMCALL(NimStringDesc*, tofullpathconsiderdirty_166407)(NI32 fileidx);
N_NIMCALL(NIM_BOOL, open_13203)(FILE** f, NimStringDesc* filename, NU8 mode, NI bufsize);
N_NIMCALL(void, rawmessage_166960)(NU16 msg, NimStringDesc* arg);
N_NIMCALL(Tllstream180204*, llstreamopen_180238)(FILE* f);
static N_INLINE(void, nimFrame)(TFrame* s);
N_NOINLINE(void, stackoverflow_20001)(void);
static N_INLINE(void, popFrame)(void);
N_NIMCALL(Tnode193813*, parseall_221063)(Tparsers221035* p);
N_NIMCALL(Tnode193813*, parseall_209216)(Tparser209205* p);
N_NIMCALL(Tnode193813*, parseall_214009)(Tparser209205* p);
N_NIMCALL(void, internalerror_167218)(NimStringDesc* errmsg);
N_NIMCALL(Tnode193813*, parsetoplevelstmt_221070)(Tparsers221035* p);
N_NIMCALL(Tnode193813*, parsetoplevelstmt_209230)(Tparser209205* p);
N_NIMCALL(Tnode193813*, parsetoplevelstmt_214020)(Tparser209205* p);
N_NIMCALL(NI, utf8bom_221122)(NimStringDesc* s);
N_NOINLINE(void, raiseIndexError)(void);
N_NIMCALL(NIM_BOOL, containsshebang_221132)(NimStringDesc* s, NI i);
static N_INLINE(NI, addInt)(NI a, NI b);
N_NOINLINE(void, raiseOverflow)(void);
N_NIMCALL(Tnode193813*, parsepipe_221170)(NimStringDesc* filename, Tllstream180204* inputstream);
N_NIMCALL(Tllstream180204*, llstreamopen_180256)(NimStringDesc* filename, NU8 mode);
N_NIMCALL(NimStringDesc*, rawNewString)(NI space);
N_NIMCALL(NimStringDesc*, rawNewString)(NI cap);
N_NIMCALL(NIM_BOOL, llstreamreadline_180610)(Tllstream180204* s, NimStringDesc** line);
N_NIMCALL(void, openparser_209417)(Tparser209205* p, NimStringDesc* filename, Tllstream180204* inputstream, NIM_BOOL strongspaces);
N_NIMCALL(Tllstream180204*, llstreamopen_180220)(NimStringDesc* data);
N_NIMCALL(NimStringDesc*, copyStr)(NimStringDesc* s, NI start);
N_NIMCALL(NimStringDesc*, copyStr)(NimStringDesc* s, NI first);
N_NIMCALL(void, closeparser_209223)(Tparser209205* p);
N_NIMCALL(void, llstreamclose_180310)(Tllstream180204* s);
N_NIMCALL(NU8, getfilter_221239)(Tident169021* ident);
static N_INLINE(NI, chckRange)(NI i, NI a, NI b);
N_NOINLINE(void, raiseRangeError)(NI64 val);
N_NIMCALL(NIM_BOOL, identeq_169484)(Tident169021* id, NimStringDesc* name);
N_NIMCALL(NU8, getparser_221447)(Tident169021* ident);
N_NIMCALL(Tident169021*, getcallee_221647)(Tnode193813* n);
N_NOINLINE(void, raiseFieldError)(NimStringDesc* f);
N_NIMCALL(NimStringDesc*, rendertree_215058)(Tnode193813* n, NU8 renderflags);
N_NIMCALL(Tllstream180204*, applyfilter_221689)(Tparsers221035* p, Tnode193813* n, NimStringDesc* filename, Tllstream180204* stdin_221695);
N_NIMCALL(Tllstream180204*, filtertmpl_220009)(Tllstream180204* stdin_220011, NimStringDesc* filename, Tnode193813* call);
N_NIMCALL(Tllstream180204*, filterstrip_219012)(Tllstream180204* stdin_219014, NimStringDesc* filename, Tnode193813* call);
N_NIMCALL(Tllstream180204*, filterreplace_219004)(Tllstream180204* stdin_219006, NimStringDesc* filename, Tnode193813* call);
N_NIMCALL(void, rawmessage_166888)(NU16 msg, NimStringDesc** args, NI argsLen0);
N_NIMCALL(void, msgwriteln_166536)(NimStringDesc* s);
N_NIMCALL(Tllstream180204*, evalpipe_221722)(Tparsers221035* p, Tnode193813* n, NimStringDesc* filename, Tllstream180204* start);
N_NIMCALL(void, openparsers_221047)(Tparsers221035* p, NI32 fileidx, Tllstream180204* inputstream);
N_NIMCALL(void, openparser_209402)(Tparser209205* p, NI32 fileidx, Tllstream180204* inputstream, NIM_BOOL strongspaces);
N_NIMCALL(void, closeparsers_221056)(Tparsers221035* p);
STRING_LITERAL(TMP3383, "standard", 8);
STRING_LITERAL(TMP3384, "strongspaces", 12);
STRING_LITERAL(TMP3385, "braces", 6);
STRING_LITERAL(TMP3386, "endx", 4);
NIM_CONST TY221020 parsernames_221019 = {((NimStringDesc*) &TMP3383),
((NimStringDesc*) &TMP3384),
((NimStringDesc*) &TMP3385),
((NimStringDesc*) &TMP3386)}
;
STRING_LITERAL(TMP3387, "none", 4);
STRING_LITERAL(TMP3388, "stdtmpl", 7);
STRING_LITERAL(TMP3389, "replace", 7);
STRING_LITERAL(TMP3390, "strip", 5);
NIM_CONST TY221026 filternames_221025 = {((NimStringDesc*) &TMP3387),
((NimStringDesc*) &TMP3388),
((NimStringDesc*) &TMP3389),
((NimStringDesc*) &TMP3390)}
;
STRING_LITERAL(TMP3395, "parser to implement", 19);
static NIM_CONST TY194031 TMP3404 = {
0xEC, 0xFF, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP3405, "sons", 4);
static NIM_CONST TY194031 TMP3406 = {
0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP3407, "ident", 5);
STRING_LITERAL(TMP3408, "|", 1);
TNimType NTI221035; /* TParsers */
TNimType NTI221007; /* TParserKind */
extern TNimType NTI209205; /* TParser */
extern TFrame* frameptr_17042;
extern Tnode193813* emptynode_194807;
extern NI gverbosity_156137;

static N_INLINE(void, nimFrame)(TFrame* s) {
	NI LOC1;
	LOC1 = 0;
	{
		if (!(frameptr_17042 == NIM_NIL)) goto LA4;
		LOC1 = ((NI) 0);
	}
	goto LA2;
	LA4: ;
	{
		LOC1 = ((NI) ((NI16)((*frameptr_17042).calldepth + ((NI16) 1))));
	}
	LA2: ;
	(*s).calldepth = ((NI16) (LOC1));
	(*s).prev = frameptr_17042;
	frameptr_17042 = s;
	{
		if (!((*s).calldepth == ((NI16) 2000))) goto LA9;
		stackoverflow_20001();
	}
	LA9: ;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_17042 = (*frameptr_17042).prev;
}

N_NIMCALL(Tnode193813*, parsefile_221041)(NI32 fileidx) {
	Tnode193813* result;
	Tparsers221035 p;
	FILE* f;
	NimStringDesc* filename;
	Tllstream180204* LOC6;
	nimfr("parseFile", "syntaxes.nim")
{	result = 0;
	memset((void*)(&p), 0, sizeof(p));
	objectInit((&p), (&NTI221035));
	f = 0;
	nimln(48, "syntaxes.nim");
	filename = tofullpathconsiderdirty_166407(fileidx);
	nimln(49, "syntaxes.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = open_13203(&f, filename, ((NU8) 0), ((NI) -1));
		if (!!(LOC3)) goto LA4;
		nimln(50, "syntaxes.nim");
		rawmessage_166960(((NU16) 3), filename);
		nimln(51, "syntaxes.nim");
		goto BeforeRet;
	}
	LA4: ;
	nimln(52, "syntaxes.nim");
	LOC6 = 0;
	LOC6 = llstreamopen_180238(f);
	openparsers_221047((&p), fileidx, LOC6);
	nimln(53, "syntaxes.nim");
	result = parseall_221063((&p));
	nimln(54, "syntaxes.nim");
	closeparsers_221056((&p));
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(Tnode193813*, parseall_221063)(Tparsers221035* p) {
	Tnode193813* result;
	nimfr("parseAll", "syntaxes.nim")
	result = 0;
	nimln(57, "syntaxes.nim");
	switch ((*p).skin) {
	case ((NU8) 0):
	case ((NU8) 1):
	{
		nimln(59, "syntaxes.nim");
		result = parseall_209216((&(*p).parser));
	}
	break;
	case ((NU8) 2):
	{
		nimln(61, "syntaxes.nim");
		result = parseall_214009((&(*p).parser));
	}
	break;
	case ((NU8) 3):
	{
		nimln(63, "syntaxes.nim");
		internalerror_167218(((NimStringDesc*) &TMP3395));
		nimln(64, "syntaxes.nim");
		result = emptynode_194807;
	}
	break;
	}
	popFrame();
	return result;
}

N_NIMCALL(Tnode193813*, parsetoplevelstmt_221070)(Tparsers221035* p) {
	Tnode193813* result;
	nimfr("parseTopLevelStmt", "syntaxes.nim")
	result = 0;
	nimln(67, "syntaxes.nim");
	switch ((*p).skin) {
	case ((NU8) 0):
	case ((NU8) 1):
	{
		nimln(69, "syntaxes.nim");
		result = parsetoplevelstmt_209230((&(*p).parser));
	}
	break;
	case ((NU8) 2):
	{
		nimln(71, "syntaxes.nim");
		result = parsetoplevelstmt_214020((&(*p).parser));
	}
	break;
	case ((NU8) 3):
	{
		nimln(73, "syntaxes.nim");
		internalerror_167218(((NimStringDesc*) &TMP3395));
		nimln(74, "syntaxes.nim");
		result = emptynode_194807;
	}
	break;
	}
	popFrame();
	return result;
}

N_NIMCALL(NI, utf8bom_221122)(NimStringDesc* s) {
	NI result;
	nimfr("utf8Bom", "syntaxes.nim")
	result = 0;
	nimln(77, "syntaxes.nim");
	{
		NIM_BOOL LOC3;
		NIM_BOOL LOC4;
		LOC3 = 0;
		LOC4 = 0;
		if ((NU)(((NI) 0)) > (NU)(s->Sup.len)) raiseIndexError();
		LOC4 = ((NU8)(s->data[((NI) 0)]) == (NU8)(239));
		if (!(LOC4)) goto LA5;
		if ((NU)(((NI) 1)) > (NU)(s->Sup.len)) raiseIndexError();
		LOC4 = ((NU8)(s->data[((NI) 1)]) == (NU8)(187));
		LA5: ;
		LOC3 = LOC4;
		if (!(LOC3)) goto LA6;
		if ((NU)(((NI) 2)) > (NU)(s->Sup.len)) raiseIndexError();
		LOC3 = ((NU8)(s->data[((NI) 2)]) == (NU8)(191));
		LA6: ;
		if (!LOC3) goto LA7;
		nimln(78, "syntaxes.nim");
		result = ((NI) 3);
	}
	goto LA1;
	LA7: ;
	{
		nimln(80, "syntaxes.nim");
		result = ((NI) 0);
	}
	LA1: ;
	popFrame();
	return result;
}

static N_INLINE(NI, addInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU32)(a) + (NU32)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ b));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

N_NIMCALL(NIM_BOOL, containsshebang_221132)(NimStringDesc* s, NI i) {
	NIM_BOOL result;
	nimfr("containsShebang", "syntaxes.nim")
	result = 0;
	nimln(83, "syntaxes.nim");
	{
		NIM_BOOL LOC3;
		NI TMP3396;
		NI j;
		NI TMP3397;
		LOC3 = 0;
		if ((NU)(i) > (NU)(s->Sup.len)) raiseIndexError();
		LOC3 = ((NU8)(s->data[i]) == (NU8)(35));
		if (!(LOC3)) goto LA4;
		TMP3396 = addInt(i, ((NI) 1));
		if ((NU)((NI)(TMP3396)) > (NU)(s->Sup.len)) raiseIndexError();
		LOC3 = ((NU8)(s->data[(NI)(TMP3396)]) == (NU8)(33));
		LA4: ;
		if (!LOC3) goto LA5;
		nimln(84, "syntaxes.nim");
		TMP3397 = addInt(i, ((NI) 2));
		j = (NI)(TMP3397);
		{
			nimln(85, "syntaxes.nim");
			while (1) {
				NI TMP3398;
				if ((NU)(j) > (NU)(s->Sup.len)) raiseIndexError();
				if (!(((NU8)(s->data[j])) == ((NU8)(32)) || ((NU8)(s->data[j])) == ((NU8)(9)) || ((NU8)(s->data[j])) == ((NU8)(11)) || ((NU8)(s->data[j])) == ((NU8)(13)) || ((NU8)(s->data[j])) == ((NU8)(10)) || ((NU8)(s->data[j])) == ((NU8)(12)))) goto LA8;
				TMP3398 = addInt(j, ((NI) 1));
				j = (NI)(TMP3398);
			} LA8: ;
		}
		nimln(86, "syntaxes.nim");
		if ((NU)(j) > (NU)(s->Sup.len)) raiseIndexError();
		result = ((NU8)(s->data[j]) == (NU8)(47));
	}
	LA5: ;
	popFrame();
	return result;
}

N_NIMCALL(Tnode193813*, parsepipe_221170)(NimStringDesc* filename, Tllstream180204* inputstream) {
	Tnode193813* result;
	Tllstream180204* s;
	nimfr("parsePipe", "syntaxes.nim")
	result = 0;
	nimln(89, "syntaxes.nim");
	result = emptynode_194807;
	nimln(90, "syntaxes.nim");
	s = llstreamopen_180256(filename, ((NU8) 0));
	nimln(91, "syntaxes.nim");
	{
		NimStringDesc* line;
		NIM_BOOL LOC5;
		NI i;
		if (!!((s == NIM_NIL))) goto LA3;
		nimln(92, "syntaxes.nim");
		line = rawNewString(((NI) 80));
		nimln(93, "syntaxes.nim");
		LOC5 = 0;
		LOC5 = llstreamreadline_180610(s, (&line));
		nimln(94, "syntaxes.nim");
		i = utf8bom_221122(line);
		nimln(95, "syntaxes.nim");
		{
			NIM_BOOL LOC8;
			NIM_BOOL LOC11;
			LOC8 = 0;
			LOC8 = containsshebang_221132(line, i);
			if (!LOC8) goto LA9;
			nimln(96, "syntaxes.nim");
			LOC11 = 0;
			LOC11 = llstreamreadline_180610(s, (&line));
			nimln(97, "syntaxes.nim");
			i = ((NI) 0);
		}
		LA9: ;
		nimln(98, "syntaxes.nim");
		{
			NIM_BOOL LOC14;
			NI TMP3399;
			NI TMP3400;
			Tparser209205 q;
			NimStringDesc* LOC20;
			Tllstream180204* LOC21;
			LOC14 = 0;
			if ((NU)(i) > (NU)(line->Sup.len)) raiseIndexError();
			LOC14 = ((NU8)(line->data[i]) == (NU8)(35));
			if (!(LOC14)) goto LA15;
			TMP3399 = addInt(i, ((NI) 1));
			if ((NU)((NI)(TMP3399)) > (NU)(line->Sup.len)) raiseIndexError();
			LOC14 = ((NU8)(line->data[(NI)(TMP3399)]) == (NU8)(33));
			LA15: ;
			if (!LOC14) goto LA16;
			nimln(99, "syntaxes.nim");
			TMP3400 = addInt(i, ((NI) 2));
			i = (NI)(TMP3400);
			{
				nimln(100, "syntaxes.nim");
				while (1) {
					NI TMP3401;
					if ((NU)(i) > (NU)(line->Sup.len)) raiseIndexError();
					if (!(((NU8)(line->data[i])) == ((NU8)(32)) || ((NU8)(line->data[i])) == ((NU8)(9)) || ((NU8)(line->data[i])) == ((NU8)(11)) || ((NU8)(line->data[i])) == ((NU8)(13)) || ((NU8)(line->data[i])) == ((NU8)(10)) || ((NU8)(line->data[i])) == ((NU8)(12)))) goto LA19;
					TMP3401 = addInt(i, ((NI) 1));
					i = (NI)(TMP3401);
				} LA19: ;
			}
			memset((void*)(&q), 0, sizeof(q));
			objectInit((&q), (&NTI209205));
			nimln(102, "syntaxes.nim");
			LOC20 = 0;
			LOC20 = copyStr(line, i);
			LOC21 = 0;
			LOC21 = llstreamopen_180220(LOC20);
			openparser_209417((&q), filename, LOC21, NIM_FALSE);
			nimln(103, "syntaxes.nim");
			result = parseall_209216((&q));
			nimln(104, "syntaxes.nim");
			closeparser_209223((&q));
		}
		LA16: ;
		nimln(105, "syntaxes.nim");
		llstreamclose_180310(s);
	}
	LA3: ;
	popFrame();
	return result;
}

static N_INLINE(NI, chckRange)(NI i, NI a, NI b) {
	NI result;
{	result = 0;
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (a <= i);
		if (!(LOC3)) goto LA4;
		LOC3 = (i <= b);
		LA4: ;
		if (!LOC3) goto LA5;
		result = i;
		goto BeforeRet;
	}
	goto LA1;
	LA5: ;
	{
		raiseRangeError(((NI64) (i)));
	}
	LA1: ;
	}BeforeRet: ;
	return result;
}

N_NIMCALL(NU8, getfilter_221239)(Tident169021* ident) {
	NU8 result;
	nimfr("getFilter", "syntaxes.nim")
{	result = 0;
	{
		NU8 i_221423;
		NI res_221429;
		i_221423 = 0;
		nimln(1598, "system.nim");
		res_221429 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP3402;
				if (!(res_221429 <= ((NI) 3))) goto LA3;
				nimln(1600, "system.nim");
				i_221423 = ((NU8)chckRange(res_221429, ((NU8) 0), ((NU8) 3)));
				nimln(109, "syntaxes.nim");
				{
					NIM_BOOL LOC6;
					LOC6 = 0;
					LOC6 = identeq_169484(ident, filternames_221025[(i_221423)- 0]);
					if (!LOC6) goto LA7;
					nimln(110, "syntaxes.nim");
					result = i_221423;
					goto BeforeRet;
				}
				LA7: ;
				nimln(1614, "system.nim");
				TMP3402 = addInt(res_221429, ((NI) 1));
				res_221429 = (NI)(TMP3402);
			} LA3: ;
		}
	}
	nimln(111, "syntaxes.nim");
	result = ((NU8) 0);
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(NU8, getparser_221447)(Tident169021* ident) {
	NU8 result;
	nimfr("getParser", "syntaxes.nim")
{	result = 0;
	{
		NU8 i_221623;
		NI res_221629;
		i_221623 = 0;
		nimln(1598, "system.nim");
		res_221629 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP3403;
				if (!(res_221629 <= ((NI) 3))) goto LA3;
				nimln(1600, "system.nim");
				i_221623 = ((NU8)chckRange(res_221629, ((NU8) 0), ((NU8) 3)));
				nimln(115, "syntaxes.nim");
				{
					NIM_BOOL LOC6;
					LOC6 = 0;
					LOC6 = identeq_169484(ident, parsernames_221019[(i_221623)- 0]);
					if (!LOC6) goto LA7;
					nimln(116, "syntaxes.nim");
					result = i_221623;
					goto BeforeRet;
				}
				LA7: ;
				nimln(1614, "system.nim");
				TMP3403 = addInt(res_221629, ((NI) 1));
				res_221629 = (NI)(TMP3403);
			} LA3: ;
		}
	}
	nimln(117, "syntaxes.nim");
	rawmessage_166960(((NU16) 30), (*ident).s);
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(Tident169021*, getcallee_221647)(Tnode193813* n) {
	Tident169021* result;
	nimfr("getCallee", "syntaxes.nim")
	result = 0;
	nimln(120, "syntaxes.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = ((*n).kind == ((NU8) 27) || (*n).kind == ((NU8) 29) || (*n).kind == ((NU8) 30) || (*n).kind == ((NU8) 31) || (*n).kind == ((NU8) 26) || (*n).kind == ((NU8) 28) || (*n).kind == ((NU8) 32));
		if (!(LOC3)) goto LA4;
		if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		LOC3 = ((*(*n).kindU.S6.sons->data[((NI) 0)]).kind == ((NU8) 2));
		LA4: ;
		if (!LOC3) goto LA5;
		nimln(121, "syntaxes.nim");
		if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		if (!(((TMP3406[(*(*n).kindU.S6.sons->data[((NI) 0)]).kind/8] &(1<<((*(*n).kindU.S6.sons->data[((NI) 0)]).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP3407));
		result = (*(*n).kindU.S6.sons->data[((NI) 0)]).kindU.S5.ident;
	}
	goto LA1;
	LA5: ;
	{
		nimln(122, "syntaxes.nim");
		if (!((*n).kind == ((NU8) 2))) goto LA8;
		nimln(123, "syntaxes.nim");
		if (!(((TMP3406[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP3407));
		result = (*n).kindU.S5.ident;
	}
	goto LA1;
	LA8: ;
	{
		NimStringDesc* LOC11;
		nimln(125, "syntaxes.nim");
		LOC11 = 0;
		LOC11 = rendertree_215058(n, 0);
		rawmessage_166960(((NU16) 163), LOC11);
	}
	LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(Tllstream180204*, applyfilter_221689)(Tparsers221035* p, Tnode193813* n, NimStringDesc* filename, Tllstream180204* stdin_221695) {
	Tllstream180204* result;
	Tident169021* ident;
	NU8 f;
	nimfr("applyFilter", "syntaxes.nim")
	result = 0;
	nimln(129, "syntaxes.nim");
	ident = getcallee_221647(n);
	nimln(130, "syntaxes.nim");
	f = getfilter_221239(ident);
	nimln(131, "syntaxes.nim");
	switch (f) {
	case ((NU8) 0):
	{
		nimln(133, "syntaxes.nim");
		(*p).skin = getparser_221447(ident);
		nimln(134, "syntaxes.nim");
		result = stdin_221695;
	}
	break;
	case ((NU8) 1):
	{
		nimln(136, "syntaxes.nim");
		result = filtertmpl_220009(stdin_221695, filename, n);
	}
	break;
	case ((NU8) 3):
	{
		nimln(138, "syntaxes.nim");
		result = filterstrip_219012(stdin_221695, filename, n);
	}
	break;
	case ((NU8) 2):
	{
		nimln(140, "syntaxes.nim");
		result = filterreplace_219004(stdin_221695, filename, n);
	}
	break;
	}
	nimln(141, "syntaxes.nim");
	{
		if (!!((f == ((NU8) 0)))) goto LA7;
		nimln(142, "syntaxes.nim");
		{
			TY146637 LOC13;
			TY146637 LOC14;
			if (!(((NI) 2) <= gverbosity_156137)) goto LA11;
			nimln(143, "syntaxes.nim");
			memset((void*)LOC13, 0, sizeof(LOC13));
			rawmessage_166888(((NU16) 270), LOC13, 0);
			nimln(144, "syntaxes.nim");
			msgwriteln_166536((*result).s);
			nimln(145, "syntaxes.nim");
			memset((void*)LOC14, 0, sizeof(LOC14));
			rawmessage_166888(((NU16) 271), LOC14, 0);
		}
		LA11: ;
	}
	LA7: ;
	popFrame();
	return result;
}

N_NIMCALL(Tllstream180204*, evalpipe_221722)(Tparsers221035* p, Tnode193813* n, NimStringDesc* filename, Tllstream180204* start) {
	Tllstream180204* result;
	nimfr("evalPipe", "syntaxes.nim")
{	result = 0;
	nimln(149, "syntaxes.nim");
	result = start;
	nimln(150, "syntaxes.nim");
	{
		if (!((*n).kind == ((NU8) 1))) goto LA3;
		goto BeforeRet;
	}
	LA3: ;
	nimln(151, "syntaxes.nim");
	{
		NIM_BOOL LOC7;
		NIM_BOOL LOC8;
		LOC7 = 0;
		LOC8 = 0;
		LOC8 = ((*n).kind == ((NU8) 29));
		if (!(LOC8)) goto LA9;
		if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		LOC8 = ((*(*n).kindU.S6.sons->data[((NI) 0)]).kind == ((NU8) 2));
		LA9: ;
		LOC7 = LOC8;
		if (!(LOC7)) goto LA10;
		nimln(152, "syntaxes.nim");
		if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		if (!(((TMP3406[(*(*n).kindU.S6.sons->data[((NI) 0)]).kind/8] &(1<<((*(*n).kindU.S6.sons->data[((NI) 0)]).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP3407));
		LOC7 = identeq_169484((*(*n).kindU.S6.sons->data[((NI) 0)]).kindU.S5.ident, ((NimStringDesc*) &TMP3408));
		LA10: ;
		if (!LOC7) goto LA11;
		{
			NI i_221772;
			NI res_221798;
			i_221772 = 0;
			nimln(1598, "system.nim");
			res_221798 = ((NI) 1);
			{
				nimln(1599, "system.nim");
				while (1) {
					NI TMP3409;
					if (!(res_221798 <= ((NI) 2))) goto LA15;
					nimln(1600, "system.nim");
					i_221772 = res_221798;
					nimln(154, "syntaxes.nim");
					{
						if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
						if ((NU)(i_221772) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
						if (!((*(*n).kindU.S6.sons->data[i_221772]).kind == ((NU8) 29))) goto LA18;
						nimln(155, "syntaxes.nim");
						if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
						if ((NU)(i_221772) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
						result = evalpipe_221722(p, (*n).kindU.S6.sons->data[i_221772], filename, result);
					}
					goto LA16;
					LA18: ;
					{
						nimln(157, "syntaxes.nim");
						if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
						if ((NU)(i_221772) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
						result = applyfilter_221689(p, (*n).kindU.S6.sons->data[i_221772], filename, result);
					}
					LA16: ;
					nimln(1614, "system.nim");
					TMP3409 = addInt(res_221798, ((NI) 1));
					res_221798 = (NI)(TMP3409);
				} LA15: ;
			}
		}
	}
	goto LA5;
	LA11: ;
	{
		nimln(158, "syntaxes.nim");
		if (!((*n).kind == ((NU8) 115))) goto LA22;
		nimln(159, "syntaxes.nim");
		if (((TMP3404[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP3405));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		result = evalpipe_221722(p, (*n).kindU.S6.sons->data[((NI) 0)], filename, result);
	}
	goto LA5;
	LA22: ;
	{
		nimln(161, "syntaxes.nim");
		result = applyfilter_221689(p, n, filename, result);
	}
	LA5: ;
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(void, openparsers_221047)(Tparsers221035* p, NI32 fileidx, Tllstream180204* inputstream) {
	Tllstream180204* s;
	NimStringDesc* filename;
	Tnode193813* pipe;
	nimfr("openParsers", "syntaxes.nim")
	s = 0;
	nimln(165, "syntaxes.nim");
	(*p).skin = ((NU8) 0);
	nimln(166, "syntaxes.nim");
	filename = tofullpathconsiderdirty_166407(fileidx);
	nimln(167, "syntaxes.nim");
	pipe = parsepipe_221170(filename, inputstream);
	nimln(168, "syntaxes.nim");
	{
		if (!!((pipe == NIM_NIL))) goto LA3;
		s = evalpipe_221722(p, pipe, filename, inputstream);
	}
	goto LA1;
	LA3: ;
	{
		nimln(169, "syntaxes.nim");
		s = inputstream;
	}
	LA1: ;
	nimln(170, "syntaxes.nim");
	switch ((*p).skin) {
	case ((NU8) 0):
	case ((NU8) 2):
	case ((NU8) 3):
	{
		nimln(172, "syntaxes.nim");
		openparser_209402((&(*p).parser), fileidx, s, NIM_FALSE);
	}
	break;
	case ((NU8) 1):
	{
		nimln(174, "syntaxes.nim");
		openparser_209402((&(*p).parser), fileidx, s, NIM_TRUE);
	}
	break;
	}
	popFrame();
}

N_NIMCALL(void, closeparsers_221056)(Tparsers221035* p) {
	nimfr("closeParsers", "syntaxes.nim")
	nimln(177, "syntaxes.nim");
	closeparser_209223((&(*p).parser));
	popFrame();
}
NIM_EXTERNC N_NOINLINE(void, compiler_syntaxesInit)(void) {
	nimfr("syntaxes", "syntaxes.nim")
	popFrame();
}

NIM_EXTERNC N_NOINLINE(void, compiler_syntaxesDatInit)(void) {
static TNimNode* TMP3391[2];
static TNimNode* TMP3392[4];
NI TMP3394;
static char* NIM_CONST TMP3393[4] = {
"skinStandard", 
"skinStrongSpaces", 
"skinBraces", 
"skinEndX"};
static TNimNode TMP2912[8];
NTI221035.size = sizeof(Tparsers221035);
NTI221035.kind = 18;
NTI221035.base = 0;
TMP3391[0] = &TMP2912[1];
NTI221007.size = sizeof(NU8);
NTI221007.kind = 14;
NTI221007.base = 0;
NTI221007.flags = 3;
for (TMP3394 = 0; TMP3394 < 4; TMP3394++) {
TMP2912[TMP3394+2].kind = 1;
TMP2912[TMP3394+2].offset = TMP3394;
TMP2912[TMP3394+2].name = TMP3393[TMP3394];
TMP3392[TMP3394] = &TMP2912[TMP3394+2];
}
TMP2912[6].len = 4; TMP2912[6].kind = 2; TMP2912[6].sons = &TMP3392[0];
NTI221007.node = &TMP2912[6];
TMP2912[1].kind = 1;
TMP2912[1].offset = offsetof(Tparsers221035, skin);
TMP2912[1].typ = (&NTI221007);
TMP2912[1].name = "skin";
TMP3391[1] = &TMP2912[7];
TMP2912[7].kind = 1;
TMP2912[7].offset = offsetof(Tparsers221035, parser);
TMP2912[7].typ = (&NTI209205);
TMP2912[7].name = "parser";
TMP2912[0].len = 2; TMP2912[0].kind = 2; TMP2912[0].sons = &TMP3391[0];
NTI221035.node = &TMP2912[0];
}

