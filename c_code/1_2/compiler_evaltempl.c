/* Generated by Nim Compiler v0.11.2 */
/*   (c) 2015 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 64
#include "nimbase.h"

#include <string.h>
typedef struct Tnode193813 Tnode193813;
typedef struct Templctx386019 Templctx386019;
typedef struct Tsym193843 Tsym193843;
typedef struct Tidtable193861 Tidtable193861;
typedef struct Tidpairseq193859 Tidpairseq193859;
typedef struct Ttype193849 Ttype193849;
typedef struct Tlineinfo165338 Tlineinfo165338;
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct Tident169021 Tident169021;
typedef struct Tnodeseq193807 Tnodeseq193807;
typedef struct Tidobj169015 Tidobj169015;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct Ttypeseq193845 Ttypeseq193845;
typedef struct Tscope193837 Tscope193837;
typedef struct TY193944 TY193944;
typedef struct Tstrtable193817 Tstrtable193817;
typedef struct Tsymseq193815 Tsymseq193815;
typedef struct Tloc193827 Tloc193827;
typedef struct Ropeobj163009 Ropeobj163009;
typedef struct Tlib193831 Tlib193831;
typedef struct TY149210 TY149210;
typedef struct Tcell46746 Tcell46746;
typedef struct Tcellseq46762 Tcellseq46762;
typedef struct Tgcheap48616 Tgcheap48616;
typedef struct Tcellset46758 Tcellset46758;
typedef struct Tpagedesc46754 Tpagedesc46754;
typedef struct Tmemregion28610 Tmemregion28610;
typedef struct Tsmallchunk27840 Tsmallchunk27840;
typedef struct Tllchunk28604 Tllchunk28604;
typedef struct Tbigchunk27842 Tbigchunk27842;
typedef struct Tintset27817 Tintset27817;
typedef struct Ttrunk27813 Ttrunk27813;
typedef struct Tavlnode28608 Tavlnode28608;
typedef struct Tgcstat48614 Tgcstat48614;
typedef struct Tidpair193857 Tidpair193857;
typedef struct Tinstantiation193833 Tinstantiation193833;
typedef struct Tlistentry133022 Tlistentry133022;
typedef struct Tbasechunk27838 Tbasechunk27838;
typedef struct Tfreecell27830 Tfreecell27830;
typedef struct TY193933 TY193933;
struct  Tidtable193861  {
NI counter;
Tidpairseq193859* data;
};
struct  Templctx386019  {
Tsym193843* owner;
Tsym193843* gensymowner;
NIM_BOOL instlines;
Tidtable193861 mapping;
};
struct  Tlineinfo165338  {
NI16 line;
NI16 col;
NI32 fileindex;
};
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
struct  Tnode193813  {
Ttype193849* typ;
Tlineinfo165338 info;
NU16 flags;
NU8 kind;
union{
struct {NI64 intval;
} S1;
struct {NF floatval;
} S2;
struct {NimStringDesc* strval;
} S3;
struct {Tsym193843* sym;
} S4;
struct {Tident169021* ident;
} S5;
struct {Tnodeseq193807* sons;
} S6;
} kindU;
NimStringDesc* comment;
};
typedef NU8 TY194031[20];
typedef N_NIMCALL_PTR(void, TY3289) (void* p, NI op);
typedef N_NIMCALL_PTR(void*, TY3294) (void* p);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY3289 marker;
TY3294 deepcopy;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  Tidobj169015  {
  TNimObject Sup;
NI id;
};
struct  Tstrtable193817  {
NI counter;
Tsymseq193815* data;
};
struct  Tloc193827  {
NU8 k;
NU8 s;
NU16 flags;
Ttype193849* t;
Ropeobj163009* r;
Ropeobj163009* heaproot;
};
struct  Tsym193843  {
  Tidobj169015 Sup;
NU8 kind;
union{
struct {Ttypeseq193845* typeinstcache;
Tscope193837* typscope;
} S1;
struct {TY193944* procinstcache;
Tsym193843* gcunsafetyreason;
} S2;
struct {TY193944* usedgenerics;
Tstrtable193817 tab;
} S3;
struct {Tsym193843* guard;
} S4;
} kindU;
NU16 magic;
Ttype193849* typ;
Tident169021* name;
Tlineinfo165338 info;
Tsym193843* owner;
NU32 flags;
Tnode193813* ast;
NU32 options;
NI position;
NI offset;
Tloc193827 loc;
Tlib193831* annex;
Tnode193813* constraint;
};
struct TY149210 {
NimStringDesc* Field0;
NI Field1;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct  Tcell46746  {
NI refcount;
TNimType* typ;
};
struct  Tcellseq46762  {
NI len;
NI cap;
Tcell46746** d;
};
struct  Tcellset46758  {
NI counter;
NI max;
Tpagedesc46754* head;
Tpagedesc46754** data;
};
typedef Tsmallchunk27840* TY28622[512];
typedef Ttrunk27813* Ttrunkbuckets27815[256];
struct  Tintset27817  {
Ttrunkbuckets27815 data;
};
struct  Tmemregion28610  {
NI minlargeobj;
NI maxlargeobj;
TY28622 freesmallchunks;
Tllchunk28604* llmem;
NI currmem;
NI maxmem;
NI freemem;
NI lastsize;
Tbigchunk27842* freechunkslist;
Tintset27817 chunkstarts;
Tavlnode28608* root;
Tavlnode28608* deleted;
Tavlnode28608* last;
Tavlnode28608* freeavlnodes;
};
struct  Tgcstat48614  {
NI stackscans;
NI cyclecollections;
NI maxthreshold;
NI maxstacksize;
NI maxstackcells;
NI cycletablesize;
NI64 maxpause;
};
struct  Tgcheap48616  {
void* stackbottom;
NI cyclethreshold;
Tcellseq46762 zct;
Tcellseq46762 decstack;
Tcellset46758 cycleroots;
Tcellseq46762 tempstack;
NI recgclock;
Tmemregion28610 region;
Tgcstat48614 stat;
};
struct  Ttype193849  {
  Tidobj169015 Sup;
NU8 kind;
NU8 callconv;
NU32 flags;
Ttypeseq193845* sons;
Tnode193813* n;
Tsym193843* owner;
Tsym193843* sym;
Tsym193843* destructor;
Tsym193843* deepcopy;
Tsym193843* assignment;
NI64 size;
NI16 align;
NI16 locklevel;
Tloc193827 loc;
};
struct  Tidpair193857  {
Tidobj169015* key;
TNimObject* val;
};
struct  Tident169021  {
  Tidobj169015 Sup;
NimStringDesc* s;
Tident169021* next;
NI h;
};
struct  Tscope193837  {
NI depthlevel;
Tstrtable193817 symbols;
Tnodeseq193807* usingsyms;
Tscope193837* parent;
};
struct  Ropeobj163009  {
  TNimObject Sup;
Ropeobj163009* left;
Ropeobj163009* right;
NI length;
NimStringDesc* data;
};
struct  Tlistentry133022  {
  TNimObject Sup;
Tlistentry133022* prev;
Tlistentry133022* next;
};
struct  Tlib193831  {
  Tlistentry133022 Sup;
NU8 kind;
NIM_BOOL generated;
NIM_BOOL isoverriden;
Ropeobj163009* name;
Tnode193813* path;
};
typedef NI TY27820[8];
struct  Tpagedesc46754  {
Tpagedesc46754* next;
NI key;
TY27820 bits;
};
struct  Tbasechunk27838  {
NI prevsize;
NI size;
NIM_BOOL used;
};
struct  Tsmallchunk27840  {
  Tbasechunk27838 Sup;
Tsmallchunk27840* next;
Tsmallchunk27840* prev;
Tfreecell27830* freelist;
NI free;
NI acc;
NF data;
};
struct  Tllchunk28604  {
NI size;
NI acc;
Tllchunk28604* next;
};
struct  Tbigchunk27842  {
  Tbasechunk27838 Sup;
Tbigchunk27842* next;
Tbigchunk27842* prev;
NI align;
NF data;
};
struct  Ttrunk27813  {
Ttrunk27813* next;
NI key;
TY27820 bits;
};
typedef Tavlnode28608* TY28614[2];
struct  Tavlnode28608  {
TY28614 link;
NI key;
NI upperbound;
NI level;
};
struct  Tinstantiation193833  {
Tsym193843* sym;
Ttypeseq193845* concretetypes;
TY193933* usedby;
};
struct  Tfreecell27830  {
Tfreecell27830* next;
NI zerofield;
};
struct Tidpairseq193859 {
  TGenericSeq Sup;
  Tidpair193857 data[SEQ_DECL_SIZE];
};
struct Tnodeseq193807 {
  TGenericSeq Sup;
  Tnode193813* data[SEQ_DECL_SIZE];
};
struct Ttypeseq193845 {
  TGenericSeq Sup;
  Ttype193849* data[SEQ_DECL_SIZE];
};
struct TY193944 {
  TGenericSeq Sup;
  Tinstantiation193833* data[SEQ_DECL_SIZE];
};
struct Tsymseq193815 {
  TGenericSeq Sup;
  Tsym193843* data[SEQ_DECL_SIZE];
};
struct TY193933 {
  TGenericSeq Sup;
  NI32 data[SEQ_DECL_SIZE];
};
N_NIMCALL(Tnode193813*, copynode_386027)(Templctx386019* ctx, Tnode193813* a, Tnode193813* b);
N_NIMCALL(Tnode193813*, copynode_197640)(Tnode193813* src);
static N_INLINE(void, nimFrame)(TFrame* s);
N_NOINLINE(void, stackoverflow_20001)(void);
static N_INLINE(void, popFrame)(void);
N_NIMCALL(void, evaltemplateaux_386039)(Tnode193813* templ, Tnode193813* actual, Templctx386019* c, Tnode193813* result);
N_NOINLINE(void, raiseFieldError)(NimStringDesc* f);
N_NOINLINE(void, raiseIndexError)(void);
static N_INLINE(NI, len_194097)(Tnode193813* n);
static N_INLINE(NI, subInt)(NI a, NI b);
N_NOINLINE(void, raiseOverflow)(void);
N_NIMCALL(void, add_194164)(Tnode193813* father, Tnode193813* son);
static N_INLINE(NI, addInt)(NI a, NI b);
N_NIMCALL(Tnode193813*, copytree_198028)(Tnode193813* src);
N_NIMCALL(void, internalerror_167218)(NimStringDesc* errmsg);
N_NIMCALL(NimStringDesc*, HEX24_167306)(TY149210 x);
N_NIMCALL(TNimObject*, idtableget_201182)(Tidtable193861 t, Tidobj169015* key);
N_NIMCALL(void, chckObj)(TNimType* obj, TNimType* subclass);
N_NIMCALL(Tsym193843*, copysym_196607)(Tsym193843* s, NIM_BOOL keepid);
static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src);
static N_INLINE(Tcell46746*, usrtocell_50246)(void* usr);
static N_INLINE(void, rtladdzct_51804)(Tcell46746* c);
N_NOINLINE(void, addzct_50217)(Tcellseq46762* s, Tcell46746* c);
N_NIMCALL(void, idtableput_201196)(Tidtable193861* t, Tidobj169015* key, TNimObject* val);
N_NIMCALL(Tnode193813*, newsymnode_195338)(Tsym193843* sym, Tlineinfo165338 info);
N_NIMCALL(NI, sonslen_196403)(Tnode193813* n);
N_NIMCALL(Tnode193813*, evaltemplateargs_386184)(Tnode193813* n, Tsym193843* s);
N_NIMCALL(NI, sonslen_196351)(Ttype193849* n);
N_NIMCALL(void, globalerror_167134)(Tlineinfo165338 info, NU16 msg, NimStringDesc* arg);
N_NIMCALL(Tnode193813*, newnodei_195351)(NU8 kind, Tlineinfo165338 info);
N_NIMCALL(void, localerror_167155)(Tlineinfo165338 info, NU16 msg, NimStringDesc* arg);
N_NIMCALL(void, addson_195819)(Tnode193813* father, Tnode193813* son);
N_NIMCALL(Tnode193813*, evaltemplate_386260)(Tnode193813* n, Tsym193843* tmpl, Tsym193843* gensymowner);
N_NIMCALL(void, initidtable_197057)(Tidtable193861* x);
N_NIMCALL(Tnode193813*, getbody_238647)(Tsym193843* s);
static N_INLINE(NIM_BOOL, isatom_198906)(Tnode193813* n);
N_NIMCALL(NimStringDesc*, rendertree_215058)(Tnode193813* n, NU8 renderflags);
static N_INLINE(NI, safelen_194125)(Tnode193813* n);
static NIM_CONST TY194031 TMP7601 = {
0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP7602, "sym", 3);
static NIM_CONST TY194031 TMP7603 = {
0xEC, 0xFF, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP7604, "sons", 4);
STRING_LITERAL(TMP7607, "compiler/evaltempl.nim", 22);
NIM_CONST TY149210 TMP7606 = {((NimStringDesc*) &TMP7607),
((NI) 39)}
;
STRING_LITERAL(TMP7610, "", 0);
extern TFrame* frameptr_17042;
extern TNimType NTI193843; /* TSym */
extern Tgcheap48616 gch_48644;
NI evaltemplatecounter_386256;

static N_INLINE(void, nimFrame)(TFrame* s) {
	NI LOC1;
	LOC1 = 0;
	{
		if (!(frameptr_17042 == NIM_NIL)) goto LA4;
		LOC1 = ((NI) 0);
	}
	goto LA2;
	LA4: ;
	{
		LOC1 = ((NI) ((NI16)((*frameptr_17042).calldepth + ((NI16) 1))));
	}
	LA2: ;
	(*s).calldepth = ((NI16) (LOC1));
	(*s).prev = frameptr_17042;
	frameptr_17042 = s;
	{
		if (!((*s).calldepth == ((NI16) 2000))) goto LA9;
		stackoverflow_20001();
	}
	LA9: ;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_17042 = (*frameptr_17042).prev;
}

N_NIMCALL(Tnode193813*, copynode_386027)(Templctx386019* ctx, Tnode193813* a, Tnode193813* b) {
	Tnode193813* result;
	nimfr("copyNode", "evaltempl.nim")
	result = 0;
	nimln(24, "evaltempl.nim");
	result = copynode_197640(a);
	nimln(25, "evaltempl.nim");
	{
		if (!(*ctx).instlines) goto LA3;
		(*result).info = (*b).info;
	}
	LA3: ;
	popFrame();
	return result;
}

static N_INLINE(NI, len_194097)(Tnode193813* n) {
	NI result;
	nimfr("len", "ast.nim")
	result = 0;
	nimln(934, "ast.nim");
	{
		if (((TMP7603[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
		if (!(*n).kindU.S6.sons == 0) goto LA3;
		result = ((NI) 0);
	}
	goto LA1;
	LA3: ;
	{
		nimln(935, "ast.nim");
		if (((TMP7603[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
		result = ((*n).kindU.S6.sons ? (*n).kindU.S6.sons->Sup.len : 0);
	}
	LA1: ;
	popFrame();
	return result;
}

static N_INLINE(NI, subInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU64)(a) - (NU64)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ (NI)((NU64) ~(b))));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

static N_INLINE(NI, addInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU64)(a) + (NU64)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ b));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

static N_INLINE(Tcell46746*, usrtocell_50246)(void* usr) {
	Tcell46746* result;
	nimfr("usrToCell", "gc.nim")
	result = 0;
	nimln(118, "gc.nim");
	result = ((Tcell46746*) ((NI)((NU64)(((NI) (usr))) - (NU64)(((NI)sizeof(Tcell46746))))));
	popFrame();
	return result;
}

static N_INLINE(void, rtladdzct_51804)(Tcell46746* c) {
	nimfr("rtlAddZCT", "gc.nim")
	nimln(199, "gc.nim");
	addzct_50217((&gch_48644.zct), c);
	popFrame();
}

static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src) {
	nimfr("asgnRefNoCycle", "gc.nim")
	nimln(251, "gc.nim");
	{
		Tcell46746* c;
		if (!!((src == NIM_NIL))) goto LA3;
		nimln(252, "gc.nim");
		c = usrtocell_50246(src);
		nimln(169, "gc.nim");
		(*c).refcount += ((NI) 8);
	}
	LA3: ;
	nimln(254, "gc.nim");
	{
		Tcell46746* c;
		if (!!(((*dest) == NIM_NIL))) goto LA7;
		nimln(255, "gc.nim");
		c = usrtocell_50246((*dest));
		nimln(256, "gc.nim");
		{
			nimln(167, "gc.nim");
			(*c).refcount -= ((NI) 8);
			nimln(168, "gc.nim");
			if (!((NU64)((*c).refcount) < (NU64)(((NI) 8)))) goto LA11;
			nimln(257, "gc.nim");
			rtladdzct_51804(c);
		}
		LA11: ;
	}
	LA7: ;
	nimln(258, "gc.nim");
	(*dest) = src;
	popFrame();
}

N_NIMCALL(void, evaltemplateaux_386039)(Tnode193813* templ, Tnode193813* actual, Templctx386019* c, Tnode193813* result) {
	nimfr("evalTemplateAux", "evaltempl.nim")
	nimln(28, "evaltempl.nim");
	switch ((*templ).kind) {
	case ((NU8) 3):
	{
		Tsym193843* s;
		nimln(30, "evaltempl.nim");
		if (!(((TMP7601[(*templ).kind/8] &(1<<((*templ).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP7602));
		s = (*templ).kindU.S4.sym;
		nimln(31, "evaltempl.nim");
		{
			if (!((*(*s).owner).Sup.id == (*(*c).owner).Sup.id)) goto LA4;
			nimln(32, "evaltempl.nim");
			{
				NIM_BOOL LOC8;
				Tnode193813* x;
				LOC8 = 0;
				LOC8 = ((*s).kind == ((NU8) 3));
				if (!(LOC8)) goto LA9;
				LOC8 = !((((*s).flags &(1<<((((NU8) 31))&31)))!=0));
				LA9: ;
				if (!LOC8) goto LA10;
				nimln(33, "evaltempl.nim");
				if (((TMP7603[(*actual).kind/8] &(1<<((*actual).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
				if ((NU)((*s).position) >= (NU)((*actual).kindU.S6.sons->Sup.len)) raiseIndexError();
				x = (*actual).kindU.S6.sons->data[(*s).position];
				nimln(34, "evaltempl.nim");
				{
					if (!((*x).kind == ((NU8) 152))) goto LA14;
					{
						Tnode193813* y_386083;
						y_386083 = 0;
						{
							NI i_386142;
							NI HEX3Atmp_386144;
							NI LOC18;
							NI res_386146;
							i_386142 = 0;
							HEX3Atmp_386144 = 0;
							nimln(1537, "ast.nim");
							LOC18 = 0;
							LOC18 = len_194097(x);
							HEX3Atmp_386144 = subInt(LOC18, 1);
							nimln(1598, "system.nim");
							res_386146 = ((NI) 0);
							{
								nimln(1599, "system.nim");
								while (1) {
									NI TMP7605;
									if (!(res_386146 <= HEX3Atmp_386144)) goto LA20;
									nimln(1600, "system.nim");
									i_386142 = res_386146;
									nimln(1537, "ast.nim");
									if (((TMP7603[(*x).kind/8] &(1<<((*x).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
									if ((NU)(i_386142) >= (NU)((*x).kindU.S6.sons->Sup.len)) raiseIndexError();
									y_386083 = (*x).kindU.S6.sons->data[i_386142];
									nimln(35, "evaltempl.nim");
									add_194164(result, y_386083);
									nimln(1619, "system.nim");
									TMP7605 = addInt(res_386146, ((NI) 1));
									res_386146 = (NI)(TMP7605);
								} LA20: ;
							}
						}
					}
				}
				goto LA12;
				LA14: ;
				{
					Tnode193813* LOC22;
					nimln(37, "evaltempl.nim");
					LOC22 = 0;
					LOC22 = copytree_198028(x);
					add_194164(result, LOC22);
				}
				LA12: ;
			}
			goto LA6;
			LA10: ;
			{
				Tsym193843* x;
				Tidobj169015* LOC29;
				TNimObject* LOC30;
				Tlineinfo165338 LOC37;
				Tnode193813* LOC43;
				nimln(854, "msgs.nim");
				{
					NimStringDesc* LOC28;
					nimln(39, "evaltempl.nim");
					if (!!((((*s).flags &(1<<((((NU8) 31))&31)))!=0))) goto LA26;
					nimln(854, "msgs.nim");
					LOC28 = 0;
					LOC28 = HEX24_167306(TMP7606);
					internalerror_167218(LOC28);
				}
				LA26: ;
				nimln(40, "evaltempl.nim");
				LOC29 = 0;
				LOC29 = &s->Sup;
				LOC30 = 0;
				LOC30 = idtableget_201182((*c).mapping, LOC29);
				if (LOC30) chckObj((*LOC30).m_type, (&NTI193843));
				x = ((Tsym193843*) (LOC30));
				nimln(41, "evaltempl.nim");
				{
					Tidobj169015* LOC35;
					TNimObject* LOC36;
					if (!(x == NIM_NIL)) goto LA33;
					nimln(42, "evaltempl.nim");
					x = copysym_196607(s, NIM_FALSE);
					nimln(43, "evaltempl.nim");
					asgnRefNoCycle((void**) (&(*x).owner), (*c).gensymowner);
					nimln(44, "evaltempl.nim");
					LOC35 = 0;
					LOC35 = &s->Sup;
					LOC36 = 0;
					LOC36 = &x->Sup.Sup;
					idtableput_201196((&(*c).mapping), LOC35, LOC36);
				}
				LA33: ;
				nimln(45, "evaltempl.nim");
				{
					if (!(*c).instlines) goto LA40;
					LOC37 = (*actual).info;
				}
				goto LA38;
				LA40: ;
				{
					LOC37 = (*templ).info;
				}
				LA38: ;
				LOC43 = 0;
				LOC43 = newsymnode_195338(x, LOC37);
				add_194164(result, LOC43);
			}
			LA6: ;
		}
		goto LA2;
		LA4: ;
		{
			Tnode193813* LOC45;
			nimln(47, "evaltempl.nim");
			LOC45 = 0;
			LOC45 = copynode_386027((&(*c)), templ, actual);
			add_194164(result, LOC45);
		}
		LA2: ;
	}
	break;
	case ((NU8) 0) ... ((NU8) 2):
	case ((NU8) 4) ... ((NU8) 23):
	{
		Tnode193813* LOC47;
		nimln(49, "evaltempl.nim");
		LOC47 = 0;
		LOC47 = copynode_386027((&(*c)), templ, actual);
		add_194164(result, LOC47);
	}
	break;
	default:
	{
		Tnode193813* res;
		nimln(51, "evaltempl.nim");
		res = copynode_386027((&(*c)), templ, actual);
		{
			NI i_386135;
			NI HEX3Atmp_386150;
			NI LOC50;
			NI TMP7608;
			NI res_386153;
			i_386135 = 0;
			HEX3Atmp_386150 = 0;
			nimln(52, "evaltempl.nim");
			LOC50 = 0;
			LOC50 = sonslen_196403(templ);
			TMP7608 = subInt(LOC50, ((NI) 1));
			HEX3Atmp_386150 = (NI)(TMP7608);
			nimln(1598, "system.nim");
			res_386153 = ((NI) 0);
			{
				nimln(1599, "system.nim");
				while (1) {
					NI TMP7609;
					if (!(res_386153 <= HEX3Atmp_386150)) goto LA52;
					nimln(1600, "system.nim");
					i_386135 = res_386153;
					nimln(53, "evaltempl.nim");
					if (((TMP7603[(*templ).kind/8] &(1<<((*templ).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
					if ((NU)(i_386135) >= (NU)((*templ).kindU.S6.sons->Sup.len)) raiseIndexError();
					evaltemplateaux_386039((*templ).kindU.S6.sons->data[i_386135], actual, c, res);
					nimln(1614, "system.nim");
					TMP7609 = addInt(res_386153, ((NI) 1));
					res_386153 = (NI)(TMP7609);
				} LA52: ;
			}
		}
		nimln(54, "evaltempl.nim");
		add_194164(result, res);
	}
	break;
	}
	popFrame();
}

N_NIMCALL(Tnode193813*, evaltemplateargs_386184)(Tnode193813* n, Tsym193843* s) {
	Tnode193813* result;
	NI a;
	NI f;
	nimfr("evalTemplateArgs", "evaltempl.nim")
	result = 0;
	a = 0;
	nimln(60, "evaltempl.nim");
	switch ((*n).kind) {
	case ((NU8) 27):
	case ((NU8) 29):
	case ((NU8) 30):
	case ((NU8) 31):
	case ((NU8) 26):
	case ((NU8) 28):
	{
		nimln(62, "evaltempl.nim");
		a = sonslen_196403(n);
	}
	break;
	default:
	{
		nimln(63, "evaltempl.nim");
		a = ((NI) 0);
	}
	break;
	}
	nimln(64, "evaltempl.nim");
	f = sonslen_196351((*s).typ);
	nimln(65, "evaltempl.nim");
	{
		if (!(f < a)) goto LA5;
		globalerror_167134((*n).info, ((NU16) 133), ((NimStringDesc*) &TMP7610));
	}
	LA5: ;
	nimln(67, "evaltempl.nim");
	result = newnodei_195351(((NU8) 152), (*n).info);
	{
		NI i_386204;
		NI HEX3Atmp_386234;
		NI TMP7611;
		NI res_386237;
		i_386204 = 0;
		HEX3Atmp_386234 = 0;
		nimln(68, "evaltempl.nim");
		TMP7611 = subInt(f, ((NI) 1));
		HEX3Atmp_386234 = (NI)(TMP7611);
		nimln(1598, "system.nim");
		res_386237 = ((NI) 1);
		{
			nimln(1599, "system.nim");
			while (1) {
				Tnode193813* arg;
				NI TMP7612;
				if (!(res_386237 <= HEX3Atmp_386234)) goto LA9;
				nimln(1600, "system.nim");
				i_386204 = res_386237;
				nimln(69, "evaltempl.nim");
				{
					if (!(i_386204 < a)) goto LA12;
					if (((TMP7603[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
					if ((NU)(i_386204) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
					arg = (*n).kindU.S6.sons->data[i_386204];
				}
				goto LA10;
				LA12: ;
				{
					if (((TMP7603[(*(*(*s).typ).n).kind/8] &(1<<((*(*(*s).typ).n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
					if ((NU)(i_386204) >= (NU)((*(*(*s).typ).n).kindU.S6.sons->Sup.len)) raiseIndexError();
					if (!(((TMP7601[(*(*(*(*s).typ).n).kindU.S6.sons->data[i_386204]).kind/8] &(1<<((*(*(*(*s).typ).n).kindU.S6.sons->data[i_386204]).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP7602));
					arg = copytree_198028((*(*(*(*(*s).typ).n).kindU.S6.sons->data[i_386204]).kindU.S4.sym).ast);
				}
				LA10: ;
				nimln(70, "evaltempl.nim");
				{
					NIM_BOOL LOC17;
					LOC17 = 0;
					LOC17 = (arg == NIM_NIL);
					if (LOC17) goto LA18;
					LOC17 = ((*arg).kind == ((NU8) 1));
					LA18: ;
					if (!LOC17) goto LA19;
					nimln(71, "evaltempl.nim");
					localerror_167155((*n).info, ((NU16) 133), ((NimStringDesc*) &TMP7610));
				}
				LA19: ;
				nimln(72, "evaltempl.nim");
				addson_195819(result, arg);
				nimln(1614, "system.nim");
				TMP7612 = addInt(res_386237, ((NI) 1));
				res_386237 = (NI)(TMP7612);
			} LA9: ;
		}
	}
	popFrame();
	return result;
}

static N_INLINE(NIM_BOOL, isatom_198906)(Tnode193813* n) {
	NIM_BOOL result;
	NIM_BOOL LOC1;
	nimfr("isAtom", "ast.nim")
	result = 0;
	nimln(1543, "ast.nim");
	LOC1 = 0;
	LOC1 = (((NU8) 0) <= (*n).kind);
	if (!(LOC1)) goto LA2;
	LOC1 = ((*n).kind <= ((NU8) 23));
	LA2: ;
	result = LOC1;
	popFrame();
	return result;
}

static N_INLINE(NI, safelen_194125)(Tnode193813* n) {
	NI result;
	nimfr("safeLen", "ast.nim")
	result = 0;
	nimln(939, "ast.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = ((*n).kind >= ((NU8) 0) && (*n).kind <= ((NU8) 23));
		if (LOC3) goto LA4;
		if (((TMP7603[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
		LOC3 = (*n).kindU.S6.sons == 0;
		LA4: ;
		if (!LOC3) goto LA5;
		result = ((NI) 0);
	}
	goto LA1;
	LA5: ;
	{
		nimln(940, "ast.nim");
		if (((TMP7603[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
		result = ((*n).kindU.S6.sons ? (*n).kindU.S6.sons->Sup.len : 0);
	}
	LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(Tnode193813*, evaltemplate_386260)(Tnode193813* n, Tsym193843* tmpl, Tsym193843* gensymowner) {
	Tnode193813* result;
	NI TMP7613;
	Tnode193813* args;
	Templctx386019 ctx;
	Tnode193813* body;
	NI TMP7616;
	nimfr("evalTemplate", "evaltempl.nim")
	result = 0;
	nimln(78, "evaltempl.nim");
	TMP7613 = addInt(evaltemplatecounter_386256, ((NI) 1));
	evaltemplatecounter_386256 = (NI)(TMP7613);
	nimln(79, "evaltempl.nim");
	{
		if (!(((NI) 100) < evaltemplatecounter_386256)) goto LA3;
		nimln(80, "evaltempl.nim");
		globalerror_167134((*n).info, ((NU16) 183), ((NimStringDesc*) &TMP7610));
		nimln(81, "evaltempl.nim");
		result = n;
	}
	LA3: ;
	nimln(84, "evaltempl.nim");
	args = evaltemplateargs_386184(n, tmpl);
	memset((void*)(&ctx), 0, sizeof(ctx));
	nimln(86, "evaltempl.nim");
	ctx.owner = tmpl;
	nimln(87, "evaltempl.nim");
	ctx.gensymowner = gensymowner;
	nimln(88, "evaltempl.nim");
	initidtable_197057((&ctx.mapping));
	nimln(90, "evaltempl.nim");
	body = getbody_238647(tmpl);
	nimln(91, "evaltempl.nim");
	{
		NIM_BOOL LOC7;
		LOC7 = 0;
		LOC7 = isatom_198906(body);
		if (!LOC7) goto LA8;
		nimln(92, "evaltempl.nim");
		result = newnodei_195351(((NU8) 37), (*body).info);
		nimln(93, "evaltempl.nim");
		evaltemplateaux_386039(body, args, (&ctx), result);
		nimln(94, "evaltempl.nim");
		{
			NI LOC12;
			LOC12 = 0;
			LOC12 = len_194097(result);
			if (!(LOC12 == ((NI) 1))) goto LA13;
			if (((TMP7603[(*result).kind/8] &(1<<((*result).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
			if ((NU)(((NI) 0)) >= (NU)((*result).kindU.S6.sons->Sup.len)) raiseIndexError();
			result = (*result).kindU.S6.sons->data[((NI) 0)];
		}
		goto LA10;
		LA13: ;
		{
			NimStringDesc* LOC16;
			nimln(96, "evaltempl.nim");
			nimln(97, "evaltempl.nim");
			LOC16 = 0;
			LOC16 = rendertree_215058(result, 4);
			localerror_167155((*result).info, ((NU16) 1), LOC16);
		}
		LA10: ;
	}
	goto LA5;
	LA8: ;
	{
		nimln(99, "evaltempl.nim");
		result = copynode_197640(body);
		nimln(100, "evaltempl.nim");
		ctx.instlines = !(((*body).kind == ((NU8) 115) || (*body).kind == ((NU8) 126) || (*body).kind == ((NU8) 112) || (*body).kind == ((NU8) 127)));
		nimln(102, "evaltempl.nim");
		{
			if (!ctx.instlines) goto LA20;
			(*result).info = (*n).info;
		}
		LA20: ;
		{
			NI i_386308;
			NI HEX3Atmp_386322;
			NI LOC23;
			NI TMP7614;
			NI res_386325;
			i_386308 = 0;
			HEX3Atmp_386322 = 0;
			nimln(103, "evaltempl.nim");
			LOC23 = 0;
			LOC23 = safelen_194125(body);
			TMP7614 = subInt(LOC23, ((NI) 1));
			HEX3Atmp_386322 = (NI)(TMP7614);
			nimln(1598, "system.nim");
			res_386325 = ((NI) 0);
			{
				nimln(1599, "system.nim");
				while (1) {
					NI TMP7615;
					if (!(res_386325 <= HEX3Atmp_386322)) goto LA25;
					nimln(1600, "system.nim");
					i_386308 = res_386325;
					nimln(104, "evaltempl.nim");
					if (((TMP7603[(*body).kind/8] &(1<<((*body).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP7604));
					if ((NU)(i_386308) >= (NU)((*body).kindU.S6.sons->Sup.len)) raiseIndexError();
					evaltemplateaux_386039((*body).kindU.S6.sons->data[i_386308], args, (&ctx), result);
					nimln(1614, "system.nim");
					TMP7615 = addInt(res_386325, ((NI) 1));
					res_386325 = (NI)(TMP7615);
				} LA25: ;
			}
		}
	}
	LA5: ;
	nimln(106, "evaltempl.nim");
	TMP7616 = subInt(evaltemplatecounter_386256, ((NI) 1));
	evaltemplatecounter_386256 = (NI)(TMP7616);
	popFrame();
	return result;
}
NIM_EXTERNC N_NOINLINE(void, compiler_evaltemplInit)(void) {
	nimfr("evaltempl", "evaltempl.nim")
	nimln(74, "evaltempl.nim");
	evaltemplatecounter_386256 = ((NI) 0);
	popFrame();
}

NIM_EXTERNC N_NOINLINE(void, compiler_evaltemplDatInit)(void) {
}

