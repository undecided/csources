/* Generated by Nim Compiler v0.11.2 */
/*   (c) 2015 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 64
#include "nimbase.h"

#include <string.h>

#include <stdio.h>
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct Ropeobj162009 Ropeobj162009;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct Tcell46747 Tcell46747;
typedef struct Tcellseq46763 Tcellseq46763;
typedef struct Tgcheap48616 Tgcheap48616;
typedef struct Tcellset46759 Tcellset46759;
typedef struct Tpagedesc46755 Tpagedesc46755;
typedef struct Tmemregion28610 Tmemregion28610;
typedef struct Tsmallchunk27840 Tsmallchunk27840;
typedef struct Tllchunk28604 Tllchunk28604;
typedef struct Tbigchunk27842 Tbigchunk27842;
typedef struct Tintset27817 Tintset27817;
typedef struct Ttrunk27813 Ttrunk27813;
typedef struct Tavlnode28608 Tavlnode28608;
typedef struct Tgcstat48614 Tgcstat48614;
typedef struct TY162599 TY162599;
typedef struct Tbasechunk27838 Tbasechunk27838;
typedef struct Tfreecell27830 Tfreecell27830;
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
typedef struct {
N_NIMCALL_PTR(void, ClPrc) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning, void* ClEnv);
void* ClEnv;
} TY162023;
typedef N_NIMCALL_PTR(void, TY3289) (void* p, NI op);
typedef N_NIMCALL_PTR(void*, TY3294) (void* p);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY3289 marker;
TY3294 deepcopy;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  Ropeobj162009  {
  TNimObject Sup;
Ropeobj162009* left;
Ropeobj162009* right;
NI length;
NimStringDesc* data;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct  Tcell46747  {
NI refcount;
TNimType* typ;
};
struct  Tcellseq46763  {
NI len;
NI cap;
Tcell46747** d;
};
struct  Tcellset46759  {
NI counter;
NI max;
Tpagedesc46755* head;
Tpagedesc46755** data;
};
typedef Tsmallchunk27840* TY28622[512];
typedef Ttrunk27813* Ttrunkbuckets27815[256];
struct  Tintset27817  {
Ttrunkbuckets27815 data;
};
struct  Tmemregion28610  {
NI minlargeobj;
NI maxlargeobj;
TY28622 freesmallchunks;
Tllchunk28604* llmem;
NI currmem;
NI maxmem;
NI freemem;
NI lastsize;
Tbigchunk27842* freechunkslist;
Tintset27817 chunkstarts;
Tavlnode28608* root;
Tavlnode28608* deleted;
Tavlnode28608* last;
Tavlnode28608* freeavlnodes;
};
struct  Tgcstat48614  {
NI stackscans;
NI cyclecollections;
NI maxthreshold;
NI maxstacksize;
NI maxstackcells;
NI cycletablesize;
NI64 maxpause;
};
struct  Tgcheap48616  {
void* stackbottom;
NI cyclethreshold;
Tcellseq46763 zct;
Tcellseq46763 decstack;
Tcellset46759 cycleroots;
Tcellseq46763 tempstack;
NI recgclock;
Tmemregion28610 region;
Tgcstat48614 stat;
};
typedef Ropeobj162009* TY162101[4096];
typedef Ropeobj162009* TY162572[1];
typedef N_CLOSURE_PTR(void, TMP835) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP847) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP855) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP857) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP861) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef NIM_CHAR TY163814[1024];
typedef NI TY27820[8];
struct  Tpagedesc46755  {
Tpagedesc46755* next;
NI key;
TY27820 bits;
};
struct  Tbasechunk27838  {
NI prevsize;
NI size;
NIM_BOOL used;
};
struct  Tsmallchunk27840  {
  Tbasechunk27838 Sup;
Tsmallchunk27840* next;
Tsmallchunk27840* prev;
Tfreecell27830* freelist;
NI free;
NI acc;
NF data;
};
struct  Tllchunk28604  {
NI size;
NI acc;
Tllchunk28604* next;
};
struct  Tbigchunk27842  {
  Tbasechunk27838 Sup;
Tbigchunk27842* next;
Tbigchunk27842* prev;
NI align;
NF data;
};
struct  Ttrunk27813  {
Ttrunk27813* next;
NI key;
TY27820 bits;
};
typedef Tavlnode28608* TY28614[2];
struct  Tavlnode28608  {
TY28614 link;
NI key;
NI upperbound;
NI level;
};
struct  Tfreecell27830  {
Tfreecell27830* next;
NI zerofield;
};
struct TY162599 {
  TGenericSeq Sup;
  Ropeobj162009* data[SEQ_DECL_SIZE];
};
N_NIMCALL(NI, len_162034)(Ropeobj162009* a);
static N_INLINE(void, nimFrame)(TFrame* s);
N_NOINLINE(void, stackoverflow_20001)(void);
static N_INLINE(void, popFrame)(void);
N_NIMCALL(Ropeobj162009*, newrope_162056)(NimStringDesc* data);
N_NIMCALL(void, nimGCvisit)(void* d, NI op);
N_NIMCALL(void, TMP823)(void* p, NI op);
N_NIMCALL(void*, newObj)(TNimType* typ, NI size);
N_NIMCALL(NimStringDesc*, copyStringRC1)(NimStringDesc* src);
static N_INLINE(void, nimGCunrefNoCycle)(void* p);
static N_INLINE(Tcell46747*, usrtocell_50246)(void* usr);
static N_INLINE(void, rtladdzct_51804)(Tcell46747* c);
N_NOINLINE(void, addzct_50217)(Tcellseq46763* s, Tcell46747* c);
N_NIMCALL(Ropeobj162009*, newmutablerope_162074)(NI capacity);
N_NIMCALL(NimStringDesc*, rawNewString)(NI space);
N_NIMCALL(NimStringDesc*, rawNewString)(NI cap);
static N_INLINE(NI, chckRange)(NI i, NI a, NI b);
N_NOINLINE(void, raiseRangeError)(NI64 val);
static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src);
static N_INLINE(void, freezemutablerope_162092)(Ropeobj162009* r);
N_NIMCALL(void, resetropecache_162111)(void);
static N_INLINE(NI, addInt)(NI a, NI b);
N_NOINLINE(void, raiseOverflow)(void);
N_NIMCALL(NIM_BOOL, ropeinvariant_162246)(Ropeobj162009* r);
N_NIMCALL(Ropeobj162009*, insertincache_162280)(NimStringDesc* s);
N_NIMCALL(NI, hash_131839)(NimStringDesc* x);
static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b);
N_NIMCALL(Ropeobj162009*, rope_162320)(NimStringDesc* s);
N_NIMCALL(void, failedassertimpl_87017)(NimStringDesc* msg);
N_NIMCALL(Ropeobj162009*, rope_162407)(NI64 i);
N_NIMCALL(NimStringDesc*, nimInt64ToStr)(NI64 x);
N_NIMCALL(Ropeobj162009*, rope_162426)(NF f);
N_NIMCALL(NimStringDesc*, nimFloatToStr)(NF f);
N_NIMCALL(Ropeobj162009*, HEX26_162436)(Ropeobj162009* a, Ropeobj162009* b);
N_NIMCALL(Ropeobj162009*, HEX26_162471)(Ropeobj162009* a, NimStringDesc* b);
N_NIMCALL(Ropeobj162009*, HEX26_162482)(NimStringDesc* a, Ropeobj162009* b);
N_NIMCALL(Ropeobj162009*, HEX26_162493)(Ropeobj162009** a, NI aLen0);
N_NOINLINE(void, raiseIndexError)(void);
N_NIMCALL(void, add_162534)(Ropeobj162009** a, Ropeobj162009* b);
N_NIMCALL(void, unsureAsgnRef)(void** dest, void* src);
N_NIMCALL(void, add_162545)(Ropeobj162009** a, NimStringDesc* b);
static N_INLINE(Ropeobj162009*, pop_162595)(TY162599** s);
static N_INLINE(NI, subInt)(NI a, NI b);
N_NIMCALL(TGenericSeq*, setLengthSeq)(TGenericSeq* seq, NI elemsize, NI newlen);
N_NIMCALL(void, writerope_163070)(FILE* f, Ropeobj162009* r);
N_NIMCALL(void*, newSeq)(TNimType* typ, NI len);
N_NIMCALL(TGenericSeq*, incrSeq)(TGenericSeq* seq, NI elemsize);
N_NIMCALL(void, write_13457)(FILE* f, NimStringDesc* s);
N_NIMCALL(void, writerope_163105)(Ropeobj162009* head, NimStringDesc* filename, NIM_BOOL usewarning);
N_NIMCALL(NIM_BOOL, open_13203)(FILE** f, NimStringDesc* filename, NU8 mode, NI bufsize);
N_NIMCALL(NimStringDesc*, HEX24_163131)(Ropeobj162009* r);
N_NIMCALL(NimStringDesc*, mnewString)(NI len);
N_NIMCALL(NimStringDesc*, mnewString)(NI len);
N_NIMCALL(NimStringDesc*, setLengthStr)(NimStringDesc* s, NI newlen);
static N_INLINE(void, appendString)(NimStringDesc* dest, NimStringDesc* src);
N_NIMCALL(NimStringDesc*, resizeString)(NimStringDesc* dest, NI addlen);
N_NIMCALL(Ropeobj162009*, ropeconcat_163164)(Ropeobj162009** a, NI aLen0);
N_NIMCALL(void, prepend_163205)(Ropeobj162009** a, Ropeobj162009* b);
N_NIMCALL(void, prepend_163216)(Ropeobj162009** a, NimStringDesc* b);
N_NIMCALL(Ropeobj162009*, HEX25_163235)(NimStringDesc* frmt, Ropeobj162009** args, NI argsLen0);
N_NIMCALL(NI, mulInt)(NI a, NI b);
N_NIMCALL(NimStringDesc*, nimIntToStr)(NI x);
N_NIMCALL(NimStringDesc*, nimCharToStr)(NIM_CHAR x);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* s, NI start_76643, NI last);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* s, NI first, NI last);
N_NIMCALL(void, addf_163635)(Ropeobj162009** c, NimStringDesc* frmt, Ropeobj162009** args, NI argsLen0);
N_NIMCALL(NIM_BOOL, equalsfile_163809)(Ropeobj162009* r, FILE* f);
N_NIMCALL(NI, readbuffer_13515)(FILE* f, void* buffer, NI len);
N_NIMCALL(NIM_BOOL, equalsfile_163916)(Ropeobj162009* r, NimStringDesc* filename);
N_NIMCALL(NIM_BOOL, writeropeifnotequal_163930)(Ropeobj162009* r, NimStringDesc* filename);
STRING_LITERAL(TMP827, "ropeInvariant(result) ", 22);
STRING_LITERAL(TMP833, "not (it == nil) ", 16);
STRING_LITERAL(TMP834, "not (it.data == nil) ", 21);
STRING_LITERAL(TMP838, "$", 1);
TY162023 errorhandler_162028;
extern TFrame* frameptr_17042;
extern TNimType NTI3411; /* RootObj */
TNimType NTI162009; /* RopeObj */
extern TNimType NTI108; /* int */
extern TNimType NTI149; /* string */
TNimType NTI162007; /* Rope */
extern Tgcheap48616 gch_48644;
TY162101 cache_162106;
NI gcachetries_162268;
NI gcachemisses_162272;
NI gcacheinttries_162276;
extern TNimType NTI162599; /* seq[Rope] */
Ropeobj162009* rnl_163227;
extern NimStringDesc* tnl_160626;
Ropeobj162009* softrnl_163228;

static N_INLINE(void, nimFrame)(TFrame* s) {
	NI LOC1;
	LOC1 = 0;
	{
		if (!(frameptr_17042 == NIM_NIL)) goto LA4;
		LOC1 = ((NI) 0);
	}
	goto LA2;
	LA4: ;
	{
		LOC1 = ((NI) ((NI16)((*frameptr_17042).calldepth + ((NI16) 1))));
	}
	LA2: ;
	(*s).calldepth = ((NI16) (LOC1));
	(*s).prev = frameptr_17042;
	frameptr_17042 = s;
	{
		if (!((*s).calldepth == ((NI16) 2000))) goto LA9;
		stackoverflow_20001();
	}
	LA9: ;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_17042 = (*frameptr_17042).prev;
}

N_NIMCALL(NI, len_162034)(Ropeobj162009* a) {
	NI result;
	nimfr("len", "ropes.nim")
	result = 0;
	nimln(86, "ropes.nim");
	{
		if (!(a == NIM_NIL)) goto LA3;
		result = ((NI) 0);
	}
	goto LA1;
	LA3: ;
	{
		nimln(87, "ropes.nim");
		result = (*a).length;
	}
	LA1: ;
	popFrame();
	return result;
}
N_NIMCALL(void, TMP823)(void* p, NI op) {
	Ropeobj162009* a;
	a = (Ropeobj162009*)p;
	nimGCvisit((void*)(*a).left, op);
	nimGCvisit((void*)(*a).right, op);
	nimGCvisit((void*)(*a).data, op);
}

static N_INLINE(Tcell46747*, usrtocell_50246)(void* usr) {
	Tcell46747* result;
	nimfr("usrToCell", "gc.nim")
	result = 0;
	nimln(118, "gc.nim");
	result = ((Tcell46747*) ((NI)((NU64)(((NI) (usr))) - (NU64)(((NI)sizeof(Tcell46747))))));
	popFrame();
	return result;
}

static N_INLINE(void, rtladdzct_51804)(Tcell46747* c) {
	nimfr("rtlAddZCT", "gc.nim")
	nimln(199, "gc.nim");
	addzct_50217((&gch_48644.zct), c);
	popFrame();
}

static N_INLINE(void, nimGCunrefNoCycle)(void* p) {
	Tcell46747* c;
	nimfr("nimGCunrefNoCycle", "gc.nim")
	nimln(233, "gc.nim");
	c = usrtocell_50246(p);
	nimln(235, "gc.nim");
	{
		nimln(167, "gc.nim");
		(*c).refcount -= ((NI) 8);
		nimln(168, "gc.nim");
		if (!((NU64)((*c).refcount) < (NU64)(((NI) 8)))) goto LA3;
		nimln(236, "gc.nim");
		rtladdzct_51804(c);
	}
	LA3: ;
	popFrame();
}

N_NIMCALL(Ropeobj162009*, newrope_162056)(NimStringDesc* data) {
	Ropeobj162009* result;
	nimfr("newRope", "ropes.nim")
	result = 0;
	nimln(90, "ropes.nim");
	result = (Ropeobj162009*) newObj((&NTI162007), sizeof(Ropeobj162009));
	(*result).Sup.m_type = (&NTI162009);
	nimln(91, "ropes.nim");
	{
		NimStringDesc* LOC5;
		if (!!((data == NIM_NIL))) goto LA3;
		nimln(92, "ropes.nim");
		(*result).length = (data ? data->Sup.len : 0);
		nimln(93, "ropes.nim");
		LOC5 = 0;
		LOC5 = (*result).data; (*result).data = copyStringRC1(data);
		if (LOC5) nimGCunrefNoCycle(LOC5);
	}
	LA3: ;
	popFrame();
	return result;
}

static N_INLINE(NI, chckRange)(NI i, NI a, NI b) {
	NI result;
{	result = 0;
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (a <= i);
		if (!(LOC3)) goto LA4;
		LOC3 = (i <= b);
		LA4: ;
		if (!LOC3) goto LA5;
		result = i;
		goto BeforeRet;
	}
	goto LA1;
	LA5: ;
	{
		raiseRangeError(((NI64) (i)));
	}
	LA1: ;
	}BeforeRet: ;
	return result;
}

static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src) {
	nimfr("asgnRefNoCycle", "gc.nim")
	nimln(251, "gc.nim");
	{
		Tcell46747* c;
		if (!!((src == NIM_NIL))) goto LA3;
		nimln(252, "gc.nim");
		c = usrtocell_50246(src);
		nimln(169, "gc.nim");
		(*c).refcount += ((NI) 8);
	}
	LA3: ;
	nimln(254, "gc.nim");
	{
		Tcell46747* c;
		if (!!(((*dest) == NIM_NIL))) goto LA7;
		nimln(255, "gc.nim");
		c = usrtocell_50246((*dest));
		nimln(256, "gc.nim");
		{
			nimln(167, "gc.nim");
			(*c).refcount -= ((NI) 8);
			nimln(168, "gc.nim");
			if (!((NU64)((*c).refcount) < (NU64)(((NI) 8)))) goto LA11;
			nimln(257, "gc.nim");
			rtladdzct_51804(c);
		}
		LA11: ;
	}
	LA7: ;
	nimln(258, "gc.nim");
	(*dest) = src;
	popFrame();
}

N_NIMCALL(Ropeobj162009*, newmutablerope_162074)(NI capacity) {
	Ropeobj162009* result;
	nimfr("newMutableRope", "ropes.nim")
	result = 0;
	nimln(98, "ropes.nim");
	result = (Ropeobj162009*) newObj((&NTI162007), sizeof(Ropeobj162009));
	(*result).Sup.m_type = (&NTI162009);
	nimln(99, "ropes.nim");
	asgnRefNoCycle((void**) (&(*result).data), rawNewString(((NI)chckRange(capacity, ((NI) 0), ((NI) IL64(9223372036854775807))))));
	popFrame();
	return result;
}

static N_INLINE(void, freezemutablerope_162092)(Ropeobj162009* r) {
	nimfr("freezeMutableRope", "ropes.nim")
	nimln(102, "ropes.nim");
	(*r).length = ((*r).data ? (*r).data->Sup.len : 0);
	popFrame();
}

static N_INLINE(NI, addInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU64)(a) + (NU64)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ b));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

N_NIMCALL(void, resetropecache_162111)(void) {
	nimfr("resetRopeCache", "ropes.nim")
	{
		NI i_162223;
		NI res_162228;
		i_162223 = 0;
		nimln(1598, "system.nim");
		res_162228 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP824;
				if (!(res_162228 <= ((NI) 4095))) goto LA3;
				nimln(1600, "system.nim");
				i_162223 = ((NI)chckRange(res_162228, ((NI) 0), ((NI) 4095)));
				nimln(109, "ropes.nim");
				asgnRefNoCycle((void**) (&cache_162106[(i_162223)- 0]), NIM_NIL);
				nimln(1619, "system.nim");
				TMP824 = addInt(res_162228, ((NI) 1));
				res_162228 = (NI)(TMP824);
			} LA3: ;
		}
	}
	popFrame();
}

N_NIMCALL(NIM_BOOL, ropeinvariant_162246)(Ropeobj162009* r) {
	NIM_BOOL result;
	nimfr("ropeInvariant", "ropes.nim")
	result = 0;
	nimln(112, "ropes.nim");
	{
		if (!(r == NIM_NIL)) goto LA3;
		nimln(113, "ropes.nim");
		result = NIM_TRUE;
	}
	goto LA1;
	LA3: ;
	{
		nimln(115, "ropes.nim");
		result = NIM_TRUE;
	}
	LA1: ;
	popFrame();
	return result;
}

static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b) {
	NIM_BOOL result;
	NIM_BOOL LOC11;
	int LOC13;
{	result = 0;
	{
		if (!(a == b)) goto LA3;
		result = NIM_TRUE;
		goto BeforeRet;
	}
	LA3: ;
	{
		NIM_BOOL LOC7;
		LOC7 = 0;
		LOC7 = (a == NIM_NIL);
		if (LOC7) goto LA8;
		LOC7 = (b == NIM_NIL);
		LA8: ;
		if (!LOC7) goto LA9;
		result = NIM_FALSE;
		goto BeforeRet;
	}
	LA9: ;
	LOC11 = 0;
	LOC11 = ((*a).Sup.len == (*b).Sup.len);
	if (!(LOC11)) goto LA12;
	LOC13 = 0;
	LOC13 = memcmp(((NCSTRING) ((*a).data)), ((NCSTRING) ((*b).data)), (*a).Sup.len);
	LOC11 = (LOC13 == ((NI32) 0));
	LA12: ;
	result = LOC11;
	goto BeforeRet;
	}BeforeRet: ;
	return result;
}

N_NIMCALL(Ropeobj162009*, insertincache_162280)(NimStringDesc* s) {
	Ropeobj162009* result;
	NI TMP825;
	NI h;
	NI LOC1;
	nimfr("insertInCache", "ropes.nim")
	result = 0;
	nimln(129, "ropes.nim");
	TMP825 = addInt(gcachetries_162268, ((NI) 1));
	gcachetries_162268 = (NI)(TMP825);
	nimln(130, "ropes.nim");
	LOC1 = 0;
	LOC1 = hash_131839(s);
	h = (NI)(LOC1 & ((NI) 4095));
	nimln(131, "ropes.nim");
	result = cache_162106[(h)- 0];
	nimln(132, "ropes.nim");
	{
		NIM_BOOL LOC4;
		NI TMP826;
		LOC4 = 0;
		LOC4 = result == 0;
		if (LOC4) goto LA5;
		LOC4 = !(eqStrings((*result).data, s));
		LA5: ;
		if (!LOC4) goto LA6;
		nimln(133, "ropes.nim");
		TMP826 = addInt(gcachemisses_162272, ((NI) 1));
		gcachemisses_162272 = (NI)(TMP826);
		nimln(134, "ropes.nim");
		result = newrope_162056(s);
		nimln(135, "ropes.nim");
		asgnRefNoCycle((void**) (&cache_162106[(h)- 0]), result);
	}
	LA6: ;
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, rope_162320)(NimStringDesc* s) {
	Ropeobj162009* result;
	nimfr("rope", "ropes.nim")
	result = 0;
	nimln(139, "ropes.nim");
	{
		if (!((s ? s->Sup.len : 0) == ((NI) 0))) goto LA3;
		nimln(140, "ropes.nim");
		result = NIM_NIL;
	}
	goto LA1;
	LA3: ;
	{
		nimln(142, "ropes.nim");
		result = insertincache_162280(s);
	}
	LA1: ;
	nimln(143, "ropes.nim");
	{
		NIM_BOOL LOC8;
		LOC8 = 0;
		LOC8 = ropeinvariant_162246(result);
		if (!!(LOC8)) goto LA9;
		failedassertimpl_87017(((NimStringDesc*) &TMP827));
	}
	LA9: ;
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, rope_162407)(NI64 i) {
	Ropeobj162009* result;
	NI TMP828;
	NimStringDesc* LOC1;
	nimfr("rope", "ropes.nim")
	result = 0;
	nimln(147, "ropes.nim");
	TMP828 = addInt(gcacheinttries_162276, ((NI) 1));
	gcacheinttries_162276 = (NI)(TMP828);
	nimln(148, "ropes.nim");
	LOC1 = 0;
	LOC1 = nimInt64ToStr(i);
	result = rope_162320(LOC1);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, rope_162426)(NF f) {
	Ropeobj162009* result;
	NimStringDesc* LOC1;
	nimfr("rope", "ropes.nim")
	result = 0;
	nimln(152, "ropes.nim");
	LOC1 = 0;
	LOC1 = nimFloatToStr(f);
	result = rope_162320(LOC1);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, HEX26_162436)(Ropeobj162009* a, Ropeobj162009* b) {
	Ropeobj162009* result;
	nimfr("&", "ropes.nim")
	result = 0;
	nimln(155, "ropes.nim");
	{
		if (!(a == NIM_NIL)) goto LA3;
		nimln(156, "ropes.nim");
		result = b;
	}
	goto LA1;
	LA3: ;
	{
		nimln(157, "ropes.nim");
		if (!(b == NIM_NIL)) goto LA6;
		nimln(158, "ropes.nim");
		result = a;
	}
	goto LA1;
	LA6: ;
	{
		NI TMP829;
		nimln(160, "ropes.nim");
		result = newrope_162056(NIM_NIL);
		nimln(161, "ropes.nim");
		TMP829 = addInt((*a).length, (*b).length);
		(*result).length = (NI)(TMP829);
		nimln(162, "ropes.nim");
		asgnRefNoCycle((void**) (&(*result).left), a);
		nimln(163, "ropes.nim");
		asgnRefNoCycle((void**) (&(*result).right), b);
	}
	LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, HEX26_162471)(Ropeobj162009* a, NimStringDesc* b) {
	Ropeobj162009* result;
	Ropeobj162009* LOC1;
	nimfr("&", "ropes.nim")
	result = 0;
	nimln(167, "ropes.nim");
	LOC1 = 0;
	LOC1 = rope_162320(b);
	result = HEX26_162436(a, LOC1);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, HEX26_162482)(NimStringDesc* a, Ropeobj162009* b) {
	Ropeobj162009* result;
	Ropeobj162009* LOC1;
	nimfr("&", "ropes.nim")
	result = 0;
	nimln(171, "ropes.nim");
	LOC1 = 0;
	LOC1 = rope_162320(a);
	result = HEX26_162436(LOC1, b);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, HEX26_162493)(Ropeobj162009** a, NI aLen0) {
	Ropeobj162009* result;
	nimfr("&", "ropes.nim")
	result = 0;
	{
		NI i_162510;
		NI HEX3Atmp_162512;
		NI res_162515;
		i_162510 = 0;
		HEX3Atmp_162512 = 0;
		nimln(175, "ropes.nim");
		HEX3Atmp_162512 = (aLen0-1);
		nimln(1598, "system.nim");
		res_162515 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP830;
				if (!(res_162515 <= HEX3Atmp_162512)) goto LA3;
				nimln(1600, "system.nim");
				i_162510 = res_162515;
				nimln(175, "ropes.nim");
				if ((NU)(i_162510) >= (NU)(aLen0)) raiseIndexError();
				result = HEX26_162436(result, a[i_162510]);
				nimln(1614, "system.nim");
				TMP830 = addInt(res_162515, ((NI) 1));
				res_162515 = (NI)(TMP830);
			} LA3: ;
		}
	}
	popFrame();
	return result;
}

N_NIMCALL(void, add_162534)(Ropeobj162009** a, Ropeobj162009* b) {
	nimfr("add", "ropes.nim")
	nimln(179, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_162436((*a), b));
	popFrame();
}

N_NIMCALL(void, add_162545)(Ropeobj162009** a, NimStringDesc* b) {
	nimfr("add", "ropes.nim")
	nimln(183, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_162471((*a), b));
	popFrame();
}

static N_INLINE(NI, subInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU64)(a) - (NU64)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ (NI)((NU64) ~(b))));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

static N_INLINE(Ropeobj162009*, pop_162595)(TY162599** s) {
	Ropeobj162009* result;
	NI L;
	NI TMP831;
	nimfr("pop", "system.nim")
	result = 0;
	nimln(1926, "system.nim");
	TMP831 = subInt(((*s) ? (*s)->Sup.len : 0), ((NI) 1));
	L = (NI)(TMP831);
	nimln(1927, "system.nim");
	if ((NU)(L) >= (NU)((*s)->Sup.len)) raiseIndexError();
	result = (*s)->data[L];
	nimln(1928, "system.nim");
	(*s) = (TY162599*) setLengthSeq(&((*s))->Sup, sizeof(Ropeobj162009*), ((NI)chckRange(L, ((NI) 0), ((NI) IL64(9223372036854775807)))));
	popFrame();
	return result;
}

N_NIMCALL(void, writerope_163070)(FILE* f, Ropeobj162009* r) {
	nimfr("writeRope", "ropes.nim")
	{
		NimStringDesc* s_163074;
		s_163074 = 0;
		nimln(187, "ropes.nim");
		{
			TY162599* stack_163081;
			TY162572 LOC6;
			if (!!((r == NIM_NIL))) goto LA4;
			nimln(188, "ropes.nim");
			stack_163081 = (TY162599*) newSeq((&NTI162599), 1);
			memset((void*)LOC6, 0, sizeof(LOC6));
			LOC6[0] = r;
			asgnRefNoCycle((void**) (&stack_163081->data[0]), LOC6[0]);
			{
				nimln(189, "ropes.nim");
				while (1) {
					Ropeobj162009* it_163083;
					if (!(((NI) 0) < (stack_163081 ? stack_163081->Sup.len : 0))) goto LA8;
					nimln(190, "ropes.nim");
					it_163083 = pop_162595((&stack_163081));
					{
						nimln(191, "ropes.nim");
						while (1) {
							if (!(*it_163083).data == 0) goto LA10;
							nimln(192, "ropes.nim");
							stack_163081 = (TY162599*) incrSeq(&(stack_163081)->Sup, sizeof(Ropeobj162009*));
							asgnRefNoCycle((void**) (&stack_163081->data[stack_163081->Sup.len-1]), (*it_163083).right);
							nimln(193, "ropes.nim");
							it_163083 = (*it_163083).left;
							nimln(194, "ropes.nim");
							{
								if (!!(!((it_163083 == NIM_NIL)))) goto LA13;
								failedassertimpl_87017(((NimStringDesc*) &TMP833));
							}
							LA13: ;
						} LA10: ;
					}
					nimln(195, "ropes.nim");
					{
						if (!!(!(((*it_163083).data == NIM_NIL)))) goto LA17;
						failedassertimpl_87017(((NimStringDesc*) &TMP834));
					}
					LA17: ;
					nimln(196, "ropes.nim");
					s_163074 = (*it_163083).data;
					nimln(205, "ropes.nim");
					write_13457(f, s_163074);
				} LA8: ;
			}
		}
		LA4: ;
	}
	popFrame();
}

N_NIMCALL(void, writerope_163105)(Ropeobj162009* head, NimStringDesc* filename, NIM_BOOL usewarning) {
	FILE* f;
	nimfr("writeRope", "ropes.nim")
	f = 0;
	nimln(209, "ropes.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = open_13203(&f, filename, ((NU8) 1), ((NI) -1));
		if (!LOC3) goto LA4;
		nimln(210, "ropes.nim");
		{
			if (!!((head == NIM_NIL))) goto LA8;
			writerope_163070(f, head);
		}
		LA8: ;
		nimln(211, "ropes.nim");
		fclose(f);
	}
	goto LA1;
	LA4: ;
	{
		nimln(213, "ropes.nim");
		errorhandler_162028.ClEnv? errorhandler_162028.ClPrc(((NU8) 0), filename, usewarning, errorhandler_162028.ClEnv):((TMP835)(errorhandler_162028.ClPrc))(((NU8) 0), filename, usewarning);
	}
	LA1: ;
	popFrame();
}

static N_INLINE(void, appendString)(NimStringDesc* dest, NimStringDesc* src) {
	memcpy(((NCSTRING) ((&(*dest).data[((*dest).Sup.len)- 0]))), ((NCSTRING) ((*src).data)), (NI)((*src).Sup.len + ((NI) 1)));
	(*dest).Sup.len += (*src).Sup.len;
}

N_NIMCALL(NimStringDesc*, HEX24_163131)(Ropeobj162009* r) {
	NimStringDesc* result;
	NI LOC1;
	nimfr("$", "ropes.nim")
	result = 0;
	nimln(217, "ropes.nim");
	LOC1 = 0;
	LOC1 = len_162034(r);
	result = mnewString(((NI)chckRange(LOC1, ((NI) 0), ((NI) IL64(9223372036854775807)))));
	nimln(218, "ropes.nim");
	result = setLengthStr(result, ((NI) 0));
	{
		NimStringDesc* s_163135;
		s_163135 = 0;
		nimln(187, "ropes.nim");
		{
			TY162599* stack_163140;
			TY162572 LOC7;
			if (!!((r == NIM_NIL))) goto LA5;
			nimln(188, "ropes.nim");
			stack_163140 = (TY162599*) newSeq((&NTI162599), 1);
			memset((void*)LOC7, 0, sizeof(LOC7));
			LOC7[0] = r;
			asgnRefNoCycle((void**) (&stack_163140->data[0]), LOC7[0]);
			{
				nimln(189, "ropes.nim");
				while (1) {
					Ropeobj162009* it_163142;
					if (!(((NI) 0) < (stack_163140 ? stack_163140->Sup.len : 0))) goto LA9;
					nimln(190, "ropes.nim");
					it_163142 = pop_162595((&stack_163140));
					{
						nimln(191, "ropes.nim");
						while (1) {
							if (!(*it_163142).data == 0) goto LA11;
							nimln(192, "ropes.nim");
							stack_163140 = (TY162599*) incrSeq(&(stack_163140)->Sup, sizeof(Ropeobj162009*));
							asgnRefNoCycle((void**) (&stack_163140->data[stack_163140->Sup.len-1]), (*it_163142).right);
							nimln(193, "ropes.nim");
							it_163142 = (*it_163142).left;
							nimln(194, "ropes.nim");
							{
								if (!!(!((it_163142 == NIM_NIL)))) goto LA14;
								failedassertimpl_87017(((NimStringDesc*) &TMP833));
							}
							LA14: ;
						} LA11: ;
					}
					nimln(195, "ropes.nim");
					{
						if (!!(!(((*it_163142).data == NIM_NIL)))) goto LA18;
						failedassertimpl_87017(((NimStringDesc*) &TMP834));
					}
					LA18: ;
					nimln(196, "ropes.nim");
					s_163135 = (*it_163142).data;
					nimln(219, "ropes.nim");
					result = resizeString(result, s_163135->Sup.len + 0);
appendString(result, s_163135);
				} LA9: ;
			}
		}
		LA5: ;
	}
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj162009*, ropeconcat_163164)(Ropeobj162009** a, NI aLen0) {
	Ropeobj162009* result;
	nimfr("ropeConcat", "ropes.nim")
	result = 0;
	{
		NI i_163181;
		NI HEX3Atmp_163183;
		NI res_163186;
		i_163181 = 0;
		HEX3Atmp_163183 = 0;
		nimln(223, "ropes.nim");
		HEX3Atmp_163183 = (aLen0-1);
		nimln(1598, "system.nim");
		res_163186 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP836;
				if (!(res_163186 <= HEX3Atmp_163183)) goto LA3;
				nimln(1600, "system.nim");
				i_163181 = res_163186;
				nimln(223, "ropes.nim");
				if ((NU)(i_163181) >= (NU)(aLen0)) raiseIndexError();
				result = HEX26_162436(result, a[i_163181]);
				nimln(1614, "system.nim");
				TMP836 = addInt(res_163186, ((NI) 1));
				res_163186 = (NI)(TMP836);
			} LA3: ;
		}
	}
	popFrame();
	return result;
}

N_NIMCALL(void, prepend_163205)(Ropeobj162009** a, Ropeobj162009* b) {
	nimfr("prepend", "ropes.nim")
	nimln(225, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_162436(b, (*a)));
	popFrame();
}

N_NIMCALL(void, prepend_163216)(Ropeobj162009** a, NimStringDesc* b) {
	nimfr("prepend", "ropes.nim")
	nimln(226, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_162482(b, (*a)));
	popFrame();
}

N_NIMCALL(Ropeobj162009*, HEX25_163235)(NimStringDesc* frmt, Ropeobj162009** args, NI argsLen0) {
	Ropeobj162009* result;
	NI i;
	NI length;
	NI num;
	nimfr("%", "ropes.nim")
	result = 0;
	nimln(233, "ropes.nim");
	i = ((NI) 0);
	nimln(234, "ropes.nim");
	length = (frmt ? frmt->Sup.len : 0);
	nimln(235, "ropes.nim");
	result = NIM_NIL;
	nimln(236, "ropes.nim");
	num = ((NI) 0);
	{
		nimln(237, "ropes.nim");
		while (1) {
			NI start;
			if (!(i < length)) goto LA2;
			nimln(238, "ropes.nim");
			{
				NI TMP837;
				if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
				if (!((NU8)(frmt->data[i]) == (NU8)(36))) goto LA5;
				nimln(239, "ropes.nim");
				TMP837 = addInt(i, ((NI) 1));
				i = (NI)(TMP837);
				nimln(240, "ropes.nim");
				if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
				switch (((NU8)(frmt->data[i]))) {
				case 36:
				{
					NI TMP839;
					nimln(242, "ropes.nim");
					add_162545(&result, ((NimStringDesc*) &TMP838));
					nimln(243, "ropes.nim");
					TMP839 = addInt(i, ((NI) 1));
					i = (NI)(TMP839);
				}
				break;
				case 35:
				{
					NI TMP840;
					NI TMP841;
					nimln(245, "ropes.nim");
					TMP840 = addInt(i, ((NI) 1));
					i = (NI)(TMP840);
					nimln(246, "ropes.nim");
					if ((NU)(num) >= (NU)(argsLen0)) raiseIndexError();
					add_162534(&result, args[num]);
					nimln(247, "ropes.nim");
					TMP841 = addInt(num, ((NI) 1));
					num = (NI)(TMP841);
				}
				break;
				case 48 ... 57:
				{
					NI j;
					nimln(249, "ropes.nim");
					j = ((NI) 0);
					{
						nimln(250, "ropes.nim");
						while (1) {
							NI TMP842;
							NI TMP843;
							NI TMP844;
							NI TMP845;
							nimln(251, "ropes.nim");
							TMP842 = mulInt(j, ((NI) 10));
							if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
							TMP843 = addInt((NI)(TMP842), ((NI) (((NU8)(frmt->data[i])))));
							TMP844 = subInt((NI)(TMP843), ((NI) 48));
							j = (NI)(TMP844);
							nimln(252, "ropes.nim");
							TMP845 = addInt(i, ((NI) 1));
							i = (NI)(TMP845);
							nimln(253, "ropes.nim");
							{
								if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
								if (!!((((NU8)(frmt->data[i])) >= ((NU8)(48)) && ((NU8)(frmt->data[i])) <= ((NU8)(57))))) goto LA14;
								goto LA10;
							}
							LA14: ;
						}
					} LA10: ;
					nimln(254, "ropes.nim");
					num = j;
					nimln(255, "ropes.nim");
					{
						NI TMP846;
						NimStringDesc* LOC20;
						TMP846 = addInt((argsLen0-1), ((NI) 1));
						if (!((NI)(TMP846) < j)) goto LA18;
						nimln(256, "ropes.nim");
						LOC20 = 0;
						LOC20 = nimIntToStr(j);
						errorhandler_162028.ClEnv? errorhandler_162028.ClPrc(((NU8) 1), LOC20, NIM_FALSE, errorhandler_162028.ClEnv):((TMP847)(errorhandler_162028.ClPrc))(((NU8) 1), LOC20, NIM_FALSE);
					}
					goto LA16;
					LA18: ;
					{
						NI TMP848;
						nimln(258, "ropes.nim");
						TMP848 = subInt(j, ((NI) 1));
						if ((NU)((NI)(TMP848)) >= (NU)(argsLen0)) raiseIndexError();
						add_162534(&result, args[(NI)(TMP848)]);
					}
					LA16: ;
				}
				break;
				case 123:
				{
					NI TMP849;
					NI j;
					nimln(260, "ropes.nim");
					TMP849 = addInt(i, ((NI) 1));
					i = (NI)(TMP849);
					nimln(261, "ropes.nim");
					j = ((NI) 0);
					{
						nimln(262, "ropes.nim");
						while (1) {
							NI TMP850;
							NI TMP851;
							NI TMP852;
							NI TMP853;
							if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
							if (!(((NU8)(frmt->data[i])) >= ((NU8)(48)) && ((NU8)(frmt->data[i])) <= ((NU8)(57)))) goto LA24;
							nimln(263, "ropes.nim");
							TMP850 = mulInt(j, ((NI) 10));
							if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
							TMP851 = addInt((NI)(TMP850), ((NI) (((NU8)(frmt->data[i])))));
							TMP852 = subInt((NI)(TMP851), ((NI) 48));
							j = (NI)(TMP852);
							nimln(264, "ropes.nim");
							TMP853 = addInt(i, ((NI) 1));
							i = (NI)(TMP853);
						} LA24: ;
					}
					nimln(265, "ropes.nim");
					num = j;
					nimln(266, "ropes.nim");
					{
						NI TMP854;
						if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
						if (!((NU8)(frmt->data[i]) == (NU8)(125))) goto LA27;
						TMP854 = addInt(i, ((NI) 1));
						i = (NI)(TMP854);
					}
					goto LA25;
					LA27: ;
					{
						NimStringDesc* LOC30;
						nimln(267, "ropes.nim");
						if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
						LOC30 = 0;
						LOC30 = nimCharToStr(frmt->data[i]);
						errorhandler_162028.ClEnv? errorhandler_162028.ClPrc(((NU8) 1), LOC30, NIM_FALSE, errorhandler_162028.ClEnv):((TMP855)(errorhandler_162028.ClPrc))(((NU8) 1), LOC30, NIM_FALSE);
					}
					LA25: ;
					nimln(269, "ropes.nim");
					{
						NI TMP856;
						NimStringDesc* LOC35;
						TMP856 = addInt((argsLen0-1), ((NI) 1));
						if (!((NI)(TMP856) < j)) goto LA33;
						nimln(270, "ropes.nim");
						LOC35 = 0;
						LOC35 = nimIntToStr(j);
						errorhandler_162028.ClEnv? errorhandler_162028.ClPrc(((NU8) 1), LOC35, NIM_FALSE, errorhandler_162028.ClEnv):((TMP857)(errorhandler_162028.ClPrc))(((NU8) 1), LOC35, NIM_FALSE);
					}
					goto LA31;
					LA33: ;
					{
						NI TMP858;
						nimln(272, "ropes.nim");
						TMP858 = subInt(j, ((NI) 1));
						if ((NU)((NI)(TMP858)) >= (NU)(argsLen0)) raiseIndexError();
						add_162534(&result, args[(NI)(TMP858)]);
					}
					LA31: ;
				}
				break;
				case 110:
				{
					NI TMP859;
					nimln(274, "ropes.nim");
					add_162534(&result, softrnl_163228);
					nimln(275, "ropes.nim");
					TMP859 = addInt(i, ((NI) 1));
					i = (NI)(TMP859);
				}
				break;
				case 78:
				{
					NI TMP860;
					nimln(277, "ropes.nim");
					add_162534(&result, rnl_163227);
					nimln(278, "ropes.nim");
					TMP860 = addInt(i, ((NI) 1));
					i = (NI)(TMP860);
				}
				break;
				default:
				{
					NimStringDesc* LOC40;
					nimln(280, "ropes.nim");
					if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
					LOC40 = 0;
					LOC40 = nimCharToStr(frmt->data[i]);
					errorhandler_162028.ClEnv? errorhandler_162028.ClPrc(((NU8) 1), LOC40, NIM_FALSE, errorhandler_162028.ClEnv):((TMP861)(errorhandler_162028.ClPrc))(((NU8) 1), LOC40, NIM_FALSE);
				}
				break;
				}
			}
			LA5: ;
			nimln(281, "ropes.nim");
			start = i;
			{
				nimln(282, "ropes.nim");
				while (1) {
					if (!(i < length)) goto LA42;
					nimln(283, "ropes.nim");
					{
						NI TMP862;
						if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
						if (!!(((NU8)(frmt->data[i]) == (NU8)(36)))) goto LA45;
						TMP862 = addInt(i, ((NI) 1));
						i = (NI)(TMP862);
					}
					goto LA43;
					LA45: ;
					{
						nimln(284, "ropes.nim");
						goto LA41;
					}
					LA43: ;
				} LA42: ;
			} LA41: ;
			nimln(285, "ropes.nim");
			{
				NI TMP863;
				NI TMP864;
				NimStringDesc* LOC52;
				TMP863 = subInt(i, ((NI) 1));
				if (!(start <= (NI)(TMP863))) goto LA50;
				nimln(286, "ropes.nim");
				TMP864 = subInt(i, ((NI) 1));
				LOC52 = 0;
				LOC52 = copyStrLast(frmt, start, (NI)(TMP864));
				add_162545(&result, LOC52);
			}
			LA50: ;
		} LA2: ;
	}
	nimln(287, "ropes.nim");
	{
		NIM_BOOL LOC55;
		LOC55 = 0;
		LOC55 = ropeinvariant_162246(result);
		if (!!(LOC55)) goto LA56;
		failedassertimpl_87017(((NimStringDesc*) &TMP827));
	}
	LA56: ;
	popFrame();
	return result;
}

N_NIMCALL(void, addf_163635)(Ropeobj162009** c, NimStringDesc* frmt, Ropeobj162009** args, NI argsLen0) {
	Ropeobj162009* LOC1;
	nimfr("addf", "ropes.nim")
	nimln(291, "ropes.nim");
	LOC1 = 0;
	LOC1 = HEX25_163235(frmt, args, argsLen0);
	add_162534(c, LOC1);
	popFrame();
}

N_NIMCALL(NIM_BOOL, equalsfile_163809)(Ropeobj162009* r, FILE* f) {
	NIM_BOOL result;
	TY163814 buf;
	NI bpos;
	NI blen;
	NI LOC34;
	nimfr("equalsFile", "ropes.nim")
{	result = 0;
	memset((void*)buf, 0, sizeof(buf));
	nimln(311, "ropes.nim");
	bpos = ((NI) 1024);
	nimln(312, "ropes.nim");
	blen = ((NI) 1024);
	{
		NimStringDesc* s_163837;
		s_163837 = 0;
		nimln(187, "ropes.nim");
		{
			TY162599* stack_163886;
			TY162572 LOC6;
			if (!!((r == NIM_NIL))) goto LA4;
			nimln(188, "ropes.nim");
			stack_163886 = (TY162599*) newSeq((&NTI162599), 1);
			memset((void*)LOC6, 0, sizeof(LOC6));
			LOC6[0] = r;
			asgnRefNoCycle((void**) (&stack_163886->data[0]), LOC6[0]);
			{
				nimln(189, "ropes.nim");
				while (1) {
					Ropeobj162009* it_163888;
					NI spos;
					NI slen;
					if (!(((NI) 0) < (stack_163886 ? stack_163886->Sup.len : 0))) goto LA8;
					nimln(190, "ropes.nim");
					it_163888 = pop_162595((&stack_163886));
					{
						nimln(191, "ropes.nim");
						while (1) {
							if (!(*it_163888).data == 0) goto LA10;
							nimln(192, "ropes.nim");
							stack_163886 = (TY162599*) incrSeq(&(stack_163886)->Sup, sizeof(Ropeobj162009*));
							asgnRefNoCycle((void**) (&stack_163886->data[stack_163886->Sup.len-1]), (*it_163888).right);
							nimln(193, "ropes.nim");
							it_163888 = (*it_163888).left;
							nimln(194, "ropes.nim");
							{
								if (!!(!((it_163888 == NIM_NIL)))) goto LA13;
								failedassertimpl_87017(((NimStringDesc*) &TMP833));
							}
							LA13: ;
						} LA10: ;
					}
					nimln(195, "ropes.nim");
					{
						if (!!(!(((*it_163888).data == NIM_NIL)))) goto LA17;
						failedassertimpl_87017(((NimStringDesc*) &TMP834));
					}
					LA17: ;
					nimln(196, "ropes.nim");
					s_163837 = (*it_163888).data;
					nimln(315, "ropes.nim");
					spos = ((NI) 0);
					nimln(316, "ropes.nim");
					slen = (s_163837 ? s_163837->Sup.len : 0);
					{
						nimln(317, "ropes.nim");
						while (1) {
							NI n;
							NI TMP865;
							NI TMP866;
							NI TMP868;
							NI TMP869;
							if (!(spos < slen)) goto LA20;
							nimln(318, "ropes.nim");
							{
								if (!(bpos == blen)) goto LA23;
								nimln(320, "ropes.nim");
								bpos = ((NI) 0);
								nimln(321, "ropes.nim");
								blen = readbuffer_13515(f, ((void*) ((&buf[(((NI) 0))- 0]))), ((NI) 1024));
								nimln(322, "ropes.nim");
								{
									if (!(blen == ((NI) 0))) goto LA27;
									nimln(323, "ropes.nim");
									result = NIM_FALSE;
									nimln(324, "ropes.nim");
									goto BeforeRet;
								}
								LA27: ;
							}
							LA23: ;
							nimln(325, "ropes.nim");
							TMP865 = subInt(blen, bpos);
							TMP866 = subInt(slen, spos);
							n = (((NI)(TMP865) <= (NI)(TMP866)) ? (NI)(TMP865) : (NI)(TMP866));
							nimln(327, "ropes.nim");
							{
								NI TMP867;
								NIM_BOOL LOC31;
								if ((NU)(bpos) > (NU)(1023)) raiseIndexError();
								TMP867 = addInt(((NI) (s_163837->data)), spos);
								LOC31 = 0;
								LOC31 = equalMem(((void*) ((&buf[(bpos)- 0]))), ((void*) ((NI)(TMP867))), ((NI)chckRange(n, ((NI) 0), ((NI) IL64(9223372036854775807)))));
								if (!!(LOC31)) goto LA32;
								nimln(328, "ropes.nim");
								result = NIM_FALSE;
								nimln(329, "ropes.nim");
								goto BeforeRet;
							}
							LA32: ;
							nimln(330, "ropes.nim");
							TMP868 = addInt(spos, n);
							spos = (NI)(TMP868);
							nimln(331, "ropes.nim");
							TMP869 = addInt(bpos, n);
							bpos = (NI)(TMP869);
						} LA20: ;
					}
				} LA8: ;
			}
		}
		LA4: ;
	}
	nimln(333, "ropes.nim");
	LOC34 = 0;
	LOC34 = readbuffer_13515(f, ((void*) ((&buf[(((NI) 0))- 0]))), ((NI) 1));
	result = (LOC34 == ((NI) 0));
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, equalsfile_163916)(Ropeobj162009* r, NimStringDesc* filename) {
	NIM_BOOL result;
	FILE* f;
	nimfr("equalsFile", "ropes.nim")
	result = 0;
	f = 0;
	nimln(339, "ropes.nim");
	result = open_13203(&f, filename, ((NU8) 0), ((NI) -1));
	nimln(340, "ropes.nim");
	{
		if (!result) goto LA3;
		nimln(341, "ropes.nim");
		result = equalsfile_163809(r, f);
		nimln(342, "ropes.nim");
		fclose(f);
	}
	LA3: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, writeropeifnotequal_163930)(Ropeobj162009* r, NimStringDesc* filename) {
	NIM_BOOL result;
	nimfr("writeRopeIfNotEqual", "ropes.nim")
	result = 0;
	nimln(346, "ropes.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = equalsfile_163916(r, filename);
		if (!!(LOC3)) goto LA4;
		nimln(347, "ropes.nim");
		writerope_163105(r, filename, NIM_FALSE);
		nimln(348, "ropes.nim");
		result = NIM_TRUE;
	}
	goto LA1;
	LA4: ;
	{
		nimln(350, "ropes.nim");
		result = NIM_FALSE;
	}
	LA1: ;
	popFrame();
	return result;
}
NIM_EXTERNC N_NOINLINE(void, compiler_ropesInit)(void) {
	nimfr("ropes", "ropes.nim")
	nimln(124, "ropes.nim");
	gcachetries_162268 = ((NI) 0);
	nimln(125, "ropes.nim");
	gcachemisses_162272 = ((NI) 0);
	nimln(126, "ropes.nim");
	gcacheinttries_162276 = ((NI) 0);
	nimln(229, "ropes.nim");
	asgnRefNoCycle((void**) (&rnl_163227), newrope_162056(tnl_160626));
	nimln(230, "ropes.nim");
	asgnRefNoCycle((void**) (&softrnl_163228), newrope_162056(tnl_160626));
	popFrame();
}

NIM_EXTERNC N_NOINLINE(void, compiler_ropesDatInit)(void) {
static TNimNode* TMP822[4];
static TNimNode TMP744[5];
NTI162009.size = sizeof(Ropeobj162009);
NTI162009.kind = 17;
NTI162009.base = (&NTI3411);
NTI162009.flags = 2;
TMP822[0] = &TMP744[1];
TMP744[1].kind = 1;
TMP744[1].offset = offsetof(Ropeobj162009, left);
TMP744[1].typ = (&NTI162007);
TMP744[1].name = "left";
TMP822[1] = &TMP744[2];
TMP744[2].kind = 1;
TMP744[2].offset = offsetof(Ropeobj162009, right);
TMP744[2].typ = (&NTI162007);
TMP744[2].name = "right";
TMP822[2] = &TMP744[3];
TMP744[3].kind = 1;
TMP744[3].offset = offsetof(Ropeobj162009, length);
TMP744[3].typ = (&NTI108);
TMP744[3].name = "length";
TMP822[3] = &TMP744[4];
TMP744[4].kind = 1;
TMP744[4].offset = offsetof(Ropeobj162009, data);
TMP744[4].typ = (&NTI149);
TMP744[4].name = "data";
TMP744[0].len = 4; TMP744[0].kind = 2; TMP744[0].sons = &TMP822[0];
NTI162009.node = &TMP744[0];
NTI162007.size = sizeof(Ropeobj162009*);
NTI162007.kind = 22;
NTI162007.base = (&NTI162009);
NTI162007.flags = 2;
NTI162007.marker = TMP823;
}

