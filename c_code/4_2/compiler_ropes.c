/* Generated by Nim Compiler v0.11.2 */
/*   (c) 2015 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 64
#include "nimbase.h"

#include <string.h>

#include <stdio.h>
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct Ropeobj161009 Ropeobj161009;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct Tcell46747 Tcell46747;
typedef struct Tcellseq46763 Tcellseq46763;
typedef struct Tgcheap48616 Tgcheap48616;
typedef struct Tcellset46759 Tcellset46759;
typedef struct Tpagedesc46755 Tpagedesc46755;
typedef struct Tmemregion28610 Tmemregion28610;
typedef struct Tsmallchunk27840 Tsmallchunk27840;
typedef struct Tllchunk28604 Tllchunk28604;
typedef struct Tbigchunk27842 Tbigchunk27842;
typedef struct Tintset27817 Tintset27817;
typedef struct Ttrunk27813 Ttrunk27813;
typedef struct Tavlnode28608 Tavlnode28608;
typedef struct Tgcstat48614 Tgcstat48614;
typedef struct TY161599 TY161599;
typedef struct Tbasechunk27838 Tbasechunk27838;
typedef struct Tfreecell27830 Tfreecell27830;
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
typedef struct {
N_NIMCALL_PTR(void, ClPrc) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning, void* ClEnv);
void* ClEnv;
} TY161023;
typedef N_NIMCALL_PTR(void, TY3289) (void* p, NI op);
typedef N_NIMCALL_PTR(void*, TY3294) (void* p);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY3289 marker;
TY3294 deepcopy;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  Ropeobj161009  {
  TNimObject Sup;
Ropeobj161009* left;
Ropeobj161009* right;
NI length;
NimStringDesc* data;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct  Tcell46747  {
NI refcount;
TNimType* typ;
};
struct  Tcellseq46763  {
NI len;
NI cap;
Tcell46747** d;
};
struct  Tcellset46759  {
NI counter;
NI max;
Tpagedesc46755* head;
Tpagedesc46755** data;
};
typedef Tsmallchunk27840* TY28622[512];
typedef Ttrunk27813* Ttrunkbuckets27815[256];
struct  Tintset27817  {
Ttrunkbuckets27815 data;
};
struct  Tmemregion28610  {
NI minlargeobj;
NI maxlargeobj;
TY28622 freesmallchunks;
Tllchunk28604* llmem;
NI currmem;
NI maxmem;
NI freemem;
NI lastsize;
Tbigchunk27842* freechunkslist;
Tintset27817 chunkstarts;
Tavlnode28608* root;
Tavlnode28608* deleted;
Tavlnode28608* last;
Tavlnode28608* freeavlnodes;
};
struct  Tgcstat48614  {
NI stackscans;
NI cyclecollections;
NI maxthreshold;
NI maxstacksize;
NI maxstackcells;
NI cycletablesize;
NI64 maxpause;
};
struct  Tgcheap48616  {
void* stackbottom;
NI cyclethreshold;
Tcellseq46763 zct;
Tcellseq46763 decstack;
Tcellset46759 cycleroots;
Tcellseq46763 tempstack;
NI recgclock;
Tmemregion28610 region;
Tgcstat48614 stat;
};
typedef Ropeobj161009* TY161101[4096];
typedef Ropeobj161009* TY161572[1];
typedef N_CLOSURE_PTR(void, TMP833) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP845) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP853) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP855) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef N_CLOSURE_PTR(void, TMP859) (NU8 err, NimStringDesc* msg, NIM_BOOL usewarning);
typedef NIM_CHAR TY162814[1024];
typedef NI TY27820[8];
struct  Tpagedesc46755  {
Tpagedesc46755* next;
NI key;
TY27820 bits;
};
struct  Tbasechunk27838  {
NI prevsize;
NI size;
NIM_BOOL used;
};
struct  Tsmallchunk27840  {
  Tbasechunk27838 Sup;
Tsmallchunk27840* next;
Tsmallchunk27840* prev;
Tfreecell27830* freelist;
NI free;
NI acc;
NF data;
};
struct  Tllchunk28604  {
NI size;
NI acc;
Tllchunk28604* next;
};
struct  Tbigchunk27842  {
  Tbasechunk27838 Sup;
Tbigchunk27842* next;
Tbigchunk27842* prev;
NI align;
NF data;
};
struct  Ttrunk27813  {
Ttrunk27813* next;
NI key;
TY27820 bits;
};
typedef Tavlnode28608* TY28614[2];
struct  Tavlnode28608  {
TY28614 link;
NI key;
NI upperbound;
NI level;
};
struct  Tfreecell27830  {
Tfreecell27830* next;
NI zerofield;
};
struct TY161599 {
  TGenericSeq Sup;
  Ropeobj161009* data[SEQ_DECL_SIZE];
};
N_NIMCALL(NI, len_161034)(Ropeobj161009* a);
static N_INLINE(void, nimFrame)(TFrame* s);
N_NOINLINE(void, stackoverflow_20001)(void);
static N_INLINE(void, popFrame)(void);
N_NIMCALL(Ropeobj161009*, newrope_161056)(NimStringDesc* data);
N_NIMCALL(void, nimGCvisit)(void* d, NI op);
N_NIMCALL(void, TMP821)(void* p, NI op);
N_NIMCALL(void*, newObj)(TNimType* typ, NI size);
N_NIMCALL(NimStringDesc*, copyStringRC1)(NimStringDesc* src);
static N_INLINE(void, nimGCunrefNoCycle)(void* p);
static N_INLINE(Tcell46747*, usrtocell_50246)(void* usr);
static N_INLINE(void, rtladdzct_51804)(Tcell46747* c);
N_NOINLINE(void, addzct_50217)(Tcellseq46763* s, Tcell46747* c);
N_NIMCALL(Ropeobj161009*, newmutablerope_161074)(NI capacity);
N_NIMCALL(NimStringDesc*, rawNewString)(NI space);
N_NIMCALL(NimStringDesc*, rawNewString)(NI cap);
static N_INLINE(NI, chckRange)(NI i, NI a, NI b);
N_NOINLINE(void, raiseRangeError)(NI64 val);
static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src);
static N_INLINE(void, freezemutablerope_161092)(Ropeobj161009* r);
N_NIMCALL(void, resetropecache_161111)(void);
static N_INLINE(NI, addInt)(NI a, NI b);
N_NOINLINE(void, raiseOverflow)(void);
N_NIMCALL(NIM_BOOL, ropeinvariant_161246)(Ropeobj161009* r);
N_NIMCALL(Ropeobj161009*, insertincache_161280)(NimStringDesc* s);
N_NIMCALL(NI, hash_131839)(NimStringDesc* x);
static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b);
N_NIMCALL(Ropeobj161009*, rope_161320)(NimStringDesc* s);
N_NIMCALL(void, failedassertimpl_87017)(NimStringDesc* msg);
N_NIMCALL(Ropeobj161009*, rope_161407)(NI64 i);
N_NIMCALL(NimStringDesc*, nimInt64ToStr)(NI64 x);
N_NIMCALL(Ropeobj161009*, rope_161426)(NF f);
N_NIMCALL(NimStringDesc*, nimFloatToStr)(NF f);
N_NIMCALL(Ropeobj161009*, HEX26_161436)(Ropeobj161009* a, Ropeobj161009* b);
N_NIMCALL(Ropeobj161009*, HEX26_161471)(Ropeobj161009* a, NimStringDesc* b);
N_NIMCALL(Ropeobj161009*, HEX26_161482)(NimStringDesc* a, Ropeobj161009* b);
N_NIMCALL(Ropeobj161009*, HEX26_161493)(Ropeobj161009** a, NI aLen0);
N_NOINLINE(void, raiseIndexError)(void);
N_NIMCALL(void, add_161534)(Ropeobj161009** a, Ropeobj161009* b);
N_NIMCALL(void, unsureAsgnRef)(void** dest, void* src);
N_NIMCALL(void, add_161545)(Ropeobj161009** a, NimStringDesc* b);
static N_INLINE(Ropeobj161009*, pop_161595)(TY161599** s);
static N_INLINE(NI, subInt)(NI a, NI b);
N_NIMCALL(TGenericSeq*, setLengthSeq)(TGenericSeq* seq, NI elemsize, NI newlen);
N_NIMCALL(void, writerope_162070)(FILE* f, Ropeobj161009* r);
N_NIMCALL(void*, newSeq)(TNimType* typ, NI len);
N_NIMCALL(TGenericSeq*, incrSeq)(TGenericSeq* seq, NI elemsize);
N_NIMCALL(void, write_13457)(FILE* f, NimStringDesc* s);
N_NIMCALL(void, writerope_162105)(Ropeobj161009* head, NimStringDesc* filename, NIM_BOOL usewarning);
N_NIMCALL(NIM_BOOL, open_13203)(FILE** f, NimStringDesc* filename, NU8 mode, NI bufsize);
N_NIMCALL(NimStringDesc*, HEX24_162131)(Ropeobj161009* r);
N_NIMCALL(NimStringDesc*, mnewString)(NI len);
N_NIMCALL(NimStringDesc*, mnewString)(NI len);
N_NIMCALL(NimStringDesc*, setLengthStr)(NimStringDesc* s, NI newlen);
static N_INLINE(void, appendString)(NimStringDesc* dest, NimStringDesc* src);
N_NIMCALL(NimStringDesc*, resizeString)(NimStringDesc* dest, NI addlen);
N_NIMCALL(Ropeobj161009*, ropeconcat_162164)(Ropeobj161009** a, NI aLen0);
N_NIMCALL(void, prepend_162205)(Ropeobj161009** a, Ropeobj161009* b);
N_NIMCALL(void, prepend_162216)(Ropeobj161009** a, NimStringDesc* b);
N_NIMCALL(Ropeobj161009*, HEX25_162235)(NimStringDesc* frmt, Ropeobj161009** args, NI argsLen0);
N_NIMCALL(NI, mulInt)(NI a, NI b);
N_NIMCALL(NimStringDesc*, nimIntToStr)(NI x);
N_NIMCALL(NimStringDesc*, nimCharToStr)(NIM_CHAR x);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* s, NI start_76643, NI last);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* s, NI first, NI last);
N_NIMCALL(void, addf_162635)(Ropeobj161009** c, NimStringDesc* frmt, Ropeobj161009** args, NI argsLen0);
N_NIMCALL(NIM_BOOL, equalsfile_162809)(Ropeobj161009* r, FILE* f);
N_NIMCALL(NI, readbuffer_13515)(FILE* f, void* buffer, NI len);
N_NIMCALL(NIM_BOOL, equalsfile_162916)(Ropeobj161009* r, NimStringDesc* filename);
N_NIMCALL(NIM_BOOL, writeropeifnotequal_162930)(Ropeobj161009* r, NimStringDesc* filename);
STRING_LITERAL(TMP825, "ropeInvariant(result) ", 22);
STRING_LITERAL(TMP831, "not (it == nil) ", 16);
STRING_LITERAL(TMP832, "not (it.data == nil) ", 21);
STRING_LITERAL(TMP836, "$", 1);
TY161023 errorhandler_161028;
extern TFrame* frameptr_17042;
extern TNimType NTI3411; /* RootObj */
TNimType NTI161009; /* RopeObj */
extern TNimType NTI108; /* int */
extern TNimType NTI149; /* string */
TNimType NTI161007; /* Rope */
extern Tgcheap48616 gch_48644;
TY161101 cache_161106;
NI gcachetries_161268;
NI gcachemisses_161272;
NI gcacheinttries_161276;
extern TNimType NTI161599; /* seq[Rope] */
Ropeobj161009* rnl_162227;
extern NimStringDesc* tnl_159626;
Ropeobj161009* softrnl_162228;

static N_INLINE(void, nimFrame)(TFrame* s) {
	NI LOC1;
	LOC1 = 0;
	{
		if (!(frameptr_17042 == NIM_NIL)) goto LA4;
		LOC1 = ((NI) 0);
	}
	goto LA2;
	LA4: ;
	{
		LOC1 = ((NI) ((NI16)((*frameptr_17042).calldepth + ((NI16) 1))));
	}
	LA2: ;
	(*s).calldepth = ((NI16) (LOC1));
	(*s).prev = frameptr_17042;
	frameptr_17042 = s;
	{
		if (!((*s).calldepth == ((NI16) 2000))) goto LA9;
		stackoverflow_20001();
	}
	LA9: ;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_17042 = (*frameptr_17042).prev;
}

N_NIMCALL(NI, len_161034)(Ropeobj161009* a) {
	NI result;
	nimfr("len", "ropes.nim")
	result = 0;
	nimln(86, "ropes.nim");
	{
		if (!(a == NIM_NIL)) goto LA3;
		result = ((NI) 0);
	}
	goto LA1;
	LA3: ;
	{
		nimln(87, "ropes.nim");
		result = (*a).length;
	}
	LA1: ;
	popFrame();
	return result;
}
N_NIMCALL(void, TMP821)(void* p, NI op) {
	Ropeobj161009* a;
	a = (Ropeobj161009*)p;
	nimGCvisit((void*)(*a).left, op);
	nimGCvisit((void*)(*a).right, op);
	nimGCvisit((void*)(*a).data, op);
}

static N_INLINE(Tcell46747*, usrtocell_50246)(void* usr) {
	Tcell46747* result;
	nimfr("usrToCell", "gc.nim")
	result = 0;
	nimln(118, "gc.nim");
	result = ((Tcell46747*) ((NI)((NU64)(((NI) (usr))) - (NU64)(((NI)sizeof(Tcell46747))))));
	popFrame();
	return result;
}

static N_INLINE(void, rtladdzct_51804)(Tcell46747* c) {
	nimfr("rtlAddZCT", "gc.nim")
	nimln(199, "gc.nim");
	addzct_50217((&gch_48644.zct), c);
	popFrame();
}

static N_INLINE(void, nimGCunrefNoCycle)(void* p) {
	Tcell46747* c;
	nimfr("nimGCunrefNoCycle", "gc.nim")
	nimln(233, "gc.nim");
	c = usrtocell_50246(p);
	nimln(235, "gc.nim");
	{
		nimln(167, "gc.nim");
		(*c).refcount -= ((NI) 8);
		nimln(168, "gc.nim");
		if (!((NU64)((*c).refcount) < (NU64)(((NI) 8)))) goto LA3;
		nimln(236, "gc.nim");
		rtladdzct_51804(c);
	}
	LA3: ;
	popFrame();
}

N_NIMCALL(Ropeobj161009*, newrope_161056)(NimStringDesc* data) {
	Ropeobj161009* result;
	nimfr("newRope", "ropes.nim")
	result = 0;
	nimln(90, "ropes.nim");
	result = (Ropeobj161009*) newObj((&NTI161007), sizeof(Ropeobj161009));
	(*result).Sup.m_type = (&NTI161009);
	nimln(91, "ropes.nim");
	{
		NimStringDesc* LOC5;
		if (!!((data == NIM_NIL))) goto LA3;
		nimln(92, "ropes.nim");
		(*result).length = (data ? data->Sup.len : 0);
		nimln(93, "ropes.nim");
		LOC5 = 0;
		LOC5 = (*result).data; (*result).data = copyStringRC1(data);
		if (LOC5) nimGCunrefNoCycle(LOC5);
	}
	LA3: ;
	popFrame();
	return result;
}

static N_INLINE(NI, chckRange)(NI i, NI a, NI b) {
	NI result;
{	result = 0;
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (a <= i);
		if (!(LOC3)) goto LA4;
		LOC3 = (i <= b);
		LA4: ;
		if (!LOC3) goto LA5;
		result = i;
		goto BeforeRet;
	}
	goto LA1;
	LA5: ;
	{
		raiseRangeError(((NI64) (i)));
	}
	LA1: ;
	}BeforeRet: ;
	return result;
}

static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src) {
	nimfr("asgnRefNoCycle", "gc.nim")
	nimln(251, "gc.nim");
	{
		Tcell46747* c;
		if (!!((src == NIM_NIL))) goto LA3;
		nimln(252, "gc.nim");
		c = usrtocell_50246(src);
		nimln(169, "gc.nim");
		(*c).refcount += ((NI) 8);
	}
	LA3: ;
	nimln(254, "gc.nim");
	{
		Tcell46747* c;
		if (!!(((*dest) == NIM_NIL))) goto LA7;
		nimln(255, "gc.nim");
		c = usrtocell_50246((*dest));
		nimln(256, "gc.nim");
		{
			nimln(167, "gc.nim");
			(*c).refcount -= ((NI) 8);
			nimln(168, "gc.nim");
			if (!((NU64)((*c).refcount) < (NU64)(((NI) 8)))) goto LA11;
			nimln(257, "gc.nim");
			rtladdzct_51804(c);
		}
		LA11: ;
	}
	LA7: ;
	nimln(258, "gc.nim");
	(*dest) = src;
	popFrame();
}

N_NIMCALL(Ropeobj161009*, newmutablerope_161074)(NI capacity) {
	Ropeobj161009* result;
	nimfr("newMutableRope", "ropes.nim")
	result = 0;
	nimln(98, "ropes.nim");
	result = (Ropeobj161009*) newObj((&NTI161007), sizeof(Ropeobj161009));
	(*result).Sup.m_type = (&NTI161009);
	nimln(99, "ropes.nim");
	asgnRefNoCycle((void**) (&(*result).data), rawNewString(((NI)chckRange(capacity, ((NI) 0), ((NI) IL64(9223372036854775807))))));
	popFrame();
	return result;
}

static N_INLINE(void, freezemutablerope_161092)(Ropeobj161009* r) {
	nimfr("freezeMutableRope", "ropes.nim")
	nimln(102, "ropes.nim");
	(*r).length = ((*r).data ? (*r).data->Sup.len : 0);
	popFrame();
}

static N_INLINE(NI, addInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU64)(a) + (NU64)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ b));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

N_NIMCALL(void, resetropecache_161111)(void) {
	nimfr("resetRopeCache", "ropes.nim")
	{
		NI i_161223;
		NI res_161228;
		i_161223 = 0;
		nimln(1598, "system.nim");
		res_161228 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP822;
				if (!(res_161228 <= ((NI) 4095))) goto LA3;
				nimln(1600, "system.nim");
				i_161223 = ((NI)chckRange(res_161228, ((NI) 0), ((NI) 4095)));
				nimln(109, "ropes.nim");
				asgnRefNoCycle((void**) (&cache_161106[(i_161223)- 0]), NIM_NIL);
				nimln(1619, "system.nim");
				TMP822 = addInt(res_161228, ((NI) 1));
				res_161228 = (NI)(TMP822);
			} LA3: ;
		}
	}
	popFrame();
}

N_NIMCALL(NIM_BOOL, ropeinvariant_161246)(Ropeobj161009* r) {
	NIM_BOOL result;
	nimfr("ropeInvariant", "ropes.nim")
	result = 0;
	nimln(112, "ropes.nim");
	{
		if (!(r == NIM_NIL)) goto LA3;
		nimln(113, "ropes.nim");
		result = NIM_TRUE;
	}
	goto LA1;
	LA3: ;
	{
		nimln(115, "ropes.nim");
		result = NIM_TRUE;
	}
	LA1: ;
	popFrame();
	return result;
}

static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b) {
	NIM_BOOL result;
	NIM_BOOL LOC11;
	int LOC13;
{	result = 0;
	{
		if (!(a == b)) goto LA3;
		result = NIM_TRUE;
		goto BeforeRet;
	}
	LA3: ;
	{
		NIM_BOOL LOC7;
		LOC7 = 0;
		LOC7 = (a == NIM_NIL);
		if (LOC7) goto LA8;
		LOC7 = (b == NIM_NIL);
		LA8: ;
		if (!LOC7) goto LA9;
		result = NIM_FALSE;
		goto BeforeRet;
	}
	LA9: ;
	LOC11 = 0;
	LOC11 = ((*a).Sup.len == (*b).Sup.len);
	if (!(LOC11)) goto LA12;
	LOC13 = 0;
	LOC13 = memcmp(((NCSTRING) ((*a).data)), ((NCSTRING) ((*b).data)), (*a).Sup.len);
	LOC11 = (LOC13 == ((NI32) 0));
	LA12: ;
	result = LOC11;
	goto BeforeRet;
	}BeforeRet: ;
	return result;
}

N_NIMCALL(Ropeobj161009*, insertincache_161280)(NimStringDesc* s) {
	Ropeobj161009* result;
	NI TMP823;
	NI h;
	NI LOC1;
	nimfr("insertInCache", "ropes.nim")
	result = 0;
	nimln(129, "ropes.nim");
	TMP823 = addInt(gcachetries_161268, ((NI) 1));
	gcachetries_161268 = (NI)(TMP823);
	nimln(130, "ropes.nim");
	LOC1 = 0;
	LOC1 = hash_131839(s);
	h = (NI)(LOC1 & ((NI) 4095));
	nimln(131, "ropes.nim");
	result = cache_161106[(h)- 0];
	nimln(132, "ropes.nim");
	{
		NIM_BOOL LOC4;
		NI TMP824;
		LOC4 = 0;
		LOC4 = result == 0;
		if (LOC4) goto LA5;
		LOC4 = !(eqStrings((*result).data, s));
		LA5: ;
		if (!LOC4) goto LA6;
		nimln(133, "ropes.nim");
		TMP824 = addInt(gcachemisses_161272, ((NI) 1));
		gcachemisses_161272 = (NI)(TMP824);
		nimln(134, "ropes.nim");
		result = newrope_161056(s);
		nimln(135, "ropes.nim");
		asgnRefNoCycle((void**) (&cache_161106[(h)- 0]), result);
	}
	LA6: ;
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, rope_161320)(NimStringDesc* s) {
	Ropeobj161009* result;
	nimfr("rope", "ropes.nim")
	result = 0;
	nimln(139, "ropes.nim");
	{
		if (!((s ? s->Sup.len : 0) == ((NI) 0))) goto LA3;
		nimln(140, "ropes.nim");
		result = NIM_NIL;
	}
	goto LA1;
	LA3: ;
	{
		nimln(142, "ropes.nim");
		result = insertincache_161280(s);
	}
	LA1: ;
	nimln(143, "ropes.nim");
	{
		NIM_BOOL LOC8;
		LOC8 = 0;
		LOC8 = ropeinvariant_161246(result);
		if (!!(LOC8)) goto LA9;
		failedassertimpl_87017(((NimStringDesc*) &TMP825));
	}
	LA9: ;
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, rope_161407)(NI64 i) {
	Ropeobj161009* result;
	NI TMP826;
	NimStringDesc* LOC1;
	nimfr("rope", "ropes.nim")
	result = 0;
	nimln(147, "ropes.nim");
	TMP826 = addInt(gcacheinttries_161276, ((NI) 1));
	gcacheinttries_161276 = (NI)(TMP826);
	nimln(148, "ropes.nim");
	LOC1 = 0;
	LOC1 = nimInt64ToStr(i);
	result = rope_161320(LOC1);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, rope_161426)(NF f) {
	Ropeobj161009* result;
	NimStringDesc* LOC1;
	nimfr("rope", "ropes.nim")
	result = 0;
	nimln(152, "ropes.nim");
	LOC1 = 0;
	LOC1 = nimFloatToStr(f);
	result = rope_161320(LOC1);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, HEX26_161436)(Ropeobj161009* a, Ropeobj161009* b) {
	Ropeobj161009* result;
	nimfr("&", "ropes.nim")
	result = 0;
	nimln(155, "ropes.nim");
	{
		if (!(a == NIM_NIL)) goto LA3;
		nimln(156, "ropes.nim");
		result = b;
	}
	goto LA1;
	LA3: ;
	{
		nimln(157, "ropes.nim");
		if (!(b == NIM_NIL)) goto LA6;
		nimln(158, "ropes.nim");
		result = a;
	}
	goto LA1;
	LA6: ;
	{
		NI TMP827;
		nimln(160, "ropes.nim");
		result = newrope_161056(NIM_NIL);
		nimln(161, "ropes.nim");
		TMP827 = addInt((*a).length, (*b).length);
		(*result).length = (NI)(TMP827);
		nimln(162, "ropes.nim");
		asgnRefNoCycle((void**) (&(*result).left), a);
		nimln(163, "ropes.nim");
		asgnRefNoCycle((void**) (&(*result).right), b);
	}
	LA1: ;
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, HEX26_161471)(Ropeobj161009* a, NimStringDesc* b) {
	Ropeobj161009* result;
	Ropeobj161009* LOC1;
	nimfr("&", "ropes.nim")
	result = 0;
	nimln(167, "ropes.nim");
	LOC1 = 0;
	LOC1 = rope_161320(b);
	result = HEX26_161436(a, LOC1);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, HEX26_161482)(NimStringDesc* a, Ropeobj161009* b) {
	Ropeobj161009* result;
	Ropeobj161009* LOC1;
	nimfr("&", "ropes.nim")
	result = 0;
	nimln(171, "ropes.nim");
	LOC1 = 0;
	LOC1 = rope_161320(a);
	result = HEX26_161436(LOC1, b);
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, HEX26_161493)(Ropeobj161009** a, NI aLen0) {
	Ropeobj161009* result;
	nimfr("&", "ropes.nim")
	result = 0;
	{
		NI i_161510;
		NI HEX3Atmp_161512;
		NI res_161515;
		i_161510 = 0;
		HEX3Atmp_161512 = 0;
		nimln(175, "ropes.nim");
		HEX3Atmp_161512 = (aLen0-1);
		nimln(1598, "system.nim");
		res_161515 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP828;
				if (!(res_161515 <= HEX3Atmp_161512)) goto LA3;
				nimln(1600, "system.nim");
				i_161510 = res_161515;
				nimln(175, "ropes.nim");
				if ((NU)(i_161510) >= (NU)(aLen0)) raiseIndexError();
				result = HEX26_161436(result, a[i_161510]);
				nimln(1614, "system.nim");
				TMP828 = addInt(res_161515, ((NI) 1));
				res_161515 = (NI)(TMP828);
			} LA3: ;
		}
	}
	popFrame();
	return result;
}

N_NIMCALL(void, add_161534)(Ropeobj161009** a, Ropeobj161009* b) {
	nimfr("add", "ropes.nim")
	nimln(179, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_161436((*a), b));
	popFrame();
}

N_NIMCALL(void, add_161545)(Ropeobj161009** a, NimStringDesc* b) {
	nimfr("add", "ropes.nim")
	nimln(183, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_161471((*a), b));
	popFrame();
}

static N_INLINE(NI, subInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU64)(a) - (NU64)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ (NI)((NU64) ~(b))));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

static N_INLINE(Ropeobj161009*, pop_161595)(TY161599** s) {
	Ropeobj161009* result;
	NI L;
	NI TMP829;
	nimfr("pop", "system.nim")
	result = 0;
	nimln(1926, "system.nim");
	TMP829 = subInt(((*s) ? (*s)->Sup.len : 0), ((NI) 1));
	L = (NI)(TMP829);
	nimln(1927, "system.nim");
	if ((NU)(L) >= (NU)((*s)->Sup.len)) raiseIndexError();
	result = (*s)->data[L];
	nimln(1928, "system.nim");
	(*s) = (TY161599*) setLengthSeq(&((*s))->Sup, sizeof(Ropeobj161009*), ((NI)chckRange(L, ((NI) 0), ((NI) IL64(9223372036854775807)))));
	popFrame();
	return result;
}

N_NIMCALL(void, writerope_162070)(FILE* f, Ropeobj161009* r) {
	nimfr("writeRope", "ropes.nim")
	{
		NimStringDesc* s_162074;
		s_162074 = 0;
		nimln(187, "ropes.nim");
		{
			TY161599* stack_162081;
			TY161572 LOC6;
			if (!!((r == NIM_NIL))) goto LA4;
			nimln(188, "ropes.nim");
			stack_162081 = (TY161599*) newSeq((&NTI161599), 1);
			memset((void*)LOC6, 0, sizeof(LOC6));
			LOC6[0] = r;
			asgnRefNoCycle((void**) (&stack_162081->data[0]), LOC6[0]);
			{
				nimln(189, "ropes.nim");
				while (1) {
					Ropeobj161009* it_162083;
					if (!(((NI) 0) < (stack_162081 ? stack_162081->Sup.len : 0))) goto LA8;
					nimln(190, "ropes.nim");
					it_162083 = pop_161595((&stack_162081));
					{
						nimln(191, "ropes.nim");
						while (1) {
							if (!(*it_162083).data == 0) goto LA10;
							nimln(192, "ropes.nim");
							stack_162081 = (TY161599*) incrSeq(&(stack_162081)->Sup, sizeof(Ropeobj161009*));
							asgnRefNoCycle((void**) (&stack_162081->data[stack_162081->Sup.len-1]), (*it_162083).right);
							nimln(193, "ropes.nim");
							it_162083 = (*it_162083).left;
							nimln(194, "ropes.nim");
							{
								if (!!(!((it_162083 == NIM_NIL)))) goto LA13;
								failedassertimpl_87017(((NimStringDesc*) &TMP831));
							}
							LA13: ;
						} LA10: ;
					}
					nimln(195, "ropes.nim");
					{
						if (!!(!(((*it_162083).data == NIM_NIL)))) goto LA17;
						failedassertimpl_87017(((NimStringDesc*) &TMP832));
					}
					LA17: ;
					nimln(196, "ropes.nim");
					s_162074 = (*it_162083).data;
					nimln(205, "ropes.nim");
					write_13457(f, s_162074);
				} LA8: ;
			}
		}
		LA4: ;
	}
	popFrame();
}

N_NIMCALL(void, writerope_162105)(Ropeobj161009* head, NimStringDesc* filename, NIM_BOOL usewarning) {
	FILE* f;
	nimfr("writeRope", "ropes.nim")
	f = 0;
	nimln(209, "ropes.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = open_13203(&f, filename, ((NU8) 1), ((NI) -1));
		if (!LOC3) goto LA4;
		nimln(210, "ropes.nim");
		{
			if (!!((head == NIM_NIL))) goto LA8;
			writerope_162070(f, head);
		}
		LA8: ;
		nimln(211, "ropes.nim");
		fclose(f);
	}
	goto LA1;
	LA4: ;
	{
		nimln(213, "ropes.nim");
		errorhandler_161028.ClEnv? errorhandler_161028.ClPrc(((NU8) 0), filename, usewarning, errorhandler_161028.ClEnv):((TMP833)(errorhandler_161028.ClPrc))(((NU8) 0), filename, usewarning);
	}
	LA1: ;
	popFrame();
}

static N_INLINE(void, appendString)(NimStringDesc* dest, NimStringDesc* src) {
	memcpy(((NCSTRING) ((&(*dest).data[((*dest).Sup.len)- 0]))), ((NCSTRING) ((*src).data)), (NI)((*src).Sup.len + ((NI) 1)));
	(*dest).Sup.len += (*src).Sup.len;
}

N_NIMCALL(NimStringDesc*, HEX24_162131)(Ropeobj161009* r) {
	NimStringDesc* result;
	NI LOC1;
	nimfr("$", "ropes.nim")
	result = 0;
	nimln(217, "ropes.nim");
	LOC1 = 0;
	LOC1 = len_161034(r);
	result = mnewString(((NI)chckRange(LOC1, ((NI) 0), ((NI) IL64(9223372036854775807)))));
	nimln(218, "ropes.nim");
	result = setLengthStr(result, ((NI) 0));
	{
		NimStringDesc* s_162135;
		s_162135 = 0;
		nimln(187, "ropes.nim");
		{
			TY161599* stack_162140;
			TY161572 LOC7;
			if (!!((r == NIM_NIL))) goto LA5;
			nimln(188, "ropes.nim");
			stack_162140 = (TY161599*) newSeq((&NTI161599), 1);
			memset((void*)LOC7, 0, sizeof(LOC7));
			LOC7[0] = r;
			asgnRefNoCycle((void**) (&stack_162140->data[0]), LOC7[0]);
			{
				nimln(189, "ropes.nim");
				while (1) {
					Ropeobj161009* it_162142;
					if (!(((NI) 0) < (stack_162140 ? stack_162140->Sup.len : 0))) goto LA9;
					nimln(190, "ropes.nim");
					it_162142 = pop_161595((&stack_162140));
					{
						nimln(191, "ropes.nim");
						while (1) {
							if (!(*it_162142).data == 0) goto LA11;
							nimln(192, "ropes.nim");
							stack_162140 = (TY161599*) incrSeq(&(stack_162140)->Sup, sizeof(Ropeobj161009*));
							asgnRefNoCycle((void**) (&stack_162140->data[stack_162140->Sup.len-1]), (*it_162142).right);
							nimln(193, "ropes.nim");
							it_162142 = (*it_162142).left;
							nimln(194, "ropes.nim");
							{
								if (!!(!((it_162142 == NIM_NIL)))) goto LA14;
								failedassertimpl_87017(((NimStringDesc*) &TMP831));
							}
							LA14: ;
						} LA11: ;
					}
					nimln(195, "ropes.nim");
					{
						if (!!(!(((*it_162142).data == NIM_NIL)))) goto LA18;
						failedassertimpl_87017(((NimStringDesc*) &TMP832));
					}
					LA18: ;
					nimln(196, "ropes.nim");
					s_162135 = (*it_162142).data;
					nimln(219, "ropes.nim");
					result = resizeString(result, s_162135->Sup.len + 0);
appendString(result, s_162135);
				} LA9: ;
			}
		}
		LA5: ;
	}
	popFrame();
	return result;
}

N_NIMCALL(Ropeobj161009*, ropeconcat_162164)(Ropeobj161009** a, NI aLen0) {
	Ropeobj161009* result;
	nimfr("ropeConcat", "ropes.nim")
	result = 0;
	{
		NI i_162181;
		NI HEX3Atmp_162183;
		NI res_162186;
		i_162181 = 0;
		HEX3Atmp_162183 = 0;
		nimln(223, "ropes.nim");
		HEX3Atmp_162183 = (aLen0-1);
		nimln(1598, "system.nim");
		res_162186 = ((NI) 0);
		{
			nimln(1599, "system.nim");
			while (1) {
				NI TMP834;
				if (!(res_162186 <= HEX3Atmp_162183)) goto LA3;
				nimln(1600, "system.nim");
				i_162181 = res_162186;
				nimln(223, "ropes.nim");
				if ((NU)(i_162181) >= (NU)(aLen0)) raiseIndexError();
				result = HEX26_161436(result, a[i_162181]);
				nimln(1614, "system.nim");
				TMP834 = addInt(res_162186, ((NI) 1));
				res_162186 = (NI)(TMP834);
			} LA3: ;
		}
	}
	popFrame();
	return result;
}

N_NIMCALL(void, prepend_162205)(Ropeobj161009** a, Ropeobj161009* b) {
	nimfr("prepend", "ropes.nim")
	nimln(225, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_161436(b, (*a)));
	popFrame();
}

N_NIMCALL(void, prepend_162216)(Ropeobj161009** a, NimStringDesc* b) {
	nimfr("prepend", "ropes.nim")
	nimln(226, "ropes.nim");
	unsureAsgnRef((void**) (&(*a)), HEX26_161482(b, (*a)));
	popFrame();
}

N_NIMCALL(Ropeobj161009*, HEX25_162235)(NimStringDesc* frmt, Ropeobj161009** args, NI argsLen0) {
	Ropeobj161009* result;
	NI i;
	NI length;
	NI num;
	nimfr("%", "ropes.nim")
	result = 0;
	nimln(233, "ropes.nim");
	i = ((NI) 0);
	nimln(234, "ropes.nim");
	length = (frmt ? frmt->Sup.len : 0);
	nimln(235, "ropes.nim");
	result = NIM_NIL;
	nimln(236, "ropes.nim");
	num = ((NI) 0);
	{
		nimln(237, "ropes.nim");
		while (1) {
			NI start;
			if (!(i < length)) goto LA2;
			nimln(238, "ropes.nim");
			{
				NI TMP835;
				if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
				if (!((NU8)(frmt->data[i]) == (NU8)(36))) goto LA5;
				nimln(239, "ropes.nim");
				TMP835 = addInt(i, ((NI) 1));
				i = (NI)(TMP835);
				nimln(240, "ropes.nim");
				if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
				switch (((NU8)(frmt->data[i]))) {
				case 36:
				{
					NI TMP837;
					nimln(242, "ropes.nim");
					add_161545(&result, ((NimStringDesc*) &TMP836));
					nimln(243, "ropes.nim");
					TMP837 = addInt(i, ((NI) 1));
					i = (NI)(TMP837);
				}
				break;
				case 35:
				{
					NI TMP838;
					NI TMP839;
					nimln(245, "ropes.nim");
					TMP838 = addInt(i, ((NI) 1));
					i = (NI)(TMP838);
					nimln(246, "ropes.nim");
					if ((NU)(num) >= (NU)(argsLen0)) raiseIndexError();
					add_161534(&result, args[num]);
					nimln(247, "ropes.nim");
					TMP839 = addInt(num, ((NI) 1));
					num = (NI)(TMP839);
				}
				break;
				case 48 ... 57:
				{
					NI j;
					nimln(249, "ropes.nim");
					j = ((NI) 0);
					{
						nimln(250, "ropes.nim");
						while (1) {
							NI TMP840;
							NI TMP841;
							NI TMP842;
							NI TMP843;
							nimln(251, "ropes.nim");
							TMP840 = mulInt(j, ((NI) 10));
							if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
							TMP841 = addInt((NI)(TMP840), ((NI) (((NU8)(frmt->data[i])))));
							TMP842 = subInt((NI)(TMP841), ((NI) 48));
							j = (NI)(TMP842);
							nimln(252, "ropes.nim");
							TMP843 = addInt(i, ((NI) 1));
							i = (NI)(TMP843);
							nimln(253, "ropes.nim");
							{
								if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
								if (!!((((NU8)(frmt->data[i])) >= ((NU8)(48)) && ((NU8)(frmt->data[i])) <= ((NU8)(57))))) goto LA14;
								goto LA10;
							}
							LA14: ;
						}
					} LA10: ;
					nimln(254, "ropes.nim");
					num = j;
					nimln(255, "ropes.nim");
					{
						NI TMP844;
						NimStringDesc* LOC20;
						TMP844 = addInt((argsLen0-1), ((NI) 1));
						if (!((NI)(TMP844) < j)) goto LA18;
						nimln(256, "ropes.nim");
						LOC20 = 0;
						LOC20 = nimIntToStr(j);
						errorhandler_161028.ClEnv? errorhandler_161028.ClPrc(((NU8) 1), LOC20, NIM_FALSE, errorhandler_161028.ClEnv):((TMP845)(errorhandler_161028.ClPrc))(((NU8) 1), LOC20, NIM_FALSE);
					}
					goto LA16;
					LA18: ;
					{
						NI TMP846;
						nimln(258, "ropes.nim");
						TMP846 = subInt(j, ((NI) 1));
						if ((NU)((NI)(TMP846)) >= (NU)(argsLen0)) raiseIndexError();
						add_161534(&result, args[(NI)(TMP846)]);
					}
					LA16: ;
				}
				break;
				case 123:
				{
					NI TMP847;
					NI j;
					nimln(260, "ropes.nim");
					TMP847 = addInt(i, ((NI) 1));
					i = (NI)(TMP847);
					nimln(261, "ropes.nim");
					j = ((NI) 0);
					{
						nimln(262, "ropes.nim");
						while (1) {
							NI TMP848;
							NI TMP849;
							NI TMP850;
							NI TMP851;
							if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
							if (!(((NU8)(frmt->data[i])) >= ((NU8)(48)) && ((NU8)(frmt->data[i])) <= ((NU8)(57)))) goto LA24;
							nimln(263, "ropes.nim");
							TMP848 = mulInt(j, ((NI) 10));
							if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
							TMP849 = addInt((NI)(TMP848), ((NI) (((NU8)(frmt->data[i])))));
							TMP850 = subInt((NI)(TMP849), ((NI) 48));
							j = (NI)(TMP850);
							nimln(264, "ropes.nim");
							TMP851 = addInt(i, ((NI) 1));
							i = (NI)(TMP851);
						} LA24: ;
					}
					nimln(265, "ropes.nim");
					num = j;
					nimln(266, "ropes.nim");
					{
						NI TMP852;
						if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
						if (!((NU8)(frmt->data[i]) == (NU8)(125))) goto LA27;
						TMP852 = addInt(i, ((NI) 1));
						i = (NI)(TMP852);
					}
					goto LA25;
					LA27: ;
					{
						NimStringDesc* LOC30;
						nimln(267, "ropes.nim");
						if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
						LOC30 = 0;
						LOC30 = nimCharToStr(frmt->data[i]);
						errorhandler_161028.ClEnv? errorhandler_161028.ClPrc(((NU8) 1), LOC30, NIM_FALSE, errorhandler_161028.ClEnv):((TMP853)(errorhandler_161028.ClPrc))(((NU8) 1), LOC30, NIM_FALSE);
					}
					LA25: ;
					nimln(269, "ropes.nim");
					{
						NI TMP854;
						NimStringDesc* LOC35;
						TMP854 = addInt((argsLen0-1), ((NI) 1));
						if (!((NI)(TMP854) < j)) goto LA33;
						nimln(270, "ropes.nim");
						LOC35 = 0;
						LOC35 = nimIntToStr(j);
						errorhandler_161028.ClEnv? errorhandler_161028.ClPrc(((NU8) 1), LOC35, NIM_FALSE, errorhandler_161028.ClEnv):((TMP855)(errorhandler_161028.ClPrc))(((NU8) 1), LOC35, NIM_FALSE);
					}
					goto LA31;
					LA33: ;
					{
						NI TMP856;
						nimln(272, "ropes.nim");
						TMP856 = subInt(j, ((NI) 1));
						if ((NU)((NI)(TMP856)) >= (NU)(argsLen0)) raiseIndexError();
						add_161534(&result, args[(NI)(TMP856)]);
					}
					LA31: ;
				}
				break;
				case 110:
				{
					NI TMP857;
					nimln(274, "ropes.nim");
					add_161534(&result, softrnl_162228);
					nimln(275, "ropes.nim");
					TMP857 = addInt(i, ((NI) 1));
					i = (NI)(TMP857);
				}
				break;
				case 78:
				{
					NI TMP858;
					nimln(277, "ropes.nim");
					add_161534(&result, rnl_162227);
					nimln(278, "ropes.nim");
					TMP858 = addInt(i, ((NI) 1));
					i = (NI)(TMP858);
				}
				break;
				default:
				{
					NimStringDesc* LOC40;
					nimln(280, "ropes.nim");
					if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
					LOC40 = 0;
					LOC40 = nimCharToStr(frmt->data[i]);
					errorhandler_161028.ClEnv? errorhandler_161028.ClPrc(((NU8) 1), LOC40, NIM_FALSE, errorhandler_161028.ClEnv):((TMP859)(errorhandler_161028.ClPrc))(((NU8) 1), LOC40, NIM_FALSE);
				}
				break;
				}
			}
			LA5: ;
			nimln(281, "ropes.nim");
			start = i;
			{
				nimln(282, "ropes.nim");
				while (1) {
					if (!(i < length)) goto LA42;
					nimln(283, "ropes.nim");
					{
						NI TMP860;
						if ((NU)(i) > (NU)(frmt->Sup.len)) raiseIndexError();
						if (!!(((NU8)(frmt->data[i]) == (NU8)(36)))) goto LA45;
						TMP860 = addInt(i, ((NI) 1));
						i = (NI)(TMP860);
					}
					goto LA43;
					LA45: ;
					{
						nimln(284, "ropes.nim");
						goto LA41;
					}
					LA43: ;
				} LA42: ;
			} LA41: ;
			nimln(285, "ropes.nim");
			{
				NI TMP861;
				NI TMP862;
				NimStringDesc* LOC52;
				TMP861 = subInt(i, ((NI) 1));
				if (!(start <= (NI)(TMP861))) goto LA50;
				nimln(286, "ropes.nim");
				TMP862 = subInt(i, ((NI) 1));
				LOC52 = 0;
				LOC52 = copyStrLast(frmt, start, (NI)(TMP862));
				add_161545(&result, LOC52);
			}
			LA50: ;
		} LA2: ;
	}
	nimln(287, "ropes.nim");
	{
		NIM_BOOL LOC55;
		LOC55 = 0;
		LOC55 = ropeinvariant_161246(result);
		if (!!(LOC55)) goto LA56;
		failedassertimpl_87017(((NimStringDesc*) &TMP825));
	}
	LA56: ;
	popFrame();
	return result;
}

N_NIMCALL(void, addf_162635)(Ropeobj161009** c, NimStringDesc* frmt, Ropeobj161009** args, NI argsLen0) {
	Ropeobj161009* LOC1;
	nimfr("addf", "ropes.nim")
	nimln(291, "ropes.nim");
	LOC1 = 0;
	LOC1 = HEX25_162235(frmt, args, argsLen0);
	add_161534(c, LOC1);
	popFrame();
}

N_NIMCALL(NIM_BOOL, equalsfile_162809)(Ropeobj161009* r, FILE* f) {
	NIM_BOOL result;
	TY162814 buf;
	NI bpos;
	NI blen;
	NI LOC34;
	nimfr("equalsFile", "ropes.nim")
{	result = 0;
	memset((void*)buf, 0, sizeof(buf));
	nimln(311, "ropes.nim");
	bpos = ((NI) 1024);
	nimln(312, "ropes.nim");
	blen = ((NI) 1024);
	{
		NimStringDesc* s_162837;
		s_162837 = 0;
		nimln(187, "ropes.nim");
		{
			TY161599* stack_162886;
			TY161572 LOC6;
			if (!!((r == NIM_NIL))) goto LA4;
			nimln(188, "ropes.nim");
			stack_162886 = (TY161599*) newSeq((&NTI161599), 1);
			memset((void*)LOC6, 0, sizeof(LOC6));
			LOC6[0] = r;
			asgnRefNoCycle((void**) (&stack_162886->data[0]), LOC6[0]);
			{
				nimln(189, "ropes.nim");
				while (1) {
					Ropeobj161009* it_162888;
					NI spos;
					NI slen;
					if (!(((NI) 0) < (stack_162886 ? stack_162886->Sup.len : 0))) goto LA8;
					nimln(190, "ropes.nim");
					it_162888 = pop_161595((&stack_162886));
					{
						nimln(191, "ropes.nim");
						while (1) {
							if (!(*it_162888).data == 0) goto LA10;
							nimln(192, "ropes.nim");
							stack_162886 = (TY161599*) incrSeq(&(stack_162886)->Sup, sizeof(Ropeobj161009*));
							asgnRefNoCycle((void**) (&stack_162886->data[stack_162886->Sup.len-1]), (*it_162888).right);
							nimln(193, "ropes.nim");
							it_162888 = (*it_162888).left;
							nimln(194, "ropes.nim");
							{
								if (!!(!((it_162888 == NIM_NIL)))) goto LA13;
								failedassertimpl_87017(((NimStringDesc*) &TMP831));
							}
							LA13: ;
						} LA10: ;
					}
					nimln(195, "ropes.nim");
					{
						if (!!(!(((*it_162888).data == NIM_NIL)))) goto LA17;
						failedassertimpl_87017(((NimStringDesc*) &TMP832));
					}
					LA17: ;
					nimln(196, "ropes.nim");
					s_162837 = (*it_162888).data;
					nimln(315, "ropes.nim");
					spos = ((NI) 0);
					nimln(316, "ropes.nim");
					slen = (s_162837 ? s_162837->Sup.len : 0);
					{
						nimln(317, "ropes.nim");
						while (1) {
							NI n;
							NI TMP863;
							NI TMP864;
							NI TMP866;
							NI TMP867;
							if (!(spos < slen)) goto LA20;
							nimln(318, "ropes.nim");
							{
								if (!(bpos == blen)) goto LA23;
								nimln(320, "ropes.nim");
								bpos = ((NI) 0);
								nimln(321, "ropes.nim");
								blen = readbuffer_13515(f, ((void*) ((&buf[(((NI) 0))- 0]))), ((NI) 1024));
								nimln(322, "ropes.nim");
								{
									if (!(blen == ((NI) 0))) goto LA27;
									nimln(323, "ropes.nim");
									result = NIM_FALSE;
									nimln(324, "ropes.nim");
									goto BeforeRet;
								}
								LA27: ;
							}
							LA23: ;
							nimln(325, "ropes.nim");
							TMP863 = subInt(blen, bpos);
							TMP864 = subInt(slen, spos);
							n = (((NI)(TMP863) <= (NI)(TMP864)) ? (NI)(TMP863) : (NI)(TMP864));
							nimln(327, "ropes.nim");
							{
								NI TMP865;
								NIM_BOOL LOC31;
								if ((NU)(bpos) > (NU)(1023)) raiseIndexError();
								TMP865 = addInt(((NI) (s_162837->data)), spos);
								LOC31 = 0;
								LOC31 = equalMem(((void*) ((&buf[(bpos)- 0]))), ((void*) ((NI)(TMP865))), ((NI)chckRange(n, ((NI) 0), ((NI) IL64(9223372036854775807)))));
								if (!!(LOC31)) goto LA32;
								nimln(328, "ropes.nim");
								result = NIM_FALSE;
								nimln(329, "ropes.nim");
								goto BeforeRet;
							}
							LA32: ;
							nimln(330, "ropes.nim");
							TMP866 = addInt(spos, n);
							spos = (NI)(TMP866);
							nimln(331, "ropes.nim");
							TMP867 = addInt(bpos, n);
							bpos = (NI)(TMP867);
						} LA20: ;
					}
				} LA8: ;
			}
		}
		LA4: ;
	}
	nimln(333, "ropes.nim");
	LOC34 = 0;
	LOC34 = readbuffer_13515(f, ((void*) ((&buf[(((NI) 0))- 0]))), ((NI) 1));
	result = (LOC34 == ((NI) 0));
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, equalsfile_162916)(Ropeobj161009* r, NimStringDesc* filename) {
	NIM_BOOL result;
	FILE* f;
	nimfr("equalsFile", "ropes.nim")
	result = 0;
	f = 0;
	nimln(339, "ropes.nim");
	result = open_13203(&f, filename, ((NU8) 0), ((NI) -1));
	nimln(340, "ropes.nim");
	{
		if (!result) goto LA3;
		nimln(341, "ropes.nim");
		result = equalsfile_162809(r, f);
		nimln(342, "ropes.nim");
		fclose(f);
	}
	LA3: ;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, writeropeifnotequal_162930)(Ropeobj161009* r, NimStringDesc* filename) {
	NIM_BOOL result;
	nimfr("writeRopeIfNotEqual", "ropes.nim")
	result = 0;
	nimln(346, "ropes.nim");
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = equalsfile_162916(r, filename);
		if (!!(LOC3)) goto LA4;
		nimln(347, "ropes.nim");
		writerope_162105(r, filename, NIM_FALSE);
		nimln(348, "ropes.nim");
		result = NIM_TRUE;
	}
	goto LA1;
	LA4: ;
	{
		nimln(350, "ropes.nim");
		result = NIM_FALSE;
	}
	LA1: ;
	popFrame();
	return result;
}
NIM_EXTERNC N_NOINLINE(void, compiler_ropesInit)(void) {
	nimfr("ropes", "ropes.nim")
	nimln(124, "ropes.nim");
	gcachetries_161268 = ((NI) 0);
	nimln(125, "ropes.nim");
	gcachemisses_161272 = ((NI) 0);
	nimln(126, "ropes.nim");
	gcacheinttries_161276 = ((NI) 0);
	nimln(229, "ropes.nim");
	asgnRefNoCycle((void**) (&rnl_162227), newrope_161056(tnl_159626));
	nimln(230, "ropes.nim");
	asgnRefNoCycle((void**) (&softrnl_162228), newrope_161056(tnl_159626));
	popFrame();
}

NIM_EXTERNC N_NOINLINE(void, compiler_ropesDatInit)(void) {
static TNimNode* TMP820[4];
static TNimNode TMP742[5];
NTI161009.size = sizeof(Ropeobj161009);
NTI161009.kind = 17;
NTI161009.base = (&NTI3411);
NTI161009.flags = 2;
TMP820[0] = &TMP742[1];
TMP742[1].kind = 1;
TMP742[1].offset = offsetof(Ropeobj161009, left);
TMP742[1].typ = (&NTI161007);
TMP742[1].name = "left";
TMP820[1] = &TMP742[2];
TMP742[2].kind = 1;
TMP742[2].offset = offsetof(Ropeobj161009, right);
TMP742[2].typ = (&NTI161007);
TMP742[2].name = "right";
TMP820[2] = &TMP742[3];
TMP742[3].kind = 1;
TMP742[3].offset = offsetof(Ropeobj161009, length);
TMP742[3].typ = (&NTI108);
TMP742[3].name = "length";
TMP820[3] = &TMP742[4];
TMP742[4].kind = 1;
TMP742[4].offset = offsetof(Ropeobj161009, data);
TMP742[4].typ = (&NTI149);
TMP742[4].name = "data";
TMP742[0].len = 4; TMP742[0].kind = 2; TMP742[0].sons = &TMP820[0];
NTI161009.node = &TMP742[0];
NTI161007.size = sizeof(Ropeobj161009*);
NTI161007.kind = 22;
NTI161007.base = (&NTI161009);
NTI161007.flags = 2;
NTI161007.marker = TMP821;
}

