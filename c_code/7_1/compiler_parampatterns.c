/* Generated by Nim Compiler v0.11.2 */
/*   (c) 2015 Andreas Rumpf */
/* The generated code is subject to the original license. */
#define NIM_INTBITS 32
#include "nimbase.h"

#include <string.h>
typedef struct Tnode189813 Tnode189813;
typedef struct Tlineinfo161338 Tlineinfo161338;
typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct Ttype189849 Ttype189849;
typedef struct Tsym189843 Tsym189843;
typedef struct Tident165021 Tident165021;
typedef struct Tnodeseq189807 Tnodeseq189807;
typedef struct Tidobj165015 Tidobj165015;
typedef struct TNimObject TNimObject;
typedef struct TNimType TNimType;
typedef struct TNimNode TNimNode;
typedef struct Ttypeseq189845 Ttypeseq189845;
typedef struct Tscope189837 Tscope189837;
typedef struct TY189944 TY189944;
typedef struct Tstrtable189817 Tstrtable189817;
typedef struct Tsymseq189815 Tsymseq189815;
typedef struct Tloc189827 Tloc189827;
typedef struct Ropeobj159009 Ropeobj159009;
typedef struct Tlib189831 Tlib189831;
typedef struct TY145211 TY145211;
typedef struct Tcell46747 Tcell46747;
typedef struct Tcellseq46763 Tcellseq46763;
typedef struct Tgcheap48616 Tgcheap48616;
typedef struct Tcellset46759 Tcellset46759;
typedef struct Tpagedesc46755 Tpagedesc46755;
typedef struct Tmemregion28610 Tmemregion28610;
typedef struct Tsmallchunk27840 Tsmallchunk27840;
typedef struct Tllchunk28604 Tllchunk28604;
typedef struct Tbigchunk27842 Tbigchunk27842;
typedef struct Tintset27817 Tintset27817;
typedef struct Ttrunk27813 Ttrunk27813;
typedef struct Tavlnode28608 Tavlnode28608;
typedef struct Tgcstat48614 Tgcstat48614;
typedef struct Tinstantiation189833 Tinstantiation189833;
typedef struct Tlistentry129022 Tlistentry129022;
typedef struct Tbasechunk27838 Tbasechunk27838;
typedef struct Tfreecell27830 Tfreecell27830;
typedef struct TY189933 TY189933;
struct  Tlineinfo161338  {
NI16 line;
NI16 col;
NI32 fileindex;
};
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
struct  Tnode189813  {
Ttype189849* typ;
Tlineinfo161338 info;
NU16 flags;
NU8 kind;
union{
struct {NI64 intval;
} S1;
struct {NF floatval;
} S2;
struct {NimStringDesc* strval;
} S3;
struct {Tsym189843* sym;
} S4;
struct {Tident165021* ident;
} S5;
struct {Tnodeseq189807* sons;
} S6;
} kindU;
NimStringDesc* comment;
};
typedef N_NIMCALL_PTR(void, TY3289) (void* p, NI op);
typedef N_NIMCALL_PTR(void*, TY3294) (void* p);
struct  TNimType  {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
TY3289 marker;
TY3294 deepcopy;
};
struct  TNimObject  {
TNimType* m_type;
};
struct  Tidobj165015  {
  TNimObject Sup;
NI id;
};
struct  Tstrtable189817  {
NI counter;
Tsymseq189815* data;
};
struct  Tloc189827  {
NU8 k;
NU8 s;
NU16 flags;
Ttype189849* t;
Ropeobj159009* r;
Ropeobj159009* heaproot;
};
struct  Tsym189843  {
  Tidobj165015 Sup;
NU8 kind;
union{
struct {Ttypeseq189845* typeinstcache;
Tscope189837* typscope;
} S1;
struct {TY189944* procinstcache;
Tsym189843* gcunsafetyreason;
} S2;
struct {TY189944* usedgenerics;
Tstrtable189817 tab;
} S3;
struct {Tsym189843* guard;
} S4;
} kindU;
NU16 magic;
Ttype189849* typ;
Tident165021* name;
Tlineinfo161338 info;
Tsym189843* owner;
NU32 flags;
Tnode189813* ast;
NU32 options;
NI position;
NI offset;
Tloc189827 loc;
Tlib189831* annex;
Tnode189813* constraint;
};
typedef NU8 TY190031[20];
struct  Tident165021  {
  Tidobj165015 Sup;
NimStringDesc* s;
Tident165021* next;
NI h;
};
struct TY145211 {
NimStringDesc* Field0;
NI Field1;
};
struct  TNimNode  {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct  Tcell46747  {
NI refcount;
TNimType* typ;
};
struct  Tcellseq46763  {
NI len;
NI cap;
Tcell46747** d;
};
struct  Tcellset46759  {
NI counter;
NI max;
Tpagedesc46755* head;
Tpagedesc46755** data;
};
typedef Tsmallchunk27840* TY28622[512];
typedef Ttrunk27813* Ttrunkbuckets27815[256];
struct  Tintset27817  {
Ttrunkbuckets27815 data;
};
struct  Tmemregion28610  {
NI minlargeobj;
NI maxlargeobj;
TY28622 freesmallchunks;
Tllchunk28604* llmem;
NI currmem;
NI maxmem;
NI freemem;
NI lastsize;
Tbigchunk27842* freechunkslist;
Tintset27817 chunkstarts;
Tavlnode28608* root;
Tavlnode28608* deleted;
Tavlnode28608* last;
Tavlnode28608* freeavlnodes;
};
struct  Tgcstat48614  {
NI stackscans;
NI cyclecollections;
NI maxthreshold;
NI maxstacksize;
NI maxstackcells;
NI cycletablesize;
NI64 maxpause;
};
struct  Tgcheap48616  {
void* stackbottom;
NI cyclethreshold;
Tcellseq46763 zct;
Tcellseq46763 decstack;
Tcellset46759 cycleroots;
Tcellseq46763 tempstack;
NI recgclock;
Tmemregion28610 region;
Tgcstat48614 stat;
};
struct  Ttype189849  {
  Tidobj165015 Sup;
NU8 kind;
NU8 callconv;
NU32 flags;
Ttypeseq189845* sons;
Tnode189813* n;
Tsym189843* owner;
Tsym189843* sym;
Tsym189843* destructor;
Tsym189843* deepcopy;
Tsym189843* assignment;
NI64 size;
NI16 align;
NI16 locklevel;
Tloc189827 loc;
};
typedef NIM_BOOL TY268289[65];
struct  Tscope189837  {
NI depthlevel;
Tstrtable189817 symbols;
Tnodeseq189807* usingsyms;
Tscope189837* parent;
};
struct  Ropeobj159009  {
  TNimObject Sup;
Ropeobj159009* left;
Ropeobj159009* right;
NI length;
NimStringDesc* data;
};
struct  Tlistentry129022  {
  TNimObject Sup;
Tlistentry129022* prev;
Tlistentry129022* next;
};
struct  Tlib189831  {
  Tlistentry129022 Sup;
NU8 kind;
NIM_BOOL generated;
NIM_BOOL isoverriden;
Ropeobj159009* name;
Tnode189813* path;
};
typedef NI TY27820[16];
struct  Tpagedesc46755  {
Tpagedesc46755* next;
NI key;
TY27820 bits;
};
struct  Tbasechunk27838  {
NI prevsize;
NI size;
NIM_BOOL used;
};
struct  Tsmallchunk27840  {
  Tbasechunk27838 Sup;
Tsmallchunk27840* next;
Tsmallchunk27840* prev;
Tfreecell27830* freelist;
NI free;
NI acc;
NF data;
};
struct  Tllchunk28604  {
NI size;
NI acc;
Tllchunk28604* next;
};
struct  Tbigchunk27842  {
  Tbasechunk27838 Sup;
Tbigchunk27842* next;
Tbigchunk27842* prev;
NI align;
NF data;
};
struct  Ttrunk27813  {
Ttrunk27813* next;
NI key;
TY27820 bits;
};
typedef Tavlnode28608* TY28614[2];
struct  Tavlnode28608  {
TY28614 link;
NI key;
NI upperbound;
NI level;
};
struct  Tinstantiation189833  {
Tsym189843* sym;
Ttypeseq189845* concretetypes;
TY189933* usedby;
};
struct  Tfreecell27830  {
Tfreecell27830* next;
NI zerofield;
};
struct Tnodeseq189807 {
  TGenericSeq Sup;
  Tnode189813* data[SEQ_DECL_SIZE];
};
struct Ttypeseq189845 {
  TGenericSeq Sup;
  Ttype189849* data[SEQ_DECL_SIZE];
};
struct TY189944 {
  TGenericSeq Sup;
  Tinstantiation189833* data[SEQ_DECL_SIZE];
};
struct Tsymseq189815 {
  TGenericSeq Sup;
  Tsym189843* data[SEQ_DECL_SIZE];
};
struct TY189933 {
  TGenericSeq Sup;
  NI32 data[SEQ_DECL_SIZE];
};
N_NIMCALL(void, patternerror_267051)(Tnode189813* n);
N_NIMCALL(void, localerror_163155)(Tlineinfo161338 info, NU16 msg, NimStringDesc* arg);
N_NIMCALL(NimStringDesc*, rendertree_211058)(Tnode189813* n, NU8 renderflags);
static N_INLINE(void, nimFrame)(TFrame* s);
N_NOINLINE(void, stackoverflow_20001)(void);
static N_INLINE(void, popFrame)(void);
static N_INLINE(void, add_267061)(NimStringDesc** code, NU8 op);
N_NIMCALL(NimStringDesc*, addChar)(NimStringDesc* s, NIM_CHAR c);
N_NIMCALL(NU8, whichalias_267080)(Tsym189843* p);
N_NOINLINE(void, raiseFieldError)(NimStringDesc* f);
N_NOINLINE(void, raiseIndexError)(void);
static N_INLINE(NI, chckRange)(NI i, NI a, NI b);
N_NOINLINE(void, raiseRangeError)(NI64 val);
N_NIMCALL(void, compileconstraints_267113)(Tnode189813* p, NimStringDesc** result);
static N_INLINE(NI, len_190097)(Tnode189813* n);
static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b);
N_NIMCALL(NimStringDesc*, nsuNormalize)(NimStringDesc* s);
N_NIMCALL(NI, hashString)(NimStringDesc* s);
N_NIMCALL(void, internalerror_163218)(NimStringDesc* errmsg);
N_NIMCALL(NimStringDesc*, HEX24_163306)(TY145211 x);
N_NIMCALL(NI, nsuCmpIgnoreStyle)(NimStringDesc* a, NimStringDesc* b);
N_NIMCALL(NimStringDesc*, copyStr)(NimStringDesc* s, NI start);
N_NIMCALL(NimStringDesc*, copyStr)(NimStringDesc* s, NI first);
N_NIMCALL(NimStringDesc*, reprEnum)(NI e_82929, TNimType* typ);
static N_INLINE(NI, addInt)(NI a, NI b);
N_NOINLINE(void, raiseOverflow)(void);
N_NIMCALL(Tnode189813*, semnodekindconstraints_267679)(Tnode189813* p);
N_NIMCALL(void, failedassertimpl_87017)(NimStringDesc* msg);
N_NIMCALL(Tnode189813*, newnodei_191351)(NU8 kind, Tlineinfo161338 info);
N_NIMCALL(NimStringDesc*, rawNewString)(NI space);
N_NIMCALL(NimStringDesc*, rawNewString)(NI cap);
static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src);
static N_INLINE(Tcell46747*, usrtocell_50246)(void* usr);
static N_INLINE(void, rtladdzct_51804)(Tcell46747* c);
N_NOINLINE(void, addzct_50217)(Tcellseq46763* s, Tcell46747* c);
static N_INLINE(NI, subInt)(NI a, NI b);
N_NIMCALL(void, internalerror_163199)(Tlineinfo161338 info, NimStringDesc* errmsg);
N_NIMCALL(NU8, checkforsideeffects_267869)(Tnode189813* n);
static N_INLINE(NIM_BOOL, isroutine_194764)(Tsym189843* s);
N_NIMCALL(NU8, isassignable_268056)(Tsym189843* owner, Tnode189813* n);
N_NIMCALL(Ttype189849*, skiptypes_193167)(Ttype189849* t, NU64 kinds);
N_NIMCALL(NIM_BOOL, comparetypes_226047)(Ttype189849* x, Ttype189849* y, NU8 cmp, NU8 flags);
N_NIMCALL(NU16, getmagic_218403)(Tnode189813* op);
N_NIMCALL(NIM_BOOL, islvalue_268263)(Tnode189813* n);
N_NIMCALL(NIM_BOOL, matchnodekinds_268284)(Tnode189813* p, Tnode189813* n);
static N_INLINE(NIM_BOOL, isatom_194905)(Tnode189813* n);
static NIM_CONST TY190031 TMP4687 = {
0x00, 0x00, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP4688, "strVal", 6);
static NIM_CONST TY190031 TMP4689 = {
0xEC, 0xFF, 0x7F, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP4690, "sons", 4);
static NIM_CONST TY190031 TMP4691 = {
0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP4692, "ident", 5);
STRING_LITERAL(TMP4693, "|", 1);
STRING_LITERAL(TMP4694, "&", 1);
STRING_LITERAL(TMP4695, "~", 1);
STRING_LITERAL(TMP4696, "atom", 4);
STRING_LITERAL(TMP4697, "lit", 3);
STRING_LITERAL(TMP4698, "sym", 3);
STRING_LITERAL(TMP4699, "ident", 5);
STRING_LITERAL(TMP4700, "call", 4);
STRING_LITERAL(TMP4701, "alias", 5);
STRING_LITERAL(TMP4702, "noalias", 7);
STRING_LITERAL(TMP4703, "lvalue", 6);
STRING_LITERAL(TMP4704, "local", 5);
STRING_LITERAL(TMP4705, "sideeffect", 10);
STRING_LITERAL(TMP4706, "nosideeffect", 12);
STRING_LITERAL(TMP4708, "compiler/parampatterns.nim", 26);
NIM_CONST TY145211 TMP4707 = {((NimStringDesc*) &TMP4708),
((NI) 100)}
;
NIM_CONST TY145211 TMP4710 = {((NimStringDesc*) &TMP4708),
((NI) 107)}
;
STRING_LITERAL(TMP4712, "p.kind == nkCurlyExpr ", 22);
STRING_LITERAL(TMP4714, "parameter pattern too complex", 29);
static NIM_CONST TY190031 TMP4715 = {
0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00}
;
STRING_LITERAL(TMP4716, "sym", 3);
extern TFrame* frameptr_17042;
extern TNimType NTI189441; /* TSymKind */
extern TNimType NTI189029; /* TNodeKind */
extern Tgcheap48616 gch_48644;

static N_INLINE(void, nimFrame)(TFrame* s) {
	NI LOC1;
	LOC1 = 0;
	{
		if (!(frameptr_17042 == NIM_NIL)) goto LA4;
		LOC1 = ((NI) 0);
	}
	goto LA2;
	LA4: ;
	{
		LOC1 = ((NI) ((NI16)((*frameptr_17042).calldepth + ((NI16) 1))));
	}
	LA2: ;
	(*s).calldepth = ((NI16) (LOC1));
	(*s).prev = frameptr_17042;
	frameptr_17042 = s;
	{
		if (!((*s).calldepth == ((NI16) 2000))) goto LA9;
		stackoverflow_20001();
	}
	LA9: ;
}

static N_INLINE(void, popFrame)(void) {
	frameptr_17042 = (*frameptr_17042).prev;
}

N_NIMCALL(void, patternerror_267051)(Tnode189813* n) {
	NimStringDesc* LOC1;
	nimfr("patternError", "parampatterns.nim")
	nimln(45, "parampatterns.nim");
	LOC1 = 0;
	LOC1 = rendertree_211058(n, 4);
	localerror_163155((*n).info, ((NU16) 1), LOC1);
	popFrame();
}

static N_INLINE(void, add_267061)(NimStringDesc** code, NU8 op) {
	nimfr("add", "parampatterns.nim")
	nimln(48, "parampatterns.nim");
	(*code) = addChar((*code), ((NIM_CHAR) (((NI) (op)))));
	popFrame();
}

static N_INLINE(NI, chckRange)(NI i, NI a, NI b) {
	NI result;
{	result = 0;
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (a <= i);
		if (!(LOC3)) goto LA4;
		LOC3 = (i <= b);
		LA4: ;
		if (!LOC3) goto LA5;
		result = i;
		goto BeforeRet;
	}
	goto LA1;
	LA5: ;
	{
		raiseRangeError(((NI64) (i)));
	}
	LA1: ;
	}BeforeRet: ;
	return result;
}

N_NIMCALL(NU8, whichalias_267080)(Tsym189843* p) {
	NU8 result;
	nimfr("whichAlias", "parampatterns.nim")
	result = 0;
	nimln(51, "parampatterns.nim");
	{
		if (!!(((*p).constraint == NIM_NIL))) goto LA3;
		nimln(52, "parampatterns.nim");
		if (!(((TMP4687[(*(*p).constraint).kind/8] &(1<<((*(*p).constraint).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
		if ((NU)(((NI) 0)) > (NU)((*(*p).constraint).kindU.S3.strval->Sup.len)) raiseIndexError();
		result = ((NU8)chckRange(((NU8)((*(*p).constraint).kindU.S3.strval->data[((NI) 0)])), ((NU8) 1), ((NU8) 3)));
	}
	goto LA1;
	LA3: ;
	{
		nimln(54, "parampatterns.nim");
		result = ((NU8) 1);
	}
	LA1: ;
	popFrame();
	return result;
}

static N_INLINE(NI, len_190097)(Tnode189813* n) {
	NI result;
	nimfr("len", "ast.nim")
	result = 0;
	nimln(934, "ast.nim");
	{
		if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
		if (!(*n).kindU.S6.sons == 0) goto LA3;
		result = ((NI) 0);
	}
	goto LA1;
	LA3: ;
	{
		nimln(935, "ast.nim");
		if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
		result = ((*n).kindU.S6.sons ? (*n).kindU.S6.sons->Sup.len : 0);
	}
	LA1: ;
	popFrame();
	return result;
}

static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* a, NimStringDesc* b) {
	NIM_BOOL result;
	NIM_BOOL LOC11;
	int LOC13;
{	result = 0;
	{
		if (!(a == b)) goto LA3;
		result = NIM_TRUE;
		goto BeforeRet;
	}
	LA3: ;
	{
		NIM_BOOL LOC7;
		LOC7 = 0;
		LOC7 = (a == NIM_NIL);
		if (LOC7) goto LA8;
		LOC7 = (b == NIM_NIL);
		LA8: ;
		if (!LOC7) goto LA9;
		result = NIM_FALSE;
		goto BeforeRet;
	}
	LA9: ;
	LOC11 = 0;
	LOC11 = ((*a).Sup.len == (*b).Sup.len);
	if (!(LOC11)) goto LA12;
	LOC13 = 0;
	LOC13 = memcmp(((NCSTRING) ((*a).data)), ((NCSTRING) ((*b).data)), (*a).Sup.len);
	LOC11 = (LOC13 == ((NI32) 0));
	LA12: ;
	result = LOC11;
	goto BeforeRet;
	}BeforeRet: ;
	return result;
}

static N_INLINE(NI, addInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU32)(a) + (NU32)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ b));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

N_NIMCALL(void, compileconstraints_267113)(Tnode189813* p, NimStringDesc** result) {
	nimfr("compileConstraints", "parampatterns.nim")
{	nimln(57, "parampatterns.nim");
	switch ((*p).kind) {
	case ((NU8) 27):
	case ((NU8) 29):
	case ((NU8) 30):
	case ((NU8) 31):
	case ((NU8) 26):
	case ((NU8) 28):
	case ((NU8) 32):
	{
		Tident165021* op;
		nimln(59, "parampatterns.nim");
		{
			if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
			if (!!(((*(*p).kindU.S6.sons->data[((NI) 0)]).kind == ((NU8) 2)))) goto LA4;
			nimln(60, "parampatterns.nim");
			if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
			patternerror_267051((*p).kindU.S6.sons->data[((NI) 0)]);
			nimln(61, "parampatterns.nim");
			goto BeforeRet;
		}
		LA4: ;
		nimln(62, "parampatterns.nim");
		if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
		if ((NU)(((NI) 0)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
		if (!(((TMP4691[(*(*p).kindU.S6.sons->data[((NI) 0)]).kind/8] &(1<<((*(*p).kindU.S6.sons->data[((NI) 0)]).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4692));
		op = (*(*p).kindU.S6.sons->data[((NI) 0)]).kindU.S5.ident;
		nimln(63, "parampatterns.nim");
		{
			NI LOC8;
			LOC8 = 0;
			LOC8 = len_190097(p);
			if (!(LOC8 == ((NI) 3))) goto LA9;
			nimln(64, "parampatterns.nim");
			{
				NIM_BOOL LOC13;
				LOC13 = 0;
				LOC13 = eqStrings((*op).s, ((NimStringDesc*) &TMP4693));
				if (LOC13) goto LA14;
				LOC13 = ((*op).Sup.id == ((NI) 49));
				LA14: ;
				if (!LOC13) goto LA15;
				nimln(65, "parampatterns.nim");
				if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
				if ((NU)(((NI) 1)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
				compileconstraints_267113((*p).kindU.S6.sons->data[((NI) 1)], result);
				nimln(66, "parampatterns.nim");
				if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
				if ((NU)(((NI) 2)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
				compileconstraints_267113((*p).kindU.S6.sons->data[((NI) 2)], result);
				nimln(67, "parampatterns.nim");
				add_267061(result, ((NU8) 2));
			}
			goto LA11;
			LA15: ;
			{
				NIM_BOOL LOC18;
				nimln(68, "parampatterns.nim");
				LOC18 = 0;
				LOC18 = eqStrings((*op).s, ((NimStringDesc*) &TMP4694));
				if (LOC18) goto LA19;
				LOC18 = ((*op).Sup.id == ((NI) 2));
				LA19: ;
				if (!LOC18) goto LA20;
				nimln(69, "parampatterns.nim");
				if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
				if ((NU)(((NI) 1)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
				compileconstraints_267113((*p).kindU.S6.sons->data[((NI) 1)], result);
				nimln(70, "parampatterns.nim");
				if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
				if ((NU)(((NI) 2)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
				compileconstraints_267113((*p).kindU.S6.sons->data[((NI) 2)], result);
				nimln(71, "parampatterns.nim");
				add_267061(result, ((NU8) 3));
			}
			goto LA11;
			LA20: ;
			{
				nimln(73, "parampatterns.nim");
				patternerror_267051(p);
			}
			LA11: ;
		}
		goto LA6;
		LA9: ;
		{
			NIM_BOOL LOC24;
			NI LOC25;
			NIM_BOOL LOC27;
			nimln(74, "parampatterns.nim");
			LOC24 = 0;
			LOC25 = 0;
			LOC25 = len_190097(p);
			LOC24 = (LOC25 == ((NI) 2));
			if (!(LOC24)) goto LA26;
			LOC27 = 0;
			LOC27 = eqStrings((*op).s, ((NimStringDesc*) &TMP4695));
			if (LOC27) goto LA28;
			LOC27 = ((*op).Sup.id == ((NI) 45));
			LA28: ;
			LOC24 = LOC27;
			LA26: ;
			if (!LOC24) goto LA29;
			nimln(75, "parampatterns.nim");
			if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 1)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
			compileconstraints_267113((*p).kindU.S6.sons->data[((NI) 1)], result);
			nimln(76, "parampatterns.nim");
			add_267061(result, ((NU8) 4));
		}
		goto LA6;
		LA29: ;
		{
			nimln(78, "parampatterns.nim");
			patternerror_267051(p);
		}
		LA6: ;
	}
	break;
	case ((NU8) 53):
	case ((NU8) 37):
	{
		nimln(80, "parampatterns.nim");
		{
			NI LOC35;
			LOC35 = 0;
			LOC35 = len_190097(p);
			if (!(LOC35 == ((NI) 1))) goto LA36;
			nimln(81, "parampatterns.nim");
			if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
			compileconstraints_267113((*p).kindU.S6.sons->data[((NI) 0)], result);
		}
		goto LA33;
		LA36: ;
		{
			nimln(83, "parampatterns.nim");
			patternerror_267051(p);
		}
		LA33: ;
	}
	break;
	case ((NU8) 2):
	{
		NimStringDesc* spec;
		nimln(85, "parampatterns.nim");
		if (!(((TMP4691[(*p).kind/8] &(1<<((*p).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4692));
		spec = nsuNormalize((*(*p).kindU.S5.ident).s);
		nimln(86, "parampatterns.nim");
		switch (hashString(spec) & 15) {
		case 1: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4706))) goto LA50;
break;
		case 3: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4702))) goto LA46;
if (eqStrings(spec, ((NimStringDesc*) &TMP4703))) goto LA47;
if (eqStrings(spec, ((NimStringDesc*) &TMP4705))) goto LA49;
break;
		case 4: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4698))) goto LA42;
break;
		case 6: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4699))) goto LA43;
if (eqStrings(spec, ((NimStringDesc*) &TMP4704))) goto LA48;
break;
		case 11: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4696))) goto LA40;
break;
		case 12: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4697))) goto LA41;
break;
		case 13: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4700))) goto LA44;
break;
		case 14: 
if (eqStrings(spec, ((NimStringDesc*) &TMP4701))) goto LA45;
break;
		}
		goto LA51;
		LA40: ;
		{
			nimln(87, "parampatterns.nim");
			add_267061(result, ((NU8) 6));
		}
		goto LA52;
		LA41: ;
		{
			nimln(88, "parampatterns.nim");
			add_267061(result, ((NU8) 7));
		}
		goto LA52;
		LA42: ;
		{
			nimln(89, "parampatterns.nim");
			add_267061(result, ((NU8) 5));
		}
		goto LA52;
		LA43: ;
		{
			nimln(90, "parampatterns.nim");
			add_267061(result, ((NU8) 8));
		}
		goto LA52;
		LA44: ;
		{
			nimln(91, "parampatterns.nim");
			add_267061(result, ((NU8) 9));
		}
		goto LA52;
		LA45: ;
		{
			nimln(92, "parampatterns.nim");
			if ((NU)(((NI) 0)) > (NU)((*result)->Sup.len)) raiseIndexError();
			(*result)->data[((NI) 0)] = 2;
		}
		goto LA52;
		LA46: ;
		{
			nimln(93, "parampatterns.nim");
			if ((NU)(((NI) 0)) > (NU)((*result)->Sup.len)) raiseIndexError();
			(*result)->data[((NI) 0)] = 3;
		}
		goto LA52;
		LA47: ;
		{
			nimln(94, "parampatterns.nim");
			add_267061(result, ((NU8) 12));
		}
		goto LA52;
		LA48: ;
		{
			nimln(95, "parampatterns.nim");
			add_267061(result, ((NU8) 13));
		}
		goto LA52;
		LA49: ;
		{
			nimln(96, "parampatterns.nim");
			add_267061(result, ((NU8) 14));
		}
		goto LA52;
		LA50: ;
		{
			nimln(97, "parampatterns.nim");
			add_267061(result, ((NU8) 15));
		}
		goto LA52;
		LA51: ;
		{
			nimln(854, "msgs.nim");
			{
				NimStringDesc* LOC69;
				if (!NIM_FALSE) goto LA67;
				LOC69 = 0;
				LOC69 = HEX24_163306(TMP4707);
				internalerror_163218(LOC69);
			}
			LA67: ;
			{
				NU8 i_267423;
				NI res_267644;
				i_267423 = 0;
				nimln(1598, "system.nim");
				res_267644 = ((NI) 0);
				{
					nimln(1599, "system.nim");
					while (1) {
						NI TMP4709;
						if (!(res_267644 <= ((NI) 25))) goto LA72;
						nimln(1600, "system.nim");
						i_267423 = ((NU8)chckRange(res_267644, ((NU8) 0), ((NU8) 25)));
						nimln(102, "parampatterns.nim");
						{
							NimStringDesc* LOC75;
							NI LOC76;
							LOC75 = 0;
							LOC75 = copyStr(reprEnum(i_267423, (&NTI189441)), ((NI) 2));
							LOC76 = 0;
							LOC76 = nsuCmpIgnoreStyle(LOC75, spec);
							if (!(LOC76 == ((NI) 0))) goto LA77;
							nimln(103, "parampatterns.nim");
							add_267061(result, ((NU8) 10));
							nimln(104, "parampatterns.nim");
							(*result) = addChar((*result), ((NIM_CHAR) (((NI) (i_267423)))));
							nimln(105, "parampatterns.nim");
							goto BeforeRet;
						}
						LA77: ;
						nimln(1619, "system.nim");
						TMP4709 = addInt(res_267644, ((NI) 1));
						res_267644 = (NI)(TMP4709);
					} LA72: ;
				}
			}
			nimln(854, "msgs.nim");
			{
				NimStringDesc* LOC83;
				if (!NIM_FALSE) goto LA81;
				LOC83 = 0;
				LOC83 = HEX24_163306(TMP4710);
				internalerror_163218(LOC83);
			}
			LA81: ;
			{
				NU8 i_267623;
				NI res_267652;
				i_267623 = 0;
				nimln(1598, "system.nim");
				res_267652 = ((NI) 0);
				{
					nimln(1599, "system.nim");
					while (1) {
						NI TMP4711;
						if (!(res_267652 <= ((NI) 158))) goto LA86;
						nimln(1600, "system.nim");
						i_267623 = ((NU8)chckRange(res_267652, ((NU8) 0), ((NU8) 158)));
						nimln(109, "parampatterns.nim");
						{
							NI LOC89;
							LOC89 = 0;
							LOC89 = nsuCmpIgnoreStyle(reprEnum(i_267623, (&NTI189029)), spec);
							if (!(LOC89 == ((NI) 0))) goto LA90;
							nimln(110, "parampatterns.nim");
							add_267061(result, ((NU8) 11));
							nimln(111, "parampatterns.nim");
							(*result) = addChar((*result), ((NIM_CHAR) (((NI) (i_267623)))));
							nimln(112, "parampatterns.nim");
							goto BeforeRet;
						}
						LA90: ;
						nimln(1619, "system.nim");
						TMP4711 = addInt(res_267652, ((NI) 1));
						res_267652 = (NI)(TMP4711);
					} LA86: ;
				}
			}
			nimln(113, "parampatterns.nim");
			patternerror_267051(p);
		}
		LA52: ;
	}
	break;
	default:
	{
		nimln(115, "parampatterns.nim");
		patternerror_267051(p);
	}
	break;
	}
	}BeforeRet: ;
	popFrame();
}

static N_INLINE(Tcell46747*, usrtocell_50246)(void* usr) {
	Tcell46747* result;
	nimfr("usrToCell", "gc.nim")
	result = 0;
	nimln(118, "gc.nim");
	result = ((Tcell46747*) ((NI)((NU32)(((NI) (usr))) - (NU32)(((NI)sizeof(Tcell46747))))));
	popFrame();
	return result;
}

static N_INLINE(void, rtladdzct_51804)(Tcell46747* c) {
	nimfr("rtlAddZCT", "gc.nim")
	nimln(199, "gc.nim");
	addzct_50217((&gch_48644.zct), c);
	popFrame();
}

static N_INLINE(void, asgnRefNoCycle)(void** dest, void* src) {
	nimfr("asgnRefNoCycle", "gc.nim")
	nimln(251, "gc.nim");
	{
		Tcell46747* c;
		if (!!((src == NIM_NIL))) goto LA3;
		nimln(252, "gc.nim");
		c = usrtocell_50246(src);
		nimln(169, "gc.nim");
		(*c).refcount += ((NI) 8);
	}
	LA3: ;
	nimln(254, "gc.nim");
	{
		Tcell46747* c;
		if (!!(((*dest) == NIM_NIL))) goto LA7;
		nimln(255, "gc.nim");
		c = usrtocell_50246((*dest));
		nimln(256, "gc.nim");
		{
			nimln(167, "gc.nim");
			(*c).refcount -= ((NI) 8);
			nimln(168, "gc.nim");
			if (!((NU32)((*c).refcount) < (NU32)(((NI) 8)))) goto LA11;
			nimln(257, "gc.nim");
			rtladdzct_51804(c);
		}
		LA11: ;
	}
	LA7: ;
	nimln(258, "gc.nim");
	(*dest) = src;
	popFrame();
}

static N_INLINE(NI, subInt)(NI a, NI b) {
	NI result;
{	result = 0;
	result = (NI)((NU32)(a) - (NU32)(b));
	{
		NIM_BOOL LOC3;
		LOC3 = 0;
		LOC3 = (((NI) 0) <= (NI)(result ^ a));
		if (LOC3) goto LA4;
		LOC3 = (((NI) 0) <= (NI)(result ^ (NI)((NU32) ~(b))));
		LA4: ;
		if (!LOC3) goto LA5;
		goto BeforeRet;
	}
	LA5: ;
	raiseOverflow();
	}BeforeRet: ;
	return result;
}

N_NIMCALL(Tnode189813*, semnodekindconstraints_267679)(Tnode189813* p) {
	Tnode189813* result;
	nimfr("semNodeKindConstraints", "parampatterns.nim")
	result = 0;
	nimln(120, "parampatterns.nim");
	{
		if (!!(((*p).kind == ((NU8) 40)))) goto LA3;
		failedassertimpl_87017(((NimStringDesc*) &TMP4712));
	}
	LA3: ;
	nimln(121, "parampatterns.nim");
	result = newnodei_191351(((NU8) 20), (*p).info);
	nimln(122, "parampatterns.nim");
	if (!(((TMP4687[(*result).kind/8] &(1<<((*result).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
	asgnRefNoCycle((void**) (&(*result).kindU.S3.strval), rawNewString(((NI) 10)));
	nimln(123, "parampatterns.nim");
	if (!(((TMP4687[(*result).kind/8] &(1<<((*result).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
	(*result).kindU.S3.strval = addChar((*result).kindU.S3.strval, 1);
	nimln(124, "parampatterns.nim");
	{
		NI LOC7;
		LOC7 = 0;
		LOC7 = len_190097(p);
		if (!(((NI) 2) <= LOC7)) goto LA8;
		{
			NI i_267831;
			NI HEX3Atmp_267839;
			NI LOC11;
			NI res_267842;
			i_267831 = 0;
			HEX3Atmp_267839 = 0;
			nimln(125, "parampatterns.nim");
			LOC11 = 0;
			LOC11 = len_190097(p);
			HEX3Atmp_267839 = subInt(LOC11, 1);
			nimln(1598, "system.nim");
			res_267842 = ((NI) 1);
			{
				nimln(1599, "system.nim");
				while (1) {
					NI TMP4713;
					if (!(res_267842 <= HEX3Atmp_267839)) goto LA13;
					nimln(1600, "system.nim");
					i_267831 = res_267842;
					nimln(126, "parampatterns.nim");
					if (((TMP4689[(*p).kind/8] &(1<<((*p).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
					if ((NU)(i_267831) >= (NU)((*p).kindU.S6.sons->Sup.len)) raiseIndexError();
					if (!(((TMP4687[(*result).kind/8] &(1<<((*result).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
					compileconstraints_267113((*p).kindU.S6.sons->data[i_267831], (&(*result).kindU.S3.strval));
					nimln(1619, "system.nim");
					TMP4713 = addInt(res_267842, ((NI) 1));
					res_267842 = (NI)(TMP4713);
				} LA13: ;
			}
		}
		nimln(127, "parampatterns.nim");
		{
			if (!(((TMP4687[(*result).kind/8] &(1<<((*result).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
			if (!(((NI) 63) < ((*result).kindU.S3.strval ? (*result).kindU.S3.strval->Sup.len : 0))) goto LA16;
			nimln(128, "parampatterns.nim");
			internalerror_163199((*p).info, ((NimStringDesc*) &TMP4714));
		}
		LA16: ;
	}
	goto LA5;
	LA8: ;
	{
		nimln(130, "parampatterns.nim");
		patternerror_267051(p);
	}
	LA5: ;
	nimln(131, "parampatterns.nim");
	if (!(((TMP4687[(*result).kind/8] &(1<<((*result).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
	add_267061((&(*result).kindU.S3.strval), ((NU8) 1));
	popFrame();
	return result;
}

static N_INLINE(NIM_BOOL, isroutine_194764)(Tsym189843* s) {
	NIM_BOOL result;
	nimfr("isRoutine", "ast.nim")
	result = 0;
	nimln(1531, "ast.nim");
	result = ((520192 &(1<<(((*s).kind)&31)))!=0);
	popFrame();
	return result;
}

N_NIMCALL(NU8, checkforsideeffects_267869)(Tnode189813* n) {
	NU8 result;
	nimfr("checkForSideEffects", "parampatterns.nim")
{	result = 0;
	nimln(138, "parampatterns.nim");
	switch ((*n).kind) {
	case ((NU8) 27):
	case ((NU8) 29):
	case ((NU8) 30):
	case ((NU8) 31):
	case ((NU8) 26):
	case ((NU8) 28):
	case ((NU8) 32):
	{
		Tnode189813* op;
		nimln(141, "parampatterns.nim");
		if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		op = (*n).kindU.S6.sons->data[((NI) 0)];
		nimln(142, "parampatterns.nim");
		{
			NIM_BOOL LOC4;
			Tsym189843* s;
			LOC4 = 0;
			LOC4 = ((*op).kind == ((NU8) 3));
			if (!(LOC4)) goto LA5;
			if (!(((TMP4715[(*op).kind/8] &(1<<((*op).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			LOC4 = isroutine_194764((*op).kindU.S4.sym);
			LA5: ;
			if (!LOC4) goto LA6;
			nimln(143, "parampatterns.nim");
			if (!(((TMP4715[(*op).kind/8] &(1<<((*op).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			s = (*op).kindU.S4.sym;
			nimln(144, "parampatterns.nim");
			{
				if (!(((*s).flags &(1<<((((NU8) 11))&31)))!=0)) goto LA10;
				nimln(145, "parampatterns.nim");
				result = ((NU8) 1);
				goto BeforeRet;
			}
			LA10: ;
			nimln(147, "parampatterns.nim");
			result = ((NU8) 2);
		}
		goto LA2;
		LA6: ;
		{
			nimln(148, "parampatterns.nim");
			if (!(((*(*op).typ).flags &(1<<((((NU8) 1))&31)))!=0)) goto LA13;
			nimln(150, "parampatterns.nim");
			result = ((NU8) 2);
		}
		goto LA2;
		LA13: ;
		{
			nimln(153, "parampatterns.nim");
			result = ((NU8) 1);
			goto BeforeRet;
		}
		LA2: ;
		{
			NI i_267927;
			NI HEX3Atmp_268007;
			NI LOC17;
			NI res_268010;
			i_267927 = 0;
			HEX3Atmp_268007 = 0;
			nimln(155, "parampatterns.nim");
			LOC17 = 0;
			LOC17 = len_190097(n);
			HEX3Atmp_268007 = subInt(LOC17, 1);
			nimln(1598, "system.nim");
			res_268010 = ((NI) 0);
			{
				nimln(1599, "system.nim");
				while (1) {
					NU8 ret;
					NI TMP4717;
					if (!(res_268010 <= HEX3Atmp_268007)) goto LA19;
					nimln(1600, "system.nim");
					i_267927 = res_268010;
					nimln(156, "parampatterns.nim");
					if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
					if ((NU)(i_267927) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
					ret = checkforsideeffects_267869((*n).kindU.S6.sons->data[i_267927]);
					nimln(157, "parampatterns.nim");
					{
						if (!(ret == ((NU8) 1))) goto LA22;
						result = ret;
						goto BeforeRet;
					}
					goto LA20;
					LA22: ;
					{
						NIM_BOOL LOC25;
						nimln(158, "parampatterns.nim");
						LOC25 = 0;
						LOC25 = (ret == ((NU8) 0));
						if (!(LOC25)) goto LA26;
						LOC25 = (result == ((NU8) 2));
						LA26: ;
						if (!LOC25) goto LA27;
						nimln(159, "parampatterns.nim");
						result = ((NU8) 0);
					}
					goto LA20;
					LA27: ;
					LA20: ;
					nimln(1619, "system.nim");
					TMP4717 = addInt(res_268010, ((NI) 1));
					res_268010 = (NI)(TMP4717);
				} LA19: ;
			}
		}
	}
	break;
	case ((NU8) 0) ... ((NU8) 23):
	{
		nimln(162, "parampatterns.nim");
		result = ((NU8) 2);
	}
	break;
	default:
	{
		nimln(165, "parampatterns.nim");
		result = ((NU8) 2);
		{
			NI i_267976;
			NI HEX3Atmp_268015;
			NI LOC32;
			NI res_268018;
			i_267976 = 0;
			HEX3Atmp_268015 = 0;
			nimln(166, "parampatterns.nim");
			LOC32 = 0;
			LOC32 = len_190097(n);
			HEX3Atmp_268015 = subInt(LOC32, 1);
			nimln(1598, "system.nim");
			res_268018 = ((NI) 0);
			{
				nimln(1599, "system.nim");
				while (1) {
					NU8 ret;
					NI TMP4718;
					if (!(res_268018 <= HEX3Atmp_268015)) goto LA34;
					nimln(1600, "system.nim");
					i_267976 = res_268018;
					nimln(167, "parampatterns.nim");
					if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
					if ((NU)(i_267976) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
					ret = checkforsideeffects_267869((*n).kindU.S6.sons->data[i_267976]);
					nimln(168, "parampatterns.nim");
					{
						if (!(ret == ((NU8) 1))) goto LA37;
						result = ret;
						goto BeforeRet;
					}
					goto LA35;
					LA37: ;
					{
						NIM_BOOL LOC40;
						nimln(169, "parampatterns.nim");
						LOC40 = 0;
						LOC40 = (ret == ((NU8) 0));
						if (!(LOC40)) goto LA41;
						LOC40 = (result == ((NU8) 2));
						LA41: ;
						if (!LOC40) goto LA42;
						nimln(170, "parampatterns.nim");
						result = ((NU8) 0);
					}
					goto LA35;
					LA42: ;
					LA35: ;
					nimln(1619, "system.nim");
					TMP4718 = addInt(res_268018, ((NI) 1));
					res_268018 = (NI)(TMP4718);
				} LA34: ;
			}
		}
	}
	break;
	}
	}BeforeRet: ;
	popFrame();
	return result;
}

N_NIMCALL(NU8, isassignable_268056)(Tsym189843* owner, Tnode189813* n) {
	NU8 result;
	nimfr("isAssignable", "parampatterns.nim")
	result = 0;
	nimln(183, "parampatterns.nim");
	result = ((NU8) 0);
	nimln(184, "parampatterns.nim");
	switch ((*n).kind) {
	case ((NU8) 3):
	{
		nimln(187, "parampatterns.nim");
		{
			if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			if (!((2336 &(1<<(((*(*n).kindU.S4.sym).kind)&31)))!=0)) goto LA4;
			nimln(188, "parampatterns.nim");
			{
				NIM_BOOL LOC8;
				NIM_BOOL LOC9;
				LOC8 = 0;
				LOC9 = 0;
				LOC9 = !((owner == NIM_NIL));
				if (!(LOC9)) goto LA10;
				if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
				LOC9 = ((*owner).Sup.id == (*(*(*n).kindU.S4.sym).owner).Sup.id);
				LA10: ;
				LOC8 = LOC9;
				if (!(LOC8)) goto LA11;
				nimln(189, "parampatterns.nim");
				if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
				LOC8 = !((((*(*n).kindU.S4.sym).flags &(1<<((((NU8) 3))&31)))!=0));
				LA11: ;
				if (!LOC8) goto LA12;
				nimln(190, "parampatterns.nim");
				result = ((NU8) 2);
			}
			goto LA6;
			LA12: ;
			{
				nimln(192, "parampatterns.nim");
				result = ((NU8) 1);
			}
			LA6: ;
		}
		goto LA2;
		LA4: ;
		{
			NIM_BOOL LOC16;
			nimln(193, "parampatterns.nim");
			LOC16 = 0;
			if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			LOC16 = ((*(*n).kindU.S4.sym).kind == ((NU8) 3));
			if (!(LOC16)) goto LA17;
			if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			LOC16 = ((*(*(*n).kindU.S4.sym).typ).kind == ((NU8) 23));
			LA17: ;
			if (!LOC16) goto LA18;
			nimln(194, "parampatterns.nim");
			result = ((NU8) 1);
		}
		goto LA2;
		LA18: ;
		{
			Ttype189849* t;
			nimln(195, "parampatterns.nim");
			if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			if (!((*(*n).kindU.S4.sym).kind == ((NU8) 7))) goto LA21;
			nimln(196, "parampatterns.nim");
			if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			t = skiptypes_193167((*(*n).kindU.S4.sym).typ, 256);
			nimln(197, "parampatterns.nim");
			{
				if (!((*t).kind == ((NU8) 23))) goto LA25;
				result = ((NU8) 4);
			}
			LA25: ;
		}
		goto LA2;
		LA21: ;
		LA2: ;
	}
	break;
	case ((NU8) 45):
	{
		nimln(199, "parampatterns.nim");
		{
			Ttype189849* LOC30;
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			LOC30 = 0;
			LOC30 = skiptypes_193167((*(*n).kindU.S6.sons->data[((NI) 0)]).typ, IL64(211106232576000));
			if (!((*LOC30).kind == ((NU8) 23) || (*LOC30).kind == ((NU8) 21) || (*LOC30).kind == ((NU8) 22))) goto LA31;
			nimln(201, "parampatterns.nim");
			result = ((NU8) 1);
		}
		goto LA28;
		LA31: ;
		{
			nimln(203, "parampatterns.nim");
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			result = isassignable_268056(owner, (*n).kindU.S6.sons->data[((NI) 0)]);
		}
		LA28: ;
		nimln(204, "parampatterns.nim");
		{
			NIM_BOOL LOC36;
			LOC36 = 0;
			LOC36 = !((result == ((NU8) 0)));
			if (!(LOC36)) goto LA37;
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 1)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			if (!(((TMP4715[(*(*n).kindU.S6.sons->data[((NI) 1)]).kind/8] &(1<<((*(*n).kindU.S6.sons->data[((NI) 1)]).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
			LOC36 = (((*(*(*n).kindU.S6.sons->data[((NI) 1)]).kindU.S4.sym).flags &(1<<((((NU8) 18))&31)))!=0);
			LA37: ;
			if (!LOC36) goto LA38;
			nimln(205, "parampatterns.nim");
			result = ((NU8) 3);
		}
		LA38: ;
	}
	break;
	case ((NU8) 42):
	{
		nimln(207, "parampatterns.nim");
		{
			Ttype189849* LOC43;
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			LOC43 = 0;
			LOC43 = skiptypes_193167((*(*n).kindU.S6.sons->data[((NI) 0)]).typ, IL64(211106232576000));
			if (!((*LOC43).kind == ((NU8) 23) || (*LOC43).kind == ((NU8) 21) || (*LOC43).kind == ((NU8) 22))) goto LA44;
			nimln(209, "parampatterns.nim");
			result = ((NU8) 1);
		}
		goto LA41;
		LA44: ;
		{
			nimln(211, "parampatterns.nim");
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			result = isassignable_268056(owner, (*n).kindU.S6.sons->data[((NI) 0)]);
		}
		LA41: ;
	}
	break;
	case ((NU8) 58):
	case ((NU8) 59):
	case ((NU8) 60):
	{
		nimln(215, "parampatterns.nim");
		{
			Ttype189849* LOC50;
			LOC50 = 0;
			LOC50 = skiptypes_193167((*n).typ, IL64(211106247256064));
			if (!((*LOC50).kind == ((NU8) 27) || (*LOC50).kind == ((NU8) 18) || (*LOC50).kind == ((NU8) 17))) goto LA51;
			nimln(217, "parampatterns.nim");
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 1)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			result = isassignable_268056(owner, (*n).kindU.S6.sons->data[((NI) 1)]);
		}
		goto LA48;
		LA51: ;
		{
			NIM_BOOL LOC54;
			nimln(218, "parampatterns.nim");
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 1)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			LOC54 = 0;
			LOC54 = comparetypes_226047((*n).typ, (*(*n).kindU.S6.sons->data[((NI) 1)]).typ, ((NU8) 1), 0);
			if (!LOC54) goto LA55;
			nimln(220, "parampatterns.nim");
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 1)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			result = isassignable_268056(owner, (*n).kindU.S6.sons->data[((NI) 1)]);
		}
		goto LA48;
		LA55: ;
		LA48: ;
	}
	break;
	case ((NU8) 65):
	case ((NU8) 47):
	case ((NU8) 64):
	{
		nimln(222, "parampatterns.nim");
		result = ((NU8) 1);
	}
	break;
	case ((NU8) 67):
	case ((NU8) 66):
	case ((NU8) 46):
	{
		nimln(224, "parampatterns.nim");
		if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
		if ((NU)(((NI) 0)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
		result = isassignable_268056(owner, (*n).kindU.S6.sons->data[((NI) 0)]);
	}
	break;
	case ((NU8) 27):
	case ((NU8) 29):
	case ((NU8) 30):
	case ((NU8) 31):
	case ((NU8) 26):
	case ((NU8) 28):
	case ((NU8) 32):
	{
		nimln(227, "parampatterns.nim");
		{
			NU16 LOC62;
			LOC62 = 0;
			LOC62 = getmagic_218403(n);
			if (!(LOC62 == ((NU16) 150))) goto LA63;
			if (((TMP4689[(*n).kind/8] &(1<<((*n).kind%8)))!=0)) raiseFieldError(((NimStringDesc*) &TMP4690));
			if ((NU)(((NI) 1)) >= (NU)((*n).kindU.S6.sons->Sup.len)) raiseIndexError();
			result = isassignable_268056(owner, (*n).kindU.S6.sons->data[((NI) 1)]);
		}
		LA63: ;
	}
	break;
	default:
	{
	}
	break;
	}
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, islvalue_268263)(Tnode189813* n) {
	NIM_BOOL result;
	NU8 LOC1;
	nimfr("isLValue", "parampatterns.nim")
	result = 0;
	nimln(232, "parampatterns.nim");
	LOC1 = 0;
	LOC1 = isassignable_268056(NIM_NIL, n);
	result = ((22 &(1<<((LOC1)&7)))!=0);
	popFrame();
	return result;
}

static N_INLINE(NIM_BOOL, isatom_194905)(Tnode189813* n) {
	NIM_BOOL result;
	NIM_BOOL LOC1;
	nimfr("isAtom", "ast.nim")
	result = 0;
	nimln(1543, "ast.nim");
	LOC1 = 0;
	LOC1 = (((NU8) 0) <= (*n).kind);
	if (!(LOC1)) goto LA2;
	LOC1 = ((*n).kind <= ((NU8) 23));
	LA2: ;
	result = LOC1;
	popFrame();
	return result;
}

N_NIMCALL(NIM_BOOL, matchnodekinds_268284)(Tnode189813* p, Tnode189813* n) {
	NIM_BOOL result;
	TY268289 stack;
	NI sp;
	NimStringDesc* code;
	NI pc;
	NI TMP4745;
	nimfr("matchNodeKinds", "parampatterns.nim")
	result = 0;
	nimln(239, "parampatterns.nim");
	stack[(((NI) 0))- 0] = NIM_TRUE;
	nimln(240, "parampatterns.nim");
	sp = ((NI) 1);
	nimln(246, "parampatterns.nim");
	if (!(((TMP4687[(*p).kind/8] &(1<<((*p).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4688));
	code = (*p).kindU.S3.strval;
	nimln(247, "parampatterns.nim");
	pc = ((NI) 1);
	{
		nimln(248, "parampatterns.nim");
		while (1) {
			NI TMP4744;
			nimln(249, "parampatterns.nim");
			if ((NU)(pc) > (NU)(code->Sup.len)) raiseIndexError();
			switch (((NU8)chckRange(((NU8)(code->data[pc])), ((NU8) 1), ((NU8) 15)))) {
			case ((NU8) 1):
			{
				nimln(250, "parampatterns.nim");
				goto LA1;
			}
			break;
			case ((NU8) 2):
			{
				NI TMP4719;
				NIM_BOOL LOC5;
				NI TMP4720;
				NI TMP4721;
				NI TMP4722;
				nimln(252, "parampatterns.nim");
				TMP4719 = subInt(sp, ((NI) 2));
				if ((NU)((NI)(TMP4719)) > (NU)(64)) raiseIndexError();
				LOC5 = 0;
				TMP4720 = subInt(sp, ((NI) 1));
				if ((NU)((NI)(TMP4720)) > (NU)(64)) raiseIndexError();
				LOC5 = stack[((NI)(TMP4720))- 0];
				if (LOC5) goto LA6;
				TMP4721 = subInt(sp, ((NI) 2));
				if ((NU)((NI)(TMP4721)) > (NU)(64)) raiseIndexError();
				LOC5 = stack[((NI)(TMP4721))- 0];
				LA6: ;
				stack[((NI)(TMP4719))- 0] = LOC5;
				nimln(253, "parampatterns.nim");
				TMP4722 = subInt(sp, ((NI) 1));
				sp = (NI)(TMP4722);
			}
			break;
			case ((NU8) 3):
			{
				NI TMP4723;
				NIM_BOOL LOC8;
				NI TMP4724;
				NI TMP4725;
				NI TMP4726;
				nimln(255, "parampatterns.nim");
				TMP4723 = subInt(sp, ((NI) 2));
				if ((NU)((NI)(TMP4723)) > (NU)(64)) raiseIndexError();
				LOC8 = 0;
				TMP4724 = subInt(sp, ((NI) 1));
				if ((NU)((NI)(TMP4724)) > (NU)(64)) raiseIndexError();
				LOC8 = stack[((NI)(TMP4724))- 0];
				if (!(LOC8)) goto LA9;
				TMP4725 = subInt(sp, ((NI) 2));
				if ((NU)((NI)(TMP4725)) > (NU)(64)) raiseIndexError();
				LOC8 = stack[((NI)(TMP4725))- 0];
				LA9: ;
				stack[((NI)(TMP4723))- 0] = LOC8;
				nimln(256, "parampatterns.nim");
				TMP4726 = subInt(sp, ((NI) 1));
				sp = (NI)(TMP4726);
			}
			break;
			case ((NU8) 4):
			{
				NI TMP4727;
				NI TMP4728;
				nimln(257, "parampatterns.nim");
				TMP4727 = subInt(sp, ((NI) 1));
				if ((NU)((NI)(TMP4727)) > (NU)(64)) raiseIndexError();
				TMP4728 = subInt(sp, ((NI) 1));
				if ((NU)((NI)(TMP4728)) > (NU)(64)) raiseIndexError();
				stack[((NI)(TMP4727))- 0] = !(stack[((NI)(TMP4728))- 0]);
			}
			break;
			case ((NU8) 5):
			{
				NI TMP4729;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(258, "parampatterns.nim");
				stack[(sp)- 0] = ((*n).kind == ((NU8) 3));
				nimln(244, "parampatterns.nim");
				TMP4729 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4729);
			}
			break;
			case ((NU8) 6):
			{
				NI TMP4730;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				stack[(sp)- 0] = isatom_194905(n);
				nimln(244, "parampatterns.nim");
				TMP4730 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4730);
			}
			break;
			case ((NU8) 7):
			{
				NI TMP4731;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(260, "parampatterns.nim");
				stack[(sp)- 0] = ((*n).kind >= ((NU8) 5) && (*n).kind <= ((NU8) 23));
				nimln(244, "parampatterns.nim");
				TMP4731 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4731);
			}
			break;
			case ((NU8) 8):
			{
				NI TMP4732;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(261, "parampatterns.nim");
				stack[(sp)- 0] = ((*n).kind == ((NU8) 2));
				nimln(244, "parampatterns.nim");
				TMP4732 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4732);
			}
			break;
			case ((NU8) 9):
			{
				NI TMP4733;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(262, "parampatterns.nim");
				stack[(sp)- 0] = ((*n).kind == ((NU8) 27) || (*n).kind == ((NU8) 29) || (*n).kind == ((NU8) 30) || (*n).kind == ((NU8) 31) || (*n).kind == ((NU8) 26) || (*n).kind == ((NU8) 28) || (*n).kind == ((NU8) 32));
				nimln(244, "parampatterns.nim");
				TMP4733 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4733);
			}
			break;
			case ((NU8) 10):
			{
				NU8 kind;
				NI TMP4734;
				NIM_BOOL LOC17;
				NI TMP4735;
				NI TMP4736;
				nimln(264, "parampatterns.nim");
				TMP4734 = addInt(pc, ((NI) 1));
				if ((NU)((NI)(TMP4734)) > (NU)(code->Sup.len)) raiseIndexError();
				kind = ((NU8)chckRange(((NU8)(code->data[(NI)(TMP4734)])), ((NU8) 0), ((NU8) 25)));
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(265, "parampatterns.nim");
				LOC17 = 0;
				LOC17 = ((*n).kind == ((NU8) 3));
				if (!(LOC17)) goto LA18;
				if (!(((TMP4715[(*n).kind/8] &(1<<((*n).kind%8)))!=0))) raiseFieldError(((NimStringDesc*) &TMP4716));
				LOC17 = ((*(*n).kindU.S4.sym).kind == kind);
				LA18: ;
				stack[(sp)- 0] = LOC17;
				nimln(244, "parampatterns.nim");
				TMP4735 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4735);
				nimln(266, "parampatterns.nim");
				TMP4736 = addInt(pc, ((NI) 1));
				pc = (NI)(TMP4736);
			}
			break;
			case ((NU8) 11):
			{
				NU8 kind;
				NI TMP4737;
				NI TMP4738;
				NI TMP4739;
				nimln(268, "parampatterns.nim");
				TMP4737 = addInt(pc, ((NI) 1));
				if ((NU)((NI)(TMP4737)) > (NU)(code->Sup.len)) raiseIndexError();
				kind = ((NU8)chckRange(((NU8)(code->data[(NI)(TMP4737)])), ((NU8) 0), ((NU8) 158)));
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(269, "parampatterns.nim");
				stack[(sp)- 0] = ((*n).kind == kind);
				nimln(244, "parampatterns.nim");
				TMP4738 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4738);
				nimln(270, "parampatterns.nim");
				TMP4739 = addInt(pc, ((NI) 1));
				pc = (NI)(TMP4739);
			}
			break;
			case ((NU8) 12):
			{
				NU8 LOC21;
				NI TMP4740;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(271, "parampatterns.nim");
				LOC21 = 0;
				LOC21 = isassignable_268056(NIM_NIL, n);
				stack[(sp)- 0] = ((6 &(1<<((LOC21)&7)))!=0);
				nimln(244, "parampatterns.nim");
				TMP4740 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4740);
			}
			break;
			case ((NU8) 13):
			{
				NU8 LOC23;
				NI TMP4741;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(272, "parampatterns.nim");
				LOC23 = 0;
				LOC23 = isassignable_268056(NIM_NIL, n);
				stack[(sp)- 0] = (LOC23 == ((NU8) 2));
				nimln(244, "parampatterns.nim");
				TMP4741 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4741);
			}
			break;
			case ((NU8) 14):
			{
				NU8 LOC25;
				NI TMP4742;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(273, "parampatterns.nim");
				LOC25 = 0;
				LOC25 = checkforsideeffects_267869(n);
				stack[(sp)- 0] = (LOC25 == ((NU8) 1));
				nimln(244, "parampatterns.nim");
				TMP4742 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4742);
			}
			break;
			case ((NU8) 15):
			{
				NU8 LOC27;
				NI TMP4743;
				nimln(243, "parampatterns.nim");
				if ((NU)(sp) > (NU)(64)) raiseIndexError();
				nimln(274, "parampatterns.nim");
				LOC27 = 0;
				LOC27 = checkforsideeffects_267869(n);
				stack[(sp)- 0] = !((LOC27 == ((NU8) 1)));
				nimln(244, "parampatterns.nim");
				TMP4743 = addInt(sp, ((NI) 1));
				sp = (NI)(TMP4743);
			}
			break;
			}
			nimln(275, "parampatterns.nim");
			TMP4744 = addInt(pc, ((NI) 1));
			pc = (NI)(TMP4744);
		}
	} LA1: ;
	nimln(276, "parampatterns.nim");
	TMP4745 = subInt(sp, ((NI) 1));
	if ((NU)((NI)(TMP4745)) > (NU)(64)) raiseIndexError();
	result = stack[((NI)(TMP4745))- 0];
	popFrame();
	return result;
}
NIM_EXTERNC N_NOINLINE(void, compiler_parampatternsInit)(void) {
	nimfr("parampatterns", "parampatterns.nim")
	popFrame();
}

NIM_EXTERNC N_NOINLINE(void, compiler_parampatternsDatInit)(void) {
}

